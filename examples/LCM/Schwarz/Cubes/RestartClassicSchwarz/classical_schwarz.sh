...
for (( step=0; step<$1; step++ )); do
   ...
   while [ $iterate_schwarz -eq 1 ]; do 
     #FIXME: write out new Exodus file for each Schwarz step
     ...
     #################  CHECK CONVERGENCE  #########################################
     matlab -nodesktop -nosplash -r "norm_displacements($schwarz_iter, $step);quit;" #this will create an ascii file error containing the value of the error
     ...
     if (($(echo $err '<=' $tol_schwarz | bc -l))); then
       let "num_schwarz_iter=schwarz_iter+1"
       echo "  ...classical Schwarz converged after $num_schwarz_iter Schwarz iterations!"
       iterate_schwarz=0 
     else
