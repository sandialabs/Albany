...
/bin/mail -s "Albany, waterman (KokkosNode=CUDA): $TT tests passed, $TTT tests failed, $TTTT tests not run, $TTTTT timeouts" "ikalash@sandia.gov, mperego@sandia.gov, jwatkin@sandia.gov, amota@sandia.gov, daibane@sandia.gov, tjfulle@sandia.gov" < results_cuda
