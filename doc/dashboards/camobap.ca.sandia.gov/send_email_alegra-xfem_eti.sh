...
echo "Subject: alegra-xfem_eti, camobap.ca.sandia.gov: $TT tests passed, $TTT tests failed, $TTTT tests not run, $TTTTT timeouts" >& a
echo "" >& b
cat a b >& c
cat c results_alegra-xfem_eti >& d
mv d results_alegra-xfem_eti
rm a b c
cat results_alegra-xfem_eti | /usr/sbin/sendmail -F ikalash@camobap.ca.sandia.gov -t "ikalash@sandia.gov"
