...
git pull
cp $BASE_DIR/saved/* $BASE_DIR/repos/ali-perf-data/ali/pm_cuda_biweekly_data
cd $BASE_DIR/repos/ali-perf-data/ali/pm_cuda_biweekly_data
now=$(date +"%Y%m%d")
FILE=ctest-$now.json
git add $FILE
FILE2=LastTest_$now-0100.log.bz2
git add $FILE2
git commit -m "Adding Perlmutter-cuda ALI nightly performance tests." 
git push