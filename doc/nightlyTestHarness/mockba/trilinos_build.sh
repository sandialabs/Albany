...
rm -rf $TRISPIRITBUILDDIR
mkdir $TRISPIRITBUILDDIR
cd $TRISPIRITBUILDDIR

#Configure Trilinos

echo "    Starting Trilinos cmake" ; date
if [ $MPI_BUILD ] ; then
  cp $ALBDIR/doc/nightlyTestHarness/mockba/do-cmake-trilinos-mpi-mockba .
  source ./do-cmake-trilinos-mpi-mockba > $TRILOUTDIR/trilinos_cmake.out 2>&1
fi

echo "    Finished Trilinos cmake, starting make" ; date

#Build Trilinos
/usr/bin/make -j 8  > $TRILOUTDIR/trilinos_make.out 2>&1
