...
cd $ALBDIR
rm -rf $ALBDIR/build_64bit
mkdir $ALBDIR/build_64bit
cd $ALBDIR/build_64bit

echo "    Starting Albany cmake" ; date

if [ $MPI_BUILD ] ; then
  cp $ALBDIR/doc/nightlyTestHarness/do-cmake-albany-mpi-tpetra-64bit .
  source ./do-cmake-albany-mpi-tpetra-64bit > $ALBOUTDIR/albany_cmake_64bit.out 2>&1
#else
...
/usr/bin/make -j 8 > $ALBOUTDIR/albany_make_64bit.out 2>&1
