...
if [ ! $1 ] ; then
    echo "ERROR: run_kdv: run_kdv.sh requires a file as an argument"
...
if [ -s $1 ] ; then
  ...
  echo "ERROR: run_kdv: File not found: $1 argument = " $1
  echo "You must define env variables with required paths!"
  exit
fi
...
time source $SCRIPTDIR/trilinos_checkout_develop.sh

echo; echo "...Starting Albany VOTD Checkout"
time source $SCRIPTDIR/albany_checkout_kdv.sh
...
time source $SCRIPTDIR/trilinos_build_cuda.sh

#rm -rf $SCRIPTDIR/Albany 

echo; echo "...Starting Albany Build"
time source $SCRIPTDIR/albany_build_kdv.sh
