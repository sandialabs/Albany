...
function(do_full_config PKG_IN SOURCE_IN BUILD_IN OPTS_IN RETVAR)
  ...
  do_config("${PKG_IN}" "${SOURCE_IN}" "${BUILD_IN}" "${OPTS_IN}" "${RETVAR}")
endfunction(do_full_config)
...
function(lcm_do_config PACKAGE PACKAGE_STRING SOURCE_DIR BUILD_DIR INSTALL_DIR)
  message("CONFIGURING ${PACKAGE_STRING} ...")
  message("${LINE}")
  ...
  do_full_config("${PACKAGE_STRING}" "${SOURCE_DIR}" "${BUILD_DIR}" "${OPTS}" CONFIG_OK)
  if (NOT CONFIG_OK)
    message(FATAL_ERROR "Failed to configure ${PACKAGE_STRING}")
  endif()
  message("${LINE}")
endfunction(lcm_do_config)

function(lcm_do_build PACKAGE PACKAGE_STRING BUILD_DIR NUM_PROCS)
  message("BUILDING ${PACKAGE_STRING} ...")
  message("${LINE}")
  if (PACKAGE STREQUAL "trilinos")
    do_build("${PACKAGE_STRING}" "${BUILD_DIR}" "${NUM_PROCS}" "all" BUILD_OK)
    if (NOT BUILD_OK)
      message(FATAL_ERROR "*** MAKE COMMAND FAILED ***")
    endif()
    do_build("${PACKAGE_STRING}" "${BUILD_DIR}" "${NUM_PROCS}" "install" BUILD_OK)
    if (NOT BUILD_OK)
...
endfunction(lcm_do_build)

function(lcm_do_clean PACKAGE_STRING PACKAGE BUILD_DIR INSTALL_DIR)
  message("CLEANING UP ${PACKAGE_STRING} ...")
  message("${LINE}")
  if ((PACKAGE STREQUAL "trilinos") AND (EXISTS "${INSTALL_DIR}"))
    file(REMOVE_RECURSE "${INSTALL_DIR}")
  endif()
  if (EXISTS "${BUILD_DIR}")
    file(REMOVE_RECURSE "${BUILD_DIR}")
  endif()
endfunction(lcm_do_clean)

function(lcm_do_test PACKAGE PACKAGE_STRING BUILD_DIR)
  if (NOT EXISTS "${BUILD_DIR}")
    set(MSG "Build directory does not exist. Run:\n")
    set(MSG ${MSG} "  [clean-]config-build.sh ${PACKAGE} ...\n")
    set(MSG ${MSG} "to create.")
    message(FATAL_ERROR "${MSG}")
  endif()
  message("TESTING ${PACKAGE_STRING} ...")
  message("${LINE}")
  set(CTEST_TEST_TIMEOUT "600")
  do_test("${BUILD_DIR}" TEST_OK)
  if (NOT TEST_OK)
    message("Testing of ${PACKAGE_STRING} failed!\n")
  endif()
endfunction(lcm_do_test)
