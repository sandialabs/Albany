...
\title{Installing and compiling \albany{}/\lcm{} and
  \trilinos{} on Fedora, Ubuntu, SEMS/CEE, or HPC Sandia clusters}
...
\trilinos{} on a machine with Fedora Linux 26, Ubuntu Linux 14.04 LTS and 16.04
LTS, the Linux CEE/SEMS environment, or Sandia's HPC systems.

The Linux CEE/SEMS environment and the HPC clusters already have all the
libraries mentioned below. Skip to Section \ref{sec:repo} for setting up the
source code.

\section{Required Packages for Fedora}
The following packages should be installed using the {\tt dnf install} command
\begin{verbatim}
blas
blas-devel
boost
...
The following packages should be installed using the {\tt apt-get install}
command
\begin{verbatim}
libblas3
libblas-dev
...
example, \verb+libboost-graph1.58+ instead of \verb+libboost-graph1.54+ or
\verb+libopenmpi1.10+ instead of \verb+libopenmpi1.6+ and so on. To install the
