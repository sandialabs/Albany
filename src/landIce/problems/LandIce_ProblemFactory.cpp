...
bool LandIceProblemFactory::provides (const std::string& key) const
{
  return key == "LandIce Stokes First Order 2D" ||
         ...
         key == "LandIce Thickness Evolution" ||
         ...
         key == "LandIce Elliptic Mock Model 2D";
}
        ...
        const Teuchos::RCP<ParamLib>&               paramLib) const
{
  ...
  } else if (key == "LandIce Thickness Evolution") {
    problem = Teuchos::rcp(new LandIce::ThicknessEvolution(problemParams, discParams, paramLib, 2));
  ...
  } else if (key == "LandIce Elliptic Mock Model 2D"){
    problem = Teuchos::rcp(new LandIce::EllipticMockModel(problemParams, discParams, paramLib, 2));
