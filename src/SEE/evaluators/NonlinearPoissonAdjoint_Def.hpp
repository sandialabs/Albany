...
NonlinearPoissonAdjoint<EvalT, Traits>::
NonlinearPoissonAdjoint(const Teuchos::ParameterList& p,
  ...
  u_grad_       (p.get<std::string>("Unknown Gradient Name"),
                 dl->qp_vector),
  lambda_       (p.get<std::string>("Unknown Name"),
                 dl->qp_scalar),
  lambda_grad_  (p.get<std::string>("Unknown Gradient Name"),
                 dl->qp_vector),
                 ...
                 dl->node_scalar)
{
  ...
  this->addDependentField(lambda_);
  this->addDependentField(lambda_grad_);
...
//**********************************************************************
template<typename EvalT, typename Traits>
void NonlinearPoissonAdjoint<EvalT, Traits>::
                      ...
                      PHX::FieldManager<Traits>& fm)
{
  ...
  this->utils.setFieldData(lambda_,fm);
  this->utils.setFieldData(lambda_grad_,fm);
...
evaluateFields(typename Traits::EvalData workset)
{
    ...
    for (std::size_t qp = 0; qp < num_qps_; ++qp) {
      for (std::size_t node = 0; node < num_nodes_; ++node) {
        for (std::size_t i = 0; i < num_dims_; ++i) {
          residual_(cell,node) +=
            2.0 * u_(cell,qp) *  w_bf_(cell,node,qp) * 
            u_grad_(cell,qp,i) * lambda_grad_(cell,qp,i) +
            (1.0 + u_(cell,qp)*u_(cell,qp)) *
            w_grad_bf_(cell,node,qp,i) * lambda_grad_(cell,qp,i);
        }
      }
    }
  }
  ...
  for (std::size_t cell = 0; cell < workset.numCells; ++cell) {
    for (std::size_t qp = 0; qp < num_qps_; ++qp) {
      for (std::size_t node = 0; node < num_nodes_; ++node) {
        residual_(cell,node) -=
          w_bf_(cell,node,qp) * u_(cell,qp) * u_(cell,qp);
      }
    }
  }
