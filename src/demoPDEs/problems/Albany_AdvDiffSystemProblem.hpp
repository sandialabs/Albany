...
#ifndef ALBANY_ADVDIFFSYSTEM_HPP
#define ALBANY_ADVDIFFSYSTEM_HPP
...
namespace Albany {
   ...
   * problem.
   */
  class AdvDiffSystemProblem : public AbstractProblem {
  public:
  
    //! Default constructor
    AdvDiffSystemProblem(const Teuchos::RCP<Teuchos::ParameterList>& params,
  ...
  private:
    ...
    AdvDiffSystemProblem(const AdvDiffSystemProblem&);
    
    //! Private to prohibit copying
    AdvDiffSystemProblem& operator=(const AdvDiffSystemProblem&);
...
Albany::AdvDiffSystemProblem::constructEvaluators(
  PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
  const Albany::MeshSpecsStruct& meshSpecs,
  Albany::StateManager& stateMgr,
  ...
  const Teuchos::RCP<Teuchos::ParameterList>& responseList)
{
  ...
  if(boost::is_same<EvalT, PHAL::AlbanyTraits::Residual>::value)
   ...
   TEUCHOS_TEST_FOR_EXCEPTION(dl->vectorAndGradientLayoutsAreEquivalent==false, std::logic_error,
                              "Data Layout Usage in AdvDiffSystemProblem problem assumes vecDim = numDim");
   ...
   TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      std::logic_error,
      "Albany_AdvDiffSystemProblem must be defined as a steady or transient calculation.");
   ...
   /*{ // Phi Residual
    ...
    ev = rcp(new PHAL::Resid<EvalT,AlbanyTraits>(*p));
...
#endif // ALBANY_ADVDIFFSYSTEM_HPP
