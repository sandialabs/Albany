      ...
      commT_(commT)
{
  ...
  const Teuchos::RCP<const Tpetra_CrsGraph> overlapJacGraphT =
      disc_->getImplicitOverlapJacobianGraphT();
#else
  const Teuchos::RCP<const Tpetra_CrsGraph> overlapJacGraphT =
      disc_->getOverlapJacobianGraphT();
#endif
...

  {
    ...
    if (Teuchos::nonnull(initial_guessT)) {
      *current_soln->getVectorNonConst(0) = *initial_guessT;
      ...
      overlapped_soln->getVectorNonConst(0)->doImport(
          *current_soln->getVector(0), *importerT, Tpetra::INSERT);

      AAdapt::InitialConditionsT(
          overlapped_soln->getVectorNonConst(0),
          wsElNodeEqID,
          wsEBNames,
          coords,
          neq,
      ...
      current_soln->getVectorNonConst(0)->doExport(
          *overlapped_soln->getVector(0), *exporterT, Tpetra::INSERT);

      if (num_time_deriv > 0) {
        overlapped_soln->getVectorNonConst(1)->doImport(
            *current_soln->getVector(1), *importerT, Tpetra::INSERT);
        AAdapt::InitialConditionsT(
            overlapped_soln->getVectorNonConst(1),
            wsElNodeEqID,
            wsEBNames,
            coords,
            neq,
        ...
        current_soln->getVectorNonConst(1)->doExport(
            *overlapped_soln->getVector(1), *exporterT, Tpetra::INSERT);
      }

      if (num_time_deriv > 1) {
        overlapped_soln->getVectorNonConst(2)->doImport(
            *current_soln->getVector(2), *importerT, Tpetra::INSERT);
        AAdapt::InitialConditionsT(
            overlapped_soln->getVectorNonConst(2),
            wsElNodeEqID,
            wsEBNames,
            coords,
            neq,
        ...
        current_soln->getVectorNonConst(2)->doExport(
            *overlapped_soln->getVector(2), *exporterT, Tpetra::INSERT);
      }
    }
  }
#if defined(ALBANY_SCOREC)
    ...
    const Teuchos::RCP<rc::Manager>& rc_mgr)
{
  ...
  if (method == "Copy Remesh") {
    adapter_ = Teuchos::rcp(
        new AAdapt::CopyRemeshT(adaptParams_, paramLib_, stateMgr_, commT_));
  } else
#if defined(ALBANY_LCM) && defined(ALBANY_BGL)
      if (method == "Topmod") {
    adapter_ = Teuchos::rcp(
        new AAdapt::TopologyModT(adaptParams_, paramLib_, stateMgr_, commT_));
  } else if (method == "Erosion") {
    adapter_ = Teuchos::rcp(
        new AAdapt::ErosionT(adaptParams_, paramLib_, stateMgr_, commT_));
  } else
#endif
#endif
  ...
  if (method == "Random") {
    ...
    adapter_ = Teuchos::rcp(new AAdapt::MeshAdapt(
        adaptParams_, paramLib_, stateMgr_, rc_mgr, commT_));
  } else
#endif
#if defined(ALBANY_LCM) && defined(ALBANY_STK_PERCEPT)
      if (method == "Unif Size") {
    adapter_ = Teuchos::rcp(new AAdapt::STKAdaptT<AAdapt::STKUnifRefineField>(
...
bool
AAdapt::AdaptiveSolutionManagerT::adaptProblem()
{
...
void
AAdapt::AdaptiveSolutionManagerT::resizeMeshDataArrays(
    ...
    const Teuchos::RCP<const Tpetra_CrsGraph>& overlapJacGraphT)
{
  ...
  overlapped_soln = Teuchos::rcp(
      new Tpetra_MultiVector(overlapMapT, num_time_deriv + 1, false));
  ...
  cas_manager = Teuchos::rcp(
      new Albany::CombineAndScatterManagerTpetra(owned_vs, overlapped_vs));
}
...
{
  overlapped_soln->getVectorNonConst(0)->doImport(
      solutionT, *importerT, Tpetra::INSERT);
...
{
  overlapped_soln->getVectorNonConst(1)->doImport(
      solution_dotT, *importerT, Tpetra::INSERT);
...
{
  overlapped_soln->getVectorNonConst(2)->doImport(
      solution_dotdotT, *importerT, Tpetra::INSERT);
...
{
  overlapped_soln->getVectorNonConst(0)->doImport(
      xT, *importerT, Tpetra::INSERT);

  if (x_dotT) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        overlapped_soln->getNumVectors() < 2,
        std::logic_error,
        "AdaptiveSolutionManager error: x_dotT defined but only a single "
        "solution vector is available");
    overlapped_soln->getVectorNonConst(1)->doImport(
        *x_dotT, *importerT, Tpetra::INSERT);
  }

  if (x_dotdotT) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        overlapped_soln->getNumVectors() < 3,
        std::logic_error,
        "AdaptiveSolutionManager error: x_dotdotT defined but xDotDot isn't "
        "defined in the multivector");
    overlapped_soln->getVectorNonConst(2)->doImport(
        *x_dotdotT, *importerT, Tpetra::INSERT);

    /*OG uncomment this to enable Laplace calculations in Aeras::Hydrostatic
   if(overlapped_soln->getNumVectors() == 3)
      overlapped_soln->getVectorNonConst(2)->doImport(*x_dotdotT, *importerT,
   Tpetra::INSERT);
      */
  }
}
...
{
  cas_manager->scatter(
      x, overlapped_soln_thyra->col(0), Albany::CombineMode::INSERT);

  if (!x_dot.is_null()) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        overlapped_soln_thyra->domain()->dim() < 2,
        std::logic_error,
        "AdaptiveSolutionManager error: x_dot defined but only a single "
        "solution vector is available");
    cas_manager->scatter(
        x_dot, overlapped_soln_thyra->col(1), Albany::CombineMode::INSERT);
  }

  if (!x_dotdot.is_null()) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        overlapped_soln_thyra->domain()->dim() < 3,
        std::logic_error,
        "AdaptiveSolutionManager error: x_dotdot defined but only two solution "
        "vectors are available");
    cas_manager->scatter(
        x_dotdot, overlapped_soln_thyra->col(2), Albany::CombineMode::INSERT);
  }
}

Teuchos::RCP<Thyra::MultiVectorBase<double>>
AAdapt::AdaptiveSolutionManagerT::getCurrentSolution()
{
  return Thyra::createMultiVector<ST, LO, Tpetra_GO, KokkosNode>(current_soln);
}

void
AAdapt::AdaptiveSolutionManagerT::projectCurrentSolution()
{
