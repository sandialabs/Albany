...
AAdapt::RandomFracture::queryAdaptationCriteria()
{
  ...
  if (iter % fracture_interval_ == 0) {
    ...
    stk::mesh::get_selected_entities(
        select_owned, bulk_data_->buckets(num_dim_ - 1), face_list);
    ...
    const Epetra_Vector& ovlp_solution)
{
  ...
  *output_stream_ << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
                  << std::endl;
  *output_stream_ << "Completed mesh adaptation                           "
                  << std::endl;
  *output_stream_ << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
                  << std::endl;
...
AAdapt::RandomFracture::getValidAdapterParameters() const
{
  ...
  validPL->set<double>("Fracture Probability", 1.0, "Probability of fracture");
  validPL->set<double>(
      "Adaptivity Step Interval", 1, "Interval to check for fracture");
...
AAdapt::RandomFracture::showTopLevelRelations()
{
  ...
  for (int i = 0; i < element_list.size(); ++i) {
    ...
    std::cout << "Entitiy " << element.identifier()
              << " relations are :" << std::endl;

    for (int j = 0; j < relations.size(); ++j) {
      std::cout << "entity:\t" << relations[j].entity()->identifier() << ","
                << relations[j].entity()->entity_rank()
                << "\tlocal id: " << relations[j].identifier() << "\n";
    }
  }
}
    ...
    std::map<EntityKey, bool>& global_entity_open)
{
  ...
  BOOST_FOREACH (me, local_entity_open) {
    ...
    /*
      ...
      const std::string & entity_rank_name = meta_data_->entity_rank_name(
  ...
  EntityKey::raw_key_type* result_array =
      new EntityKey::raw_key_type[total_number_of_open_entities];
  MPI_Allgatherv(
      &v[0],
      num_open_on_pe,
      MPI_UINT64_T,
  ...
  for (int i = 0; i < total_number_of_open_entities; i++) {
    ...
    const std::string&        entity_rank_name =
        meta_data_->entity_rank_name(entity_rank);
    Entity entity = bulk_data_->get_entity(key);
    std::cout << "Global proc fracture list contains "
              << " " << entity_rank_name << " [" << entity_id
              << "] Proc:" << entity->owner_rank() << std::endl;
  }
