...
evaluateResponse(const double current_time,
		...
		Tpetra_Vector& gT)
{
  int dbMode = (comm->getRank() == 0) ? debugMode : 0;
  if(comm->getRank() != 0) outputFilename = ""; //Only root process outputs to files
  if(comm->getRank() != 0) debugFilename = ""; //Only root process outputs to files
  ...
  if(maxIterations > 0) {
    ...
    doNudgedElasticBand(current_time, x, xdot, p, gT, dbMode);
  }
  else {
  ...
  doLevelSet(current_time, x, xdot, p, gT, dbMode);
  /*
		     ...
		     Tpetra_Vector& gT, int dbMode)
{
  ...
  if(beginRegionType == "Point") {
    ...
    Teuchos::reduceAll(*comm, Teuchos::REDUCE_MIN, imagePtsValue, Teuchos::ptr(&globalMin));
    if( fabs(imagePts[0].value - globalMin) < 1e-8 ) {
      procToBcast = comm->getRank();
    } else {
      procToBcast = -1;
    }
    ...
    Teuchos::reduceAll(*comm, Teuchos::REDUCE_MAX, procToBcastInt, Teuchos::ptr(&winner));
    double *imagePtsData = imagePts[0].coords.data();
    Teuchos::broadcast<LO, ST>(*comm, winner, imagePtsData);
  ...
  while( ++nIters <= maxIterations) {
    ...
    if(nIters > backtraceAfterIters) {
      while(dt > minTimeStep && highestPtGradNorm > acceptedHighestPtGradNorm) {
        ...
        getImagePointValues(current_time, x, xdot, p, gT,
                globalPtValues, globalPtWeights, globalPtGrads,
                lastPositions, dbMode);
        iHighestPt = getHighestPtIndex();
        highestPtGradNorm = imagePts[iHighestPt].grad.norm();
    ...
    for(std::size_t i=1; i<nImagePts-1; i++) {
      ...
      if((int)i == iHighestPt && bClimbing && nIters > nInitialIterations) {
        computeClimbingForce(i, tangent, gradScale, force[i], dbMode);
	      ...
	      computeForce(i, tangent, springConstants, gradScale, springScale, force[i], dt, dt2, dbMode);
      }
    ...
    if(imagePts[iHighestPt].grad.norm() < convergeTolerance) {
      if(dbMode > 2) {
        std::cout << "Saddle Point:  ** Converged (grad norm " <<
	                    imagePts[iHighestPt].grad.norm() << " < " << convergeTolerance << ")" << std::endl;
      }
      break; // exit iterations loop
    } else if(nIters == maxIterations) {
      break; //max iterations reached -- exit iterations loop now
    ...
    if(bAdaptivePointSize) {
      for(std::size_t i=0; i<nImagePts; i++) {
        if(imagePts[i].weight < minAdaptivePointWt) {
          imagePts[i].radius *= 2;
        } else if(imagePts[i].weight > maxAdaptivePointWt) {
          imagePts[i].radius /= 2;
        }
      }
    }
	   ...
	   Tpetra_Vector& gT, int dbMode)
{
  ...
  if( fabs(levelSetRadius) < 1e-9 ) {
    return; //don't run if level-set radius is zero
  }
  ...
  QCAD::gatherVectorT(vlsFieldValues, allFieldVals, comm);
  QCAD::gatherVectorT(vlsCellAreas, allCellAreas, comm);
  for(std::size_t k=0; k<numDims; k++) {
    QCAD::gatherVectorT(vlsCoords[k], allCoords[k], comm);
  }
		...
		Tpetra_Vector& gT)
{
  ...
  for(std::size_t i=0; i < N; i++) {
      ...
      if( QCAD::distance(allCoords, I, J, numDims) < cutoffDistance ) {
        if(treeIDs[I] == -1) {
          ...
          treeSizes[treeIDs[I]]++;
	        ...
	        if(dbMode > 3) {
            std::cout << "DEBUG:   also --> " << treeIDs[J]
	                  << " [merged] size=" << treeSizes[treeIDs[J]]
	                  << " (treecount after merge = " << nTrees << ")" << std::endl;
          }

	        if(dbMode > 1) {
            std::cout << "--- Saddle: i=" << i << "merge: nPools=" << nTrees
                      << " nDeep=" << nDeepTrees << std::endl;
          }
    ...
    if(treeIDs[I] == -1) {
      if(dbMode > 3) {
         std::cout << " --> new tree with ID " << nextAvailableTreeID
			             << " (treecount after new = " << (nTrees+1) << ")" << std::endl;
      }
      if(dbMode > 1) {
        std::cout << "--- Saddle: i=" << i << " new pool: nPools=" << (nTrees+1)
			            << " nDeep=" << nDeepTrees << std::endl;
      }
		...
		Tpetra_MultiVector* dg_dpT)
{
  ...
  if(gT != NULL) {
    ...
    if(!matchesCurrentResultsT(*gT)) { 
      evaluateResponse(current_time, x, xdot, Teuchos::null, p, *gT);
    }

    mode = "Fill saddle point";
    Albany::FieldManagerScalarResponseFunction::evaluateGradient(
	   current_time, x, xdot, xdotdot, p, deriv_p, gT, dg_dxT, dg_dxdotT, dg_dxdotdotT, dg_dpT);
  }
  else {
		    ...
		    int dbMode)
{
  ...
  Teuchos::reduceAll<LO, ST>(*comm, Teuchos::REDUCE_SUM, nImagePts, imagePtValues.data(), globalPtValues);
  Teuchos::reduceAll<LO, ST>(*comm, Teuchos::REDUCE_SUM, nImagePts, imagePtWeights.data(), globalPtWeights);
  Teuchos::reduceAll<LO, ST>(*comm, Teuchos::REDUCE_SUM, nImagePts*numDims, imagePtGradComps.data(), globalPtGrads);
...
initialIterationSetup(double& gradScale, double& springScale, int dbMode)
{
  ...
  if(dbMode) {
     std::cout << "Saddle Point:  First iteration:  maxGradMag=" << maxGradMag
