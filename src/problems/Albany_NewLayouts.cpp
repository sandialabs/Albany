...
Albany::Layouts::Layouts (int worksetSize, int numVertices, int numNodes, int numQPts, int numCellDim, int vecDim, int numSides)
// numCellDim is the number of spatial dimensions
// vecDim is the length of a vector quantity
// -- For many problems, numCellDim is used for both since there are
// typically numCellDim displacements and velocities.
{
  ...
  node_gradient = rcp(new MDALayout<Cell,Node,Dim>(worksetSize,numSides,numNodes,numCellDim));
  qp_gradient   = rcp(new MDALayout<Cell,QuadPoint,Dim>(worksetSize,numSides,numQPts,numCellDim));
  cell_gradient = rcp(new MDALayout<Cell,Dim>(worksetSize,numSides,numCellDim));

  node_tensor = rcp(new MDALayout<Cell,Side,Node,Dim,Dim>(worksetSize,numSides,numNodes,numCellDim,numCellDim));
  qp_tensor   = rcp(new MDALayout<Cell,Side,QuadPoint,Dim,Dim>(worksetSize,numSides,numQPts,numCellDim,numCellDim));
  cell_tensor = rcp(new MDALayout<Cell,Side,Dim,Dim>(worksetSize,numSides,numCellDim,numCellDim));

  node_vecgradient = rcp(new MDALayout<Cell,Side,Node,Dim,Dim>(worksetSize,numSides,numNodes,vecDim,numCellDim));
  qp_vecgradient   = rcp(new MDALayout<Cell,Side,QuadPoint,Dim,Dim>(worksetSize,numSides,numQPts,vecDim,numCellDim));
  cell_vecgradient = rcp(new MDALayout<Cell,Side,Dim,Dim>(worksetSize,numSides,vecDim,numCellDim));

  qp_tensorgradient = rcp(new MDALayout<Cell,Side,QuadPoint,Dim,Dim,Dim>(worksetSize,numSides,numQPts,vecDim,vecDim,numCellDim));

  node_tensor3  = rcp(new MDALayout<Cell,Side,Node,Dim,Dim,Dim>(worksetSize,numSides,numNodes,numCellDim,numCellDim,numCellDim));
  qp_tensor3    = rcp(new MDALayout<Cell,Side,QuadPoint,Dim,Dim,Dim>(worksetSize,numSides,numQPts,numCellDim,numCellDim,numCellDim));
  cell_tensor3  = rcp(new MDALayout<Cell,Side,Dim,Dim,Dim>(worksetSize,numSides,numCellDim,numCellDim,numCellDim));

  node_tensor4  = rcp(new MDALayout<Cell,Side,Node,Dim,Dim,Dim,Dim>(worksetSize,numSides,numNodes,numCellDim,numCellDim,numCellDim,numCellDim));
  qp_tensor4    = rcp(new MDALayout<Cell,Side,QuadPoint,Dim,Dim,Dim,Dim>(worksetSize,numSides,numQPts,numCellDim,numCellDim,numCellDim,numCellDim));
  cell_tensor4  = rcp(new MDALayout<Cell,Side,Dim,Dim,Dim,Dim>(worksetSize,numSides,numCellDim,numCellDim,numCellDim,numCellDim));
  ...
  node_node_tensor = rcp(new MDALayout<Node,Side,Dim,Dim>(worksetSize,numSides,numCellDim,numCellDim));

  // Coordinates:  3vector is for shells 2D topology 3 coordinates
  vertices_vector = rcp(new MDALayout<Cell,Side,Vertex,Dim>(worksetSize,numSides,numVertices,numCellDim));
  ...
  node_qp_gradient = rcp(new MDALayout<Cell,Side,Node,QuadPoint,Dim>(worksetSize,numSides,numNodes,numQPts,numCellDim));
