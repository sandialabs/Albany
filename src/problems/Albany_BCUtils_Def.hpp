    ...
    for (std::size_t j = 0; j < bcNames.size(); j++)
    {
      for (std::size_t k = 0; k < conditions.size(); k++)
      {
        ...
        if (BCparams.isParameter(ss))
        {
          ...
          else if (conditions[k] == "basal_non_linear")
          {
            p->set<std::string> ("DOF Name", dof_names[0]);
            p->set<bool> ("Vector Field", isVectorField);
            if (isVectorField)
            ...
            p->set<double> ("Glen's Law n",params->sublist("FELIX Viscosity").get<double>("Glen's Law n"));
            p->set<std::string> ("FELIX Nonlinearity Type",params->sublist("Neumann BCs").get<std::string>("FELIX Nonlinearity Type"));
          }
                                             ...
                                             const std::vector<std::string>& conditions)
{
  ...
  validPL->set<std::string>("FELIX Nonlinearity Type","Power Law","Type of nonlinearity for FELIX basal_non_linear bc");
