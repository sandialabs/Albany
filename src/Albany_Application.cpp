...
void
Albany::Application::initialSetUp(const RCP<Teuchos::ParameterList>& params)
{
...
//#ifdef ATO_USES_COGENT
  ...
  if (scaleType == "Constant") {
    ...
    if (scaleBCdofs == true) {
      TEUCHOS_TEST_FOR_EXCEPTION(
          ...
          std::endl
              ...
              << "Scale BC dofs does not work with "
              << scaleType
              << "Type scaling, only Type = Constant Scaling."
              << std::endl);
    }
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        "The scaling Type you selected "
            << scaleType
            << " is not supported!"
            << "Supported scaling Types are currently: Constant"
            << std::endl);
  }
  ...
  if ((problemParams->get("Name", "Heat 1D") == "Poisson 1D") ||
    ...
    if (scaleBCdofs == true) {
      TEUCHOS_TEST_FOR_EXCEPTION(
          ...
          std::endl
              << "Error in Albany::Application: "
              << "Scale BC dofs does not work for QCAD Poisson or Schrodinger "
                 "problems. "
              << std::endl);
    }
  }
  ...
  countJac =
      0;  // initiate counter that counts instances of Jacobian matrix to 0
  countRes =
      0;  // initiate counter that counts instances of residual vector to 0

  // FIXME: call setScaleBCDofs only on first step rather than at every Newton
  // step.
  // It's called every step now b/c calling it once did not work for Schwarz
  // problems.
  countScale = 0;
...
void
Albany::Application::createMeshSpecs()
{
...
void
Albany::Application::buildProblem()
{
#if defined(ALBANY_LCM)
  ...
  if ((requires_sdbcs_ == true) && (problem->useSDBCs() == false) &&
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Error in Albany::Application: you are using a solver that requires "
        "SDBCs yet you are not using SDBCs!\n");
  }
  if ((requires_orig_dbcs_ == true) && (problem->useSDBCs() == true)) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Error in Albany::Application: you are using a solver with SDBCs that "
...
void
Albany::Application::createDiscretization()
{
    ...
    const Teuchos::RCP<const Tpetra_Vector>&    initial_guess)
{
  ...
  if (!stateMgr.areStateVarsAllocated()) stateMgr.setStateArrays(disc);
    ...
    for (int is = 0; is < distParamSIS.size(); is++) {
      ...
      Teuchos::RCP<TpetraDistributedParameter> parameter(
          new TpetraDistributedParameter(
              param_name, dist_paramT, node_mapT, overlap_node_mapT));
  ...
  if (!TpetraBuild) {
    ...
    for (int i = 0; i < responses.size(); i++) responses[i]->setupT();
  }
  ...
  *out << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n"
  ...
  // as a default value for the new flag location when the latter has been left
  // undefined
...
#ifdef ALBANY_MOR
#if defined(ALBANY_EPETRA)
  if (disc->supportsMOR()) morFacade = createMORFacade(disc, problemParams);
#endif
#endif
  // MPerego: Preforming post registration setup here to make sure that the
  // discretization is already created, so that
  // derivative dimensions are known. Cannot do post registration right before
  // the evaluate , as done for other field managers.
...
Albany::Application::getProblem() const
{
  return problem;
}
...
Albany::Application::getMapT() const
{
  return disc->getMapT();
}
...
Albany::Application::getJacobianGraphT() const
{
  return disc->getJacobianGraphT();
}

RCP<Tpetra_Operator>
Albany::Application::getPreconditionerT()
{
//#if defined(ATO_USES_COGENT)
#ifdef ALBANY_ATO
  if (precType == "XFEM") {
...
RCP<const Epetra_Vector>
Albany::Application::getInitialSolutionDotDot() const
{
  const Teuchos::RCP<const Tpetra_MultiVector> xMV =
      solMgrT->getInitialSolution();
...
RCP<ParamLib>
Albany::Application::getParamLib() const
{
  return paramLib;
}

RCP<DistParamLib>
Albany::Application::getDistParamLib() const
{
  return distParamLib;
}

int
Albany::Application::getNumResponses() const
{
...
Albany::Application::init_sg(
    const RCP<const Stokhos::OrthogPolyBasis<int, double>>& basis,
    ...
    const RCP<const EpetraExt::MultiComm>& multiComm)
{
  ...
  if (sg_overlapped_x == Teuchos::null) {
    sg_overlap_map = rcp(new Epetra_LocalMap(
        sg_basis->size(), 0, product_comm->TimeDomainComm()));
    sg_overlapped_x = rcp(new Stokhos::EpetraVectorOrthogPoly(
        sg_basis, sg_overlap_map, disc->getOverlapMap(), product_comm));
    sg_overlapped_xdot = rcp(new Stokhos::EpetraVectorOrthogPoly(
        sg_basis, sg_overlap_map, disc->getOverlapMap(), product_comm));
    sg_overlapped_xdotdot = rcp(new Stokhos::EpetraVectorOrthogPoly(
        sg_basis, sg_overlap_map, disc->getOverlapMap(), product_comm));
    sg_overlapped_f = rcp(new Stokhos::EpetraVectorOrthogPoly(
        sg_basis, sg_overlap_map, disc->getOverlapMap(), product_comm));
    ...
    const int                                   check_lvl)
{
  ...
  if (check_lvl > 1) mv = Teuchos::rcp(new Tpetra_MultiVector(x->getMap(), 5));
...
void
Albany::Application::computeGlobalResidualImplT(
    ...
    Epetra_Vector&                  f)
{
  ...
  if (writeToCoutRes != 0) {     // If requesting writing of residual to cout...
    if (writeToCoutRes == -1) {  // cout residual time it arises
      ...
      if (countRes ==
          writeToCoutRes) {  // cout residual only at requested count#
        std::cout << "Global Residual #" << countRes << ": " << std::endl;
        std::cout << f << std::endl;
      }
    }
    ...
    Epetra_Vector&                  f)
{
  ...
  if (problem->useSDBCs() == false) {
    ...
    this->computeGlobalResidualSDBCsImplT(
        current_time, xdotT, xdotdotT, xT, p, fT);
  }
...
#if !defined(ALBANY_LCM)
void
Albany::Application::computeGlobalResidualT(
    const double                    current_time,
...
#if defined(ALBANY_EPETRA)
double
Albany::Application::computeConditionNumber(Epetra_CrsMatrix& matrix)
{
...
void
Albany::Application::computeGlobalJacobianImplT(
    const double                             alpha,
    const double                             beta,
    const double                             omega,
    const double                             current_time,
    ...
    const Teuchos::RCP<Tpetra_CrsMatrix>&    jacT)
{
  ...
  if (!overlapped_jacT->isFillActive()) overlapped_jacT->resumeFill();
#endif
...
#if defined(ALBANY_LCM)
void
Albany::Application::computeGlobalJacobianSDBCsImplT(
    const double                             alpha,
    const double                             beta,
    const double                             omega,
    const double                             current_time,
    ...
    const Teuchos::RCP<Tpetra_CrsMatrix>&    jacT)
{
  ...
  if (scale != 1.0) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        true,
        std::logic_error,
        "Scaling cannot be used with computeGlobalResidualTempusSDBCsImplT "
        "routine!  "
    ...
    Epetra_CrsMatrix&               jac)
{
  ...
  this->computeGlobalJacobianImplT(
      alpha, beta, omega, current_time, xdotT, xdotdotT, xT, p, fT, jacT);
  ...
  if (writeToCoutJac !=
    ...
    if (writeToCoutJac == -1) {  // cout jacobian every time it arises
      ...
      *out << jac << "\n";
      ...
      if (countJac ==
          writeToCoutJac) {  // cout jacobian only at requested count#
        *out << "Global Jacobian #" << countJac << ":\n";
        *out << jac << "\n";
      }
    }
  }
  if (computeJacCondNum != 0) {  // If requesting computation of condition
                                 // number
...
//#if !defined(ALBANY_LCM)
void
Albany::Application::computeGlobalJacobianT(
    const double                    alpha,
    const double                    beta,
    const double                    omega,
    const double                    current_time,
    ...
    Tpetra_CrsMatrix&               jacT)
{
  ...
  if (computeJacCondNum != 0) {  // If requesting computation of condition
    ...
    Teuchos::RCP<Epetra_CrsMatrix> jac =
        Petra::TpetraCrsMatrix_To_EpetraCrsMatrix(
            Teuchos::rcpFromRef(jacT), comm);
    if (computeJacCondNum ==
        -1) {  // cout jacobian condition # every time it arises
      double condNum = computeConditionNumber(*jac);
      *out << "Jacobian #" << countJac << " condition number = " << condNum
...
Jacobian...
    char name[100];  //create string for file name
    if (writeToMatrixMarketJac == -1) { //write jacobian to MatrixMarket every
time it arises
      sprintf(name, "jac%i.mm", countJac);
      Tpetra_MatrixMarket_Writer::writeSparseFile(
          name,
...
#if defined(ALBANY_EPETRA)
      Teuchos::RCP<Epetra_CrsMatrix> jac =
Petra::TpetraCrsMatrix_To_EpetraCrsMatrix(Teuchos::rcpFromRef(jacT), comm);
      if (computeJacCondNum == -1) { //cout jacobian condition # every time it
arises
        double condNum = computeConditionNumber(*jac);
        *out << "Jacobian #" << countJac << " condition number = " << condNum <<
"\n";
...
void
Albany::Application::computeGlobalPreconditionerT(
    const RCP<Tpetra_CrsMatrix>& jac,
...
void
Albany::Application::computeGlobalTangentImplT(
    const double                                  alpha,
    const double                                  beta,
    const double                                  omega,
    const double                                  current_time,
    ...
    const Teuchos::RCP<Tpetra_MultiVector>&       fpT)
{
  ...
  if (Teuchos::nonnull(VxdotT)) {
    overlapped_VxdotT = rcp(new Tpetra_MultiVector(
        disc->getOverlapMapT(), VxdotT->getNumVectors()));
    ...
    Epetra_MultiVector*             fp)
{
  ...
  Teuchos::RCP<Tpetra_Vector> fT;
  if (f != NULL) fT = Petra::EpetraVector_To_TpetraVectorNonConst(*f, commT);
...
void
Albany::Application::computeGlobalTangentT(
    const double                    alpha,
    const double                    beta,
    const double                    omega,
    const double                    current_time,
...
void
Albany::Application::applyGlobalDistParamDerivImplT(
    const double                                  current_time,
    const Teuchos::RCP<const Tpetra_Vector>&      xdotT,
    const Teuchos::RCP<const Tpetra_Vector>&      xdotdotT,
    const Teuchos::RCP<const Tpetra_Vector>&      xT,
...
void
Albany::Application::evaluateResponseT(
    int                             response_index,
    const double                    current_time,
    const Tpetra_Vector*            xdotT,
    const Tpetra_Vector*            xdotdotT,
...
void
Albany::Application::evaluateResponseTangentT(
    int                             response_index,
    const double                    alpha,
    const double                    beta,
    const double                    omega,
...
#if defined(ALBANY_EPETRA)
void
Albany::Application::evaluateResponseDerivative(
    int                                          response_index,
    const double                                 current_time,
    const Epetra_Vector*                         xdot,
    const Epetra_Vector*                         xdotdot,
...
void
Albany::Application::evaluateResponseDerivativeT(
    int                                              response_index,
    const double                                     current_time,
    const Tpetra_Vector*                             xdotT,
    const Tpetra_Vector*                             xdotdotT,
...
void
Albany::Application::evaluateResponseDistParamDerivT(
    int                             response_index,
    const double                    current_time,
    ...
    Stokhos::EpetraVectorOrthogPoly&              sg_f)
{
  ...
  // Load connectivity map and coordinates
  const WorksetArray<Kokkos::View<LO***, PHX::Device>>::type& wsElNodeEqID =
  ...
  for (int i = 0; i < sg_x.size(); i++) {
    ...
    if (sg_xdotdot != NULL)
      (*sg_overlapped_xdotdot)[i].Import((*sg_xdotdot)[i], *importer, Insert);
  ...
  // put current_time (from Rythmos) if this is a transient problem, then
  // compute dt
  ...
  for (int i = 0; i < sg_p_index.size(); i++) {
    ...
    for (unsigned int j = 0; j < p[ii].size(); j++)
      p[ii][j].family->setValue<PHAL::AlbanyTraits::SGResidual>(
          sg_p_vals[ii][j]);
  }
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws = 0; ws < numWorksets; ws++) {
      ...
      // FillType template argument used to specialize Sacado
      fm[wsPhysIndex[ws]]->evaluateFields<PHAL::AlbanyTraits::SGResidual>(
          workset);
      if (nfm != Teuchos::null)
        deref_nfm(nfm, wsPhysIndex, ws)
            ->evaluateFields<PHAL::AlbanyTraits::SGResidual>(workset);
    }
  }
    ...
    Stokhos::VectorOrthogPoly<Epetra_CrsMatrix>&  sg_jac)
{
  ...
  if (sg_overlapped_jac == Teuchos::null ||
    ...
    RCP<Epetra_LocalMap> sg_overlap_jac_map =
        rcp(new Epetra_LocalMap(sg_jac.size(), 0, sg_overlap_map->Comm()));
    sg_overlapped_jac = rcp(new Stokhos::VectorOrthogPoly<Epetra_CrsMatrix>(
  ...
  for (int i = 0; i < sg_p_index.size(); i++) {
    int ii = sg_p_index[i];
    for (unsigned int j = 0; j < p[ii].size(); j++)
      p[ii][j].family->setValue<PHAL::AlbanyTraits::SGJacobian>(
          sg_p_vals[ii][j]);
  }

  RCP<Stokhos::EpetraVectorOrthogPoly> sg_overlapped_ff;
  if (sg_f != NULL) sg_overlapped_ff = sg_overlapped_f;
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws = 0; ws < numWorksets; ws++) {
      ...
      // FillType template argument used to specialize Sacado
      fm[wsPhysIndex[ws]]->evaluateFields<PHAL::AlbanyTraits::SGJacobian>(
          workset);
      if (nfm != Teuchos::null)
        deref_nfm(nfm, wsPhysIndex, ws)
            ->evaluateFields<PHAL::AlbanyTraits::SGJacobian>(workset);
    }
  }
...
void
Albany::Application::computeGlobalSGTangent(
    const double                                  alpha,
    const double                                  beta,
    const double                                  omega,
    const double                                  current_time,
    ...
    Stokhos::EpetraMultiVectorOrthogPoly*         sg_fVp)
{
  ...
  if (Vxdotdot != NULL) {
    overlapped_Vxdotdot = rcp(new Epetra_MultiVector(
        *(disc->getOverlapMap()), Vxdotdot->NumVectors()));
  ...
  for (int i = 0; i < sg_p_index.size(); i++) {
    ...
    for (unsigned int j = 0; j < par[ii].size(); j++)
      par[ii][j].family->setValue<PHAL::AlbanyTraits::SGTangent>(
          sg_p_vals[ii][j]);
  }
  ...
  if (sg_JVx != NULL) {
    sg_overlapped_JVx = Teuchos::rcp(new Stokhos::EpetraMultiVectorOrthogPoly(
        sg_basis,
        sg_overlap_map,
        disc->getOverlapMap(),
        sg_x.productComm(),
  ...
  if (sg_fVp != NULL) {
    sg_overlapped_fVp = Teuchos::rcp(new Stokhos::EpetraMultiVectorOrthogPoly(
        sg_basis,
        sg_overlap_map,
        disc->getOverlapMap(),
        sg_x.productComm(),
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws = 0; ws < numWorksets; ws++) {
      ...
      // FillType template argument used to specialize Sacado
      fm[wsPhysIndex[ws]]->evaluateFields<PHAL::AlbanyTraits::SGTangent>(
          workset);
      if (nfm != Teuchos::null)
        deref_nfm(nfm, wsPhysIndex, ws)
            ->evaluateFields<PHAL::AlbanyTraits::SGTangent>(workset);
    }
  }
...
void
Albany::Application::evaluateSGResponse(
    int                                           response_index,
...
void
Albany::Application::evaluateSGResponseTangent(
    int                                           response_index,
    const double                                  alpha,
    const double                                  beta,
    const double                                  omega,
...
void
Albany::Application::evaluateSGResponseDerivative(
    int                                            response_index,
    const double                                   current_time,
    const Stokhos::EpetraVectorOrthogPoly*         sg_xdot,
    const Stokhos::EpetraVectorOrthogPoly*         sg_xdotdot,
    ...
    Stokhos::ProductEpetraVector&                 mp_f)
{
  ...
  if (mp_overlapped_x == Teuchos::null ||
    ...
    mp_overlapped_x = rcp(new Stokhos::ProductEpetraVector(
        mp_x.map(), disc->getOverlapMap(), mp_x.productComm()));

    if (mp_xdot != NULL)
      mp_overlapped_xdot = rcp(new Stokhos::ProductEpetraVector(
          mp_xdot->map(), disc->getOverlapMap(), mp_x.productComm()));

    if (mp_xdotdot != NULL)
      mp_overlapped_xdotdot = rcp(new Stokhos::ProductEpetraVector(
          mp_xdotdot->map(), disc->getOverlapMap(), mp_x.productComm()));
  }

  if (mp_overlapped_f == Teuchos::null ||
      mp_overlapped_f->size() != mp_f.size()) {
    mp_overlapped_f = rcp(new Stokhos::ProductEpetraVector(
  ...
  for (int i = 0; i < mp_x.size(); i++) {
    ...
    if (mp_xdotdot != NULL)
      (*mp_overlapped_xdotdot)[i].Import((*mp_xdotdot)[i], *importer, Insert);
  ...
  for (int i = 0; i < mp_p_index.size(); i++) {
    int ii = mp_p_index[i];
    for (unsigned int j = 0; j < p[ii].size(); j++)
      p[ii][j].family->setValue<PHAL::AlbanyTraits::MPResidual>(
          mp_p_vals[ii][j]);
  }
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws = 0; ws < numWorksets; ws++) {
      ...
      // FillType template argument used to specialize Sacado
      fm[wsPhysIndex[ws]]->evaluateFields<PHAL::AlbanyTraits::MPResidual>(
          workset);
      if (nfm != Teuchos::null)
        deref_nfm(nfm, wsPhysIndex, ws)
            ->evaluateFields<PHAL::AlbanyTraits::MPResidual>(workset);
    }
  }
    ...
    Stokhos::ProductContainer<Epetra_CrsMatrix>&  mp_jac)
{
  ...
  if (mp_overlapped_jac == Teuchos::null ||
    ...
    mp_overlapped_jac = rcp(new Stokhos::ProductContainer<Epetra_CrsMatrix>(
  ...
  for (int i = 0; i < mp_p_index.size(); i++) {
    int ii = mp_p_index[i];
    for (unsigned int j = 0; j < p[ii].size(); j++)
      p[ii][j].family->setValue<PHAL::AlbanyTraits::MPJacobian>(
          mp_p_vals[ii][j]);
  }

  RCP<Stokhos::ProductEpetraVector> mp_overlapped_ff;
  if (mp_f != NULL) mp_overlapped_ff = mp_overlapped_f;
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws = 0; ws < numWorksets; ws++) {
      ...
      // FillType template argument used to specialize Sacado
      fm[wsPhysIndex[ws]]->evaluateFields<PHAL::AlbanyTraits::MPJacobian>(
          workset);
      if (nfm != Teuchos::null)
        deref_nfm(nfm, wsPhysIndex, ws)
            ->evaluateFields<PHAL::AlbanyTraits::MPJacobian>(workset);
    }
  }
...
void
Albany::Application::computeGlobalMPTangent(
    const double                                  alpha,
    const double                                  beta,
    const double                                  omega,
    const double                                  current_time,
    ...
    Stokhos::ProductEpetraMultiVector*            mp_fVp)
{
  ...
  for (int i = 0; i < mp_p_index.size(); i++) {
    int ii = mp_p_index[i];
    for (unsigned int j = 0; j < par[ii].size(); j++)
      par[ii][j].family->setValue<PHAL::AlbanyTraits::MPTangent>(
          mp_p_vals[ii][j]);
  }
  ...
  if (mp_JVx != NULL) {
    mp_overlapped_JVx = Teuchos::rcp(new Stokhos::ProductEpetraMultiVector(
        mp_JVx->map(),
        disc->getOverlapMap(),
        mp_x.productComm(),
        mp_JVx->numVectors()));
  ...
  if (mp_fVp != NULL) {
    mp_overlapped_fVp = Teuchos::rcp(new Stokhos::ProductEpetraMultiVector(
        mp_fVp->map(),
        disc->getOverlapMap(),
        mp_x.productComm(),
        mp_fVp->numVectors()));
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws = 0; ws < numWorksets; ws++) {
      ...
      // FillType template argument used to specialize Sacado
      fm[wsPhysIndex[ws]]->evaluateFields<PHAL::AlbanyTraits::MPTangent>(
          workset);
      if (nfm != Teuchos::null)
        deref_nfm(nfm, wsPhysIndex, ws)
            ->evaluateFields<PHAL::AlbanyTraits::MPTangent>(workset);
    }
  }
...
void
Albany::Application::evaluateMPResponse(
    int                                           response_index,
    const double                                  curr_time,
    const Stokhos::ProductEpetraVector*           mp_xdot,
    const Stokhos::ProductEpetraVector*           mp_xdotdot,
...
void
Albany::Application::evaluateMPResponseTangent(
    int                                           response_index,
    const double                                  alpha,
    const double                                  beta,
    const double                                  omega,
...
void
Albany::Application::evaluateMPResponseDerivative(
    int                                            response_index,
    const double                                   current_time,
    const Stokhos::ProductEpetraVector*            mp_xdot,
    const Stokhos::ProductEpetraVector*            mp_xdotdot,
...
#if defined(ALBANY_EPETRA)
void
Albany::Application::evaluateStateFieldManager(
    const double         current_time,
    const Epetra_Vector* xdot,
    const Epetra_Vector* xdotdot,
    const Epetra_Vector& x)
{
  ...
  this->evaluateStateFieldManagerT(
      current_time, xdotT.ptr(), xdotdotT.ptr(), *xT);
}
#endif

void
Albany::Application::evaluateStateFieldManagerT(
    const double              current_time,
    const Tpetra_MultiVector& xT)
{
  ...
  if (num_vecs == 1)
    this->evaluateStateFieldManagerT(
        current_time, Teuchos::null, Teuchos::null, *xT.getVector(0));
  else if (num_vecs == 2)
    this->evaluateStateFieldManagerT(
        current_time, xT.getVector(1).ptr(), Teuchos::null, *xT.getVector(0));
  else
    this->evaluateStateFieldManagerT(
        current_time,
        xT.getVector(1).ptr(),
...
void
Albany::Application::registerShapeParameters()
{
  ...
  Albany::DummyParameterAccessor<PHAL::AlbanyTraits::Jacobian, SPL_Traits>* dJ =
      new Albany::
          DummyParameterAccessor<PHAL::AlbanyTraits::Jacobian, SPL_Traits>();
  Albany::DummyParameterAccessor<PHAL::AlbanyTraits::Tangent, SPL_Traits>* dT =
      new Albany::
          DummyParameterAccessor<PHAL::AlbanyTraits::Tangent, SPL_Traits>();
#ifdef ALBANY_SG
  Albany::DummyParameterAccessor<PHAL::AlbanyTraits::SGResidual, SPL_Traits>*
      dSGR = new Albany::
          DummyParameterAccessor<PHAL::AlbanyTraits::SGResidual, SPL_Traits>();
  Albany::DummyParameterAccessor<PHAL::AlbanyTraits::SGJacobian, SPL_Traits>*
      dSGJ = new Albany::
          DummyParameterAccessor<PHAL::AlbanyTraits::SGJacobian, SPL_Traits>();
#endif
#ifdef ALBANY_ENSEMBLE
  Albany::DummyParameterAccessor<PHAL::AlbanyTraits::MPResidual, SPL_Traits>*
      dMPR = new Albany::
          DummyParameterAccessor<PHAL::AlbanyTraits::MPResidual, SPL_Traits>();
  Albany::DummyParameterAccessor<PHAL::AlbanyTraits::MPJacobian, SPL_Traits>*
      dMPJ = new Albany::
          DummyParameterAccessor<PHAL::AlbanyTraits::MPJacobian, SPL_Traits>();
#endif
...
#ifdef ALBANY_SG
    new Sacado::
        ParameterRegistration<PHAL::AlbanyTraits::SGResidual, SPL_Traits>(
            shapeParamNames[i], dSGR, paramLib);
    new Sacado::
        ParameterRegistration<PHAL::AlbanyTraits::SGJacobian, SPL_Traits>(
            shapeParamNames[i], dSGJ, paramLib);
#endif
#ifdef ALBANY_ENSEMBLE
    new Sacado::
        ParameterRegistration<PHAL::AlbanyTraits::MPResidual, SPL_Traits>(
            shapeParamNames[i], dMPR, paramLib);
    new Sacado::
        ParameterRegistration<PHAL::AlbanyTraits::MPJacobian, SPL_Traits>(
...
void
Albany::Application::postRegSetup(std::string eval)
{
  ...
  if (eval == "Residual") {
    ...
    for (int ps = 0; ps < fm.size(); ps++)
      fm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::SGResidual>(
          eval);
    if (dfm != Teuchos::null)
      dfm->postRegistrationSetupForType<PHAL::AlbanyTraits::SGResidual>(eval);
    if (nfm != Teuchos::null)
      for (int ps = 0; ps < nfm.size(); ps++)
        nfm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::SGResidual>(
            eval);
    ...
    for (int ps = 0; ps < fm.size(); ps++) {
      ...
      derivative_dimensions.push_back(
          PHAL::getDerivativeDimensions<PHAL::AlbanyTraits::Jacobian>(
              this, ps));
      fm[ps]
          ->setKokkosExtendedDataTypeDimensions<PHAL::AlbanyTraits::SGJacobian>(
              derivative_dimensions);
      fm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::SGJacobian>(
          eval);
      if (nfm != Teuchos::null && ps < nfm.size()) {
        nfm[ps]
            ->setKokkosExtendedDataTypeDimensions<
                PHAL::AlbanyTraits::SGJacobian>(derivative_dimensions);
        nfm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::SGJacobian>(
            eval);
      }
    }
    if (dfm != Teuchos::null) {
      ...
      dfm->setKokkosExtendedDataTypeDimensions<PHAL::AlbanyTraits::SGJacobian>(
          derivative_dimensions);
    ...
    for (int ps = 0; ps < fm.size(); ps++) {
      ...
      derivative_dimensions.push_back(
          PHAL::getDerivativeDimensions<PHAL::AlbanyTraits::Tangent>(this, ps));
      fm[ps]
          ->setKokkosExtendedDataTypeDimensions<PHAL::AlbanyTraits::SGTangent>(
              derivative_dimensions);
      fm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::SGTangent>(eval);
      if (nfm != Teuchos::null && ps < nfm.size()) {
        nfm[ps]
            ->setKokkosExtendedDataTypeDimensions<
                PHAL::AlbanyTraits::SGTangent>(derivative_dimensions);
        nfm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::SGTangent>(
            eval);
      }
    }
    if (dfm != Teuchos::null) {
      ...
      derivative_dimensions.push_back(
          PHAL::getDerivativeDimensions<PHAL::AlbanyTraits::Tangent>(this, 0));
      dfm->setKokkosExtendedDataTypeDimensions<PHAL::AlbanyTraits::SGTangent>(
          derivative_dimensions);
    ...
    for (int ps = 0; ps < fm.size(); ps++)
      fm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::MPResidual>(
          eval);
    if (dfm != Teuchos::null)
      dfm->postRegistrationSetupForType<PHAL::AlbanyTraits::MPResidual>(eval);
    if (nfm != Teuchos::null)
      for (int ps = 0; ps < nfm.size(); ps++)
        nfm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::MPResidual>(
            eval);
    ...
    for (int ps = 0; ps < fm.size(); ps++) {
      ...
      derivative_dimensions.push_back(
          PHAL::getDerivativeDimensions<PHAL::AlbanyTraits::Jacobian>(
              this, ps));
      fm[ps]
          ->setKokkosExtendedDataTypeDimensions<PHAL::AlbanyTraits::MPJacobian>(
              derivative_dimensions);
      fm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::MPJacobian>(
          eval);
      if (nfm != Teuchos::null && ps < nfm.size()) {
        nfm[ps]
            ->setKokkosExtendedDataTypeDimensions<
                PHAL::AlbanyTraits::MPJacobian>(derivative_dimensions);
        nfm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::MPJacobian>(
            eval);
      }
    }
    if (dfm != Teuchos::null) {
      ...
      derivative_dimensions.push_back(
          PHAL::getDerivativeDimensions<PHAL::AlbanyTraits::Jacobian>(this, 0));
      dfm->setKokkosExtendedDataTypeDimensions<PHAL::AlbanyTraits::MPJacobian>(
          derivative_dimensions);
    ...
    for (int ps = 0; ps < fm.size(); ps++) {
      ...
      derivative_dimensions.push_back(
          PHAL::getDerivativeDimensions<PHAL::AlbanyTraits::Tangent>(this, ps));
      fm[ps]
          ->setKokkosExtendedDataTypeDimensions<PHAL::AlbanyTraits::MPTangent>(
              derivative_dimensions);
      fm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::MPTangent>(eval);
      if (nfm != Teuchos::null && ps < nfm.size()) {
        nfm[ps]
            ->setKokkosExtendedDataTypeDimensions<
                PHAL::AlbanyTraits::MPTangent>(derivative_dimensions);
        nfm[ps]->postRegistrationSetupForType<PHAL::AlbanyTraits::MPTangent>(
            eval);
      }
    }
    if (dfm != Teuchos::null) {
      ...
      derivative_dimensions.push_back(
          PHAL::getDerivativeDimensions<PHAL::AlbanyTraits::Tangent>(this, 0));
      dfm->setKokkosExtendedDataTypeDimensions<PHAL::AlbanyTraits::MPTangent>(
          derivative_dimensions);
...
RCP<Epetra_Operator>
Albany::Application::buildWrappedOperator(
    const RCP<Epetra_Operator>& Jac,
...
#if defined(ALBANY_EPETRA)
void
Albany::Application::loadBasicWorksetInfo(
    PHAL::Workset& workset,
    double         current_time)
{
  workset.numEqs       = neq;
...
void
Albany::Application::loadBasicWorksetInfoT(
    PHAL::Workset& workset,
    double         current_time)
{
  workset.numEqs = neq;
...
void
Albany::Application::loadBasicWorksetInfoSDBCsT(
    PHAL::Workset&                    workset,
    ...
    double                            current_time)
{
  ...
  overlapped_sol->doImport(
      *owned_sol, *(solMgrT->get_importerT()), Tpetra::INSERT);
...
void
Albany::Application::loadWorksetJacobianInfo(
    PHAL::Workset& workset,
    const double&  alpha,
    const double&  beta,
    const double&  omega)
...
void
Albany::Application::loadWorksetNodesetInfo(PHAL::Workset& workset)
{
...
void
Albany::Application::setScale(Teuchos::RCP<Tpetra_CrsMatrix> jacT)
{
...
void
Albany::Application::setScaleBCDofs(PHAL::Workset& workset)
{
  scaleVec_->putScalar(1.0);
  int l = 0;
  for (auto iterator = workset.nodeSets->begin();
       iterator != workset.nodeSets->end();
       iterator++) {
...
void
Albany::Application::loadWorksetSidesetInfo(
    PHAL::Workset& workset,
    const int      ws)
{
...
#if defined(ALBANY_EPETRA)
void
Albany::Application::setupBasicWorksetInfo(
    PHAL::Workset&                  workset,
    double                          current_time,
    const Epetra_Vector*            xdot,
    const Epetra_Vector*            xdotdot,
...
void
Albany::Application::setupBasicWorksetInfoT(
    PHAL::Workset&                    workset,
    double                            current_time,
    Teuchos::RCP<const Tpetra_Vector> xdotT,
    Teuchos::RCP<const Tpetra_Vector> xdotdotT,
    ...
    const Teuchos::Array<ParamVec>&   p)
{
  /*
   ...
   solMgrT->get_overlapped_xdotT();
   Teuchos::RCP<Tpetra_Vector> overlapped_xdotdotT =
   solMgrT->get_overlapped_xdotdotT();
   */
    ...
    const Teuchos::Array<Teuchos::Array<SGType>>& sg_p_vals)
{
  ...
  for (int i = 0; i < sg_p_index.size(); i++) {
    ...
    for (unsigned int j = 0; j < p[ii].size(); j++) {
      p[ii][j].family->setValue<PHAL::AlbanyTraits::SGResidual>(
          sg_p_vals[ii][j]);
      p[ii][j].family->setValue<PHAL::AlbanyTraits::SGTangent>(
          sg_p_vals[ii][j]);
      p[ii][j].family->setValue<PHAL::AlbanyTraits::SGJacobian>(
          sg_p_vals[ii][j]);
    }
    ...
    const Teuchos::Array<Teuchos::Array<MPType>>& mp_p_vals)
{
  ...
  for (int i = 0; i < mp_p_index.size(); i++) {
    ...
    for (unsigned int j = 0; j < p[ii].size(); j++) {
      p[ii][j].family->setValue<PHAL::AlbanyTraits::MPResidual>(
          mp_p_vals[ii][j]);
      p[ii][j].family->setValue<PHAL::AlbanyTraits::MPTangent>(
          mp_p_vals[ii][j]);
      p[ii][j].family->setValue<PHAL::AlbanyTraits::MPJacobian>(
          mp_p_vals[ii][j]);
    }
...
#if defined(ALBANY_EPETRA)
void
Albany::Application::setupTangentWorksetInfo(
    PHAL::Workset&                  workset,
    double                          current_time,
    bool                            sum_derivs,
    const Epetra_Vector*            xdot,
...
void
Albany::Application::setupTangentWorksetInfoT(
    PHAL::Workset&                         workset,
    double                                 current_time,
    bool                                   sum_derivs,
    Teuchos::RCP<const Tpetra_Vector>      xdotT,
...
#ifdef ALBANY_MOR
#if defined(ALBANY_EPETRA)
Teuchos::RCP<Albany::MORFacade>
Albany::Application::getMorFacade()
{
  return morFacade;
}
#endif
...
void
Albany::Application::removeEpetraRelatedPLs(
    const Teuchos::RCP<Teuchos::ParameterList>& params)
{
...
#if defined(ALBANY_LCM)
void
Albany::Application::setCoupledAppBlockNodeset(
...
void
Albany::Application::computeGlobalResidualSDBCsImplT(
    double const                             current_time,
    Teuchos::RCP<Tpetra_Vector const> const& xdotT,
    Teuchos::RCP<Tpetra_Vector const> const& xdotdotT,
    Teuchos::RCP<Tpetra_Vector const> const& xT,
