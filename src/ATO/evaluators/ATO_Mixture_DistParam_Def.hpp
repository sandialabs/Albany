...
Mixture_DistParam<EvalT, Traits>::
Mixture_DistParam(const Teuchos::ParameterList& p,
                  const Teuchos::RCP<Albany::Layouts>& dl) :
BF(p.get<std::string> ("BF Name"), dl->node_qp_scalar)
{

  topologies = p.get< Teuchos::RCP<TopologyArray> >("Topologies");
  ...
  for(int iTopo=0; iTopo<nTopos; iTopo++){
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      << "Error!  Mixture_DistParam requires 'Distributed Parameter' based topology" << std::endl);

    topos[iTopo] = PHX::MDField<const ParamScalarT,Cell,Node>((*topologies)[topologyIndices[iTopo]]->getName(),dl->node_scalar);
...
//**********************************************************************
template<typename EvalT, typename Traits>
void Mixture_DistParam<EvalT, Traits>::
