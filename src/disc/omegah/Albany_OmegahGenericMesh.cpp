...
loadOmegahMesh ()
{
  ...
  for (const auto& pn : parts_names) {
    if (m_params->isSublist(pn)) {
      // Old approach: explicit Topo and Id parameters in a sublist
      ...
      const bool markDownward = pl.get<int>("Mark Downward",true);
      ...
      // New approach: use class_sets stored in the mesh file (created by exo2osh)
      TEUCHOS_TEST_FOR_EXCEPTION(m_mesh->class_sets.count(pn)==0, std::runtime_error,
          ...
          "and no 'Id'/'Topo' sublist was provided.\n");
      ...
      this->declare_part(pn, topo, is_in_part, true /*markDownward*/);
