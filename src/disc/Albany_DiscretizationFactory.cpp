        ...
        Teuchos::RCP<const Teuchos_Comm> comm, const int numParams)
{
  ...
  if (method == "STK1D") {
      ...
      meshStruct2D->setFieldAndBulkData(comm,
              sis, meshStruct2D->meshSpecs->worksetSize);
...
void
DiscretizationFactory::
setMeshStructFieldData(const Teuchos::RCP<StateInfoStruct>& sis)
{
...
void
DiscretizationFactory::
setMeshStructBulkData(const Teuchos::RCP<StateInfoStruct>& sis)
{
