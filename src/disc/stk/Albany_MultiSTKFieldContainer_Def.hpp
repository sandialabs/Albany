...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::initializeSTKAdaptation()
{
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::fillSolnVector(
    Epetra_Vector&                        soln,
    ...
    const Teuchos::RCP<const Epetra_Map>& node_map)
{
  ...
  stk::mesh::BucketVector const& all_elements =
      this->bulkData->get_buckets(stk::topology::NODE_RANK, sel);
  this->numNodes = node_map->NumMyElements();  // Needed for the getDOF function
                                               // to work correctly
    ...
    for (int k = 0; k < sol_index[0].size(); k++) {
      if (sol_index[0][k] == 1) {  // Scalar
        ...
        SFT* field = this->metaData->template get_field<SFT>(
            stk::topology::NODE_RANK, sol_vector_name[0][k]);
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::fillVector(
    ...
    const NodalDOFManager&                nodalDofManager)
{
  ...
  if (nodalDofManager.numComponents() > 1) {
    AbstractSTKFieldContainer::VectorFieldType* field =
        this->metaData
            ->template get_field<AbstractSTKFieldContainer::VectorFieldType>(
                stk::topology::NODE_RANK, field_name);
    for (stk::mesh::BucketVector::const_iterator it = all_elements.begin();
         it != all_elements.end();
         ++it) {
      const stk::mesh::Bucket& bucket = **it;
      this->fillVectorHelper(
          field_vector, field, field_node_map, bucket, nodalDofManager);
    }
  } else {
    AbstractSTKFieldContainer::ScalarFieldType* field =
        this->metaData
            ->template get_field<AbstractSTKFieldContainer::ScalarFieldType>(
                stk::topology::NODE_RANK, field_name);
    for (stk::mesh::BucketVector::const_iterator it = all_elements.begin();
         it != all_elements.end();
         ++it) {
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::fillVectorT(
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::fillSolnVectorT(
    Tpetra_Vector&                        solnT,
    stk::mesh::Selector&                  sel,
    const Teuchos::RCP<const Tpetra_Map>& node_mapT)
{
  ...
  stk::mesh::BucketVector const& all_elements =
      this->bulkData->get_buckets(stk::topology::NODE_RANK, sel);
  this->numNodes =
      node_mapT->getNodeNumElements();  // Needed for the getDOF function to
                                        // work correctly This is either
                                        // numOwnedNodes or numOverlapNodes,
                                        // depending on which map is passed in
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::fillSolnMultiVector(
    ...
    const Teuchos::RCP<const Tpetra_Map>& node_mapT)
{
  ...
  for (stk::mesh::BucketVector::const_iterator it = all_elements.begin();
    ...
    for (int vector_component = 0; vector_component < solnT.getNumVectors();
         vector_component++) {
      int offset = 0;
      ...
      for (int k = 0; k < sol_index[vector_component].size(); k++) {
        if (sol_index[vector_component][k] == 1) {  // Scalar
          ...
          this->fillMultiVectorHelper(
              solnT, field, node_mapT, bucket, vector_component, offset);
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::saveSolnVector(
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::saveVector(
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::saveSolnVectorT(
  ...
  for (stk::mesh::BucketVector::const_iterator it = all_elements.begin();
    ...
    for (int k = 0; k < sol_index[0].size(); k++) {
      if (sol_index[0][k] == 1) {  // Scalar
        SFT* field = this->metaData->template get_field<SFT>(
            stk::topology::NODE_RANK, sol_vector_name[0][k]);
        SFT* field_dot = this->metaData->template get_field<SFT>(
            stk::topology::NODE_RANK, sol_vector_name[1][k]);
        this->saveVectorHelperT(solnT, field, node_mapT, bucket, offset);
        this->saveVectorHelperT(
            soln_dotT, field_dot, node_mapT, bucket, offset);
      } else {
        VFT* field = this->metaData->template get_field<VFT>(
            stk::topology::NODE_RANK, sol_vector_name[0][k]);
        VFT* field_dot = this->metaData->template get_field<VFT>(
            stk::topology::NODE_RANK, sol_vector_name[1][k]);
        this->saveVectorHelperT(solnT, field, node_mapT, bucket, offset);
        this->saveVectorHelperT(
            soln_dotT, field_dot, node_mapT, bucket, offset);
  ...
  for (stk::mesh::BucketVector::const_iterator it = all_elements.begin();
    ...
    for (int k = 0; k < sol_index[0].size(); k++) {
      if (sol_index[0][k] == 1) {  // Scalar
        ...
        SFT* field_dot = this->metaData->template get_field<SFT>(
            stk::topology::NODE_RANK, sol_vector_name[1][k]);
        SFT* field_dotdot = this->metaData->template get_field<SFT>(
            stk::topology::NODE_RANK, sol_vector_name[2][k]);
        this->saveVectorHelperT(solnT, field, node_mapT, bucket, offset);
        this->saveVectorHelperT(
            soln_dotT, field_dot, node_mapT, bucket, offset);
        this->saveVectorHelperT(
            soln_dotdotT, field_dotdot, node_mapT, bucket, offset);
      } else {
        VFT* field = this->metaData->template get_field<VFT>(
            stk::topology::NODE_RANK, sol_vector_name[0][k]);
        VFT* field_dot = this->metaData->template get_field<VFT>(
            stk::topology::NODE_RANK, sol_vector_name[1][k]);
        VFT* field_dotdot = this->metaData->template get_field<VFT>(
            stk::topology::NODE_RANK, sol_vector_name[2][k]);
        this->saveVectorHelperT(solnT, field, node_mapT, bucket, offset);
        this->saveVectorHelperT(
            soln_dotT, field_dot, node_mapT, bucket, offset);
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::saveVectorT(
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::saveSolnMultiVector(
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::saveResVector(
  ...
  for (stk::mesh::BucketVector::const_iterator it = all_elements.begin();
    ...
    for (int k = 0; k < res_index.size(); k++) {
      if (res_index[k] == 1) {  // Scalar
        ...
        SFT* field = this->metaData->template get_field<SFT>(
            stk::topology::NODE_RANK, res_vector_name[k]);
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::saveResVectorT(
    ...
    const Teuchos::RCP<const Tpetra_Map>& node_map)
{
  ...
  // Iterate over the nodes by getting node buckets and iterating over each
  // bucket.
  stk::mesh::BucketVector const& all_elements =
      this->bulkData->get_buckets(stk::topology::NODE_RANK, sel);
  this->numNodes =
      node_map->getNodeNumElements();  // Needed for the getDOF function to work
                                       // correctly
  // This is either numOwnedNodes or numOverlapNodes, depending on
  // which map is passed in
...
template <bool Interleaved>
void
Albany::MultiSTKFieldContainer<Interleaved>::transferSolutionToCoords()
{
