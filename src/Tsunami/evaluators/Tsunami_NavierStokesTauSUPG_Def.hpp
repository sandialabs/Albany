...
NavierStokesTauSUPG<EvalT, Traits>::
NavierStokesTauSUPG(const Teuchos::ParameterList& p,
  ...
  TauSUPG    (p.get<std::string> ("Tau SUPG Name"), dl->qp_scalar)
{
  delta = p.get<double>("SUPG Constant"); 

  this->addDependentField(V);
  this->addDependentField(Gc);
  this->addDependentField(jacobian_det);

  this->addEvaluatedField(TauSUPG);
  ...
  this->setName("NavierStokesTauSUPG"+PHX::typeAsString<EvalT>());
}

//**********************************************************************
template<typename EvalT, typename Traits>
void NavierStokesTauSUPG<EvalT, Traits>::
...
evaluateFields(typename Traits::EvalData workset)
{
  ...
  for (std::size_t cell=0; cell < workset.numCells; ++cell) {
    for (std::size_t qp=0; qp < numQPs; ++qp) {
        ...
        TauSUPG(cell, qp) = delta*meshSize*meshSize;
    }
  }
}
