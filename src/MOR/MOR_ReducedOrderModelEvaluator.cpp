...
//   preconditioner type in the input file, you'll need to explicitly set invJacPrec
//   to true below AND in MOR_GaussNewtonOperatorFactory.cpp.
							...
							for (int j = 0; j < outArgs.Ng(); ++j) {
								if (!outArgs.supports(OUT_ARG_DgDx_dot, j).none()) {
									...
									for (int l = 0; l < outArgs.Np(); ++l) {
										...
										Teuchos::RCP<Albany::Application> app = dynamic_cast<Albany::ModelEvaluator &>(*fullOrderModel_).get_app();

										double this_time = 0;
										if (!app->getParamLib()->isParameter("Time"))
											this_time = inArgs.get_t();
										else
											this_time = app->getParamLib()->getRealValue<PHAL::AlbanyTraits::Residual>("Time");
										bool SDBC = app->getProblem()->useSDBCs();
										...
										if (requestedResidual) {
											...
											RCP<Epetra_Vector> red_f = solutionSpace_->linearReduction(*fullOutArgs.get_f()); // make a copy of the reduced full residual (i.e. Phi'*f) that will be used to replace the DBC rows of f_r later
											...
											for (size_t row = 0; row < reducedOpFactory_->num_dbc_modes(); ++row) {
												outArgs.get_f().get()[0][row] = red_f.get()[0][row];
											}
										...
										if (requestedJacobian) {
											...
											Teuchos::RCP<Tpetra_CrsMatrix> W_rT = Petra::EpetraCrsMatrix_To_TpetraCrsMatrix(*W_r, app->getComm());
											...
											size_t const
											num_local_rows = W_rT->getNodeNumRows();
											...
											for (auto local_row = min_local_row; local_row <= max_local_row; ++local_row)
											{
												...
												auto row_is_dbc = (local_row < reducedOpFactory_->num_dbc_modes());
												...
												for (size_t row_entry = 0; row_entry < num_row_entries; ++row_entry)
												{
													...
													auto col_is_dbc = (local_col<reducedOpFactory_->num_dbc_modes());
													if (row_is_dbc || (col_is_dbc && SDBC))
													{
											...
											W_rT->fillComplete();
											Petra::TpetraCrsMatrix_To_EpetraCrsMatrix(W_rT, *W_r, app->getEpetraComm());
										}
