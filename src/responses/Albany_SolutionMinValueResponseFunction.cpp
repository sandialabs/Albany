		 ...
		 Tpetra_Vector& gT)
{
  ...
  computeMinValueT(xT, gT_nonconstView[0], index);
}
		 ...
		 Tpetra_MultiVector* dg_dpT)
{
  int global_index;
  double mnv;
  computeMinValueT(xT, mnv, global_index);
  
  // Evaluate response g
  if (gT != NULL) {
    Teuchos::ArrayRCP<ST> gT_nonconstView = gT->get1dViewNonConst();
    gT_nonconstView[0] = mnv;
  }
  ...
  if (dg_dxT != NULL) {
    ...
    for (int i=0; i<xT.getMap()->getNodeNumElements(); i++) {
       ...
       if (xT_constView[i] == mnv) { dg_dxT_nonconstView[i] = 1.0; im = i; }
...
void
Albany::SolutionMinValueResponseFunction::
computeMinValueT(const Tpetra_Vector& xT, double& global_min, int& global_index)
{
  //The following is needed b/c Epetra_MaxDouble comes from Trilinos Epetra package.
  double Tpetra_MinDouble = -1.0E+100;
  double my_min = -Tpetra_MinDouble;
  int my_index = -1, index;
  ...
  for (int node=0; node<num_my_nodes; node++) {
    ...
    if (xT_constView[index] < my_min) {
      my_min = xT_constView[index];
      my_index = index;
    }
  }
  ...
  // Get max value across all proc's
  Teuchos::reduceAll(*commT, Teuchos::REDUCE_MIN, my_min, Teuchos::ptr(&global_min));

  // Compute min of all global indices equal to min value
