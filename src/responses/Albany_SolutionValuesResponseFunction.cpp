...
evaluateResponseT(const double current_time,
		 ...
		 Tpetra_Vector& gT)
{
//IK, 4/12/13 -- neet to convert to Tpetra!
/*
  const Epetra_BlockMap& map = x.Map();
  int N = map.NumGlobalElements();
  int max = map.MaxAllGID();
...
{
  this->evaluateResponseT(current_time, xdotT, xT, p, *gT);
  cout << "SolutionValuesResponseFunction::evaluateTangentT NOT IMPLEMETED" << endl;
}
...
{
  //this->evaluateResponseT(current_time, xdotT, xT, p, *gT);
  cout << "SolutionValuesResponseFunction::evaluateGradientT NOT IMPLEMETED" << endl;
}
