...
createCombinedTpetraVector(
  ...
  for (int m = 0; m < n_vecs; ++m) {
    ...
    for (int l = 0; l < local_num_elements_n; ++l) {
      my_global_elements[counter_local + l] =
          counter_global + disc_global_elements[l];
    }
  ...
  Teuchos::RCP<const Tpetra_Map> combined_map = Teuchos::rcp(
      new Tpetra_Map(
          global_num_elements, my_global_elements_AV, 0,
          vecs[0]->getMap()->getComm()));

  Teuchos::RCP<Tpetra_Vector> combined_vec =
      Teuchos::rcp(new Tpetra_Vector(combined_map));

  counter_local = 0;
  for (int m = 0; m < n_vecs; ++m) {
    int disc_local_elements_m = vecs[m]->getMap()->getNodeNumElements();
    Teuchos::RCP<Tpetra_Vector> temp =
        combined_vec->offsetViewNonConst(vecs[m]->getMap(), counter_local);
...
// IKT: Similar to tpetraFromThyra but for thyra product vectors (currently only
// used in Coupled Schwarz problems).
void
tpetraFromThyraProdVec(
    const Teuchos::Array<Teuchos::RCP<const Thyra::VectorBase<ST>>>
        &thyraResponses,
    const Teuchos::Array<Teuchos::Array<
        Teuchos::RCP<const Thyra::MultiVectorBase<ST>>>> &thyraSensitivities,
  ...
  responses.clear();
  // FIXME: right now only setting # of responses to 1 (solution vector) as
  // printing of other
  ...
  for (ThyraResponseArray::const_iterator it_begin = thyraResponses.begin(),
    ...
    if (it == it_end - 1) {
      ...
      if (r_prod != Teuchos::null) {
        // FIXME: productSpace()->numBlocks() when we have responses and >1
        // model does not work for some reason.
        // Need to figure out why!
        const int nProdVecs = r_prod->productSpace()->numBlocks();
        // create Teuchos array of spaces / vectors, to be populated from the
        // product vector
  ...
  if (thyraSensitivities.size() > 0)
    *out << "WARNING: For Thyra::ProductVectorBase, sensitivities are not yet "
            "supported! \n";
      ...
      Teuchos::RCP<const Thyra::ProductVectorBase<ST>> s_prod =
          Teuchos::nonnull(*jt)
              ? Teuchos::rcp_dynamic_cast<const Thyra::ProductVectorBase<ST>>(
                    *jt, true)
              : Teuchos::null;
      // FIXME: ultimately we'll need to change this to set the sensitivity
      // vector
    ...
    if (num_p > 0) {
      p_prod =
          Teuchos::nonnull(nominal.get_p(0))
              ...
              : Teuchos::null;
      // If p_prod is not product vector, print out parameter vector by
      // converting thyra vector to tpetra vector
      if (p_prod == Teuchos::null) {  // Thyra vector case (default -- for
