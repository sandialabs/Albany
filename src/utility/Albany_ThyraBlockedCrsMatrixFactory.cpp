...
struct ThyraBlockedCrsMatrixFactory::Impl {
...
Teuchos::RCP<Thyra_BlockedLinearOp> ThyraBlockedCrsMatrixFactory::createOp () const {
  ...
  Teuchos::RCP<Thyra_BlockedLinearOp> op;
  auto bt = Albany::build_type();
  if (bt==BuildType::Epetra) {
#ifdef ALBANY_EPETRA
    ...
    op = createThyraBlockedLinearOp(mat);
#else
    TEUCHOS_TEST_FOR_EXCEPTION (true, std::logic_error, "Error! Epetra is not enabled in albany.\n");
#endif
  } else {
    ...
    mat->fillComplete();
    op = createThyraBlockedLinearOp(Teuchos::rcp_implicit_cast<Tpetra_Operator>(mat));
  }
