      ...
      TEUCHOS_TEST_FOR_EXCEPTION(
          ...
          std::endl
              << "Error!  In Albany::ModelEvaluatorT constructor:  Provided "
                 ...
                 "Parameters list: \""
              << *p_name_ptr << "\" and \"" << name_from_list << "\""
              << std::endl);
    }

    TEUCHOS_TEST_FOR_EXCEPTION(
        !distParamLib->has(*p_name_ptr), Teuchos::Exceptions::InvalidParameter,
        std::endl
            << "Error!  In Albany::ModelEvaluatorT constructor:  "
...
Albany::ModelEvaluatorT::allocateVectors() {
  ...
  const Teuchos::RCP<const Thyra::VectorSpaceBase<ST>> xT_space =
      Thyra::createVectorSpace<ST>(map);
...
Albany::ModelEvaluatorT::get_x_space() const {
  ...
  Teuchos::RCP<const Thyra::VectorSpaceBase<ST>> x_space =
      Thyra::createVectorSpace<ST>(map);
  return x_space;
}
...
Albany::ModelEvaluatorT::get_f_space() const {
  Teuchos::RCP<const Tpetra_Map> map = app->getMapT();
  Teuchos::RCP<const Thyra::VectorSpaceBase<ST>> f_space =
      Thyra::createVectorSpace<ST>(map);
  return f_space;
}
...
Albany::ModelEvaluatorT::create_DfDp_op_impl(int j) const {
  ...
  const Teuchos::RCP<Tpetra_Operator> DfDp = Teuchos::rcp(
      new DistributedParameterDerivativeOpT(
...
Albany::ModelEvaluatorT::createOutArgsImpl() const {
  ...
  result.set_W_properties(
      Thyra::ModelEvaluatorBase::DerivativeProperties(
          ...
          Thyra::ModelEvaluatorBase::DERIV_RANK_FULL, true));
  ...
  for (int i = 0; i < num_dist_param_vecs; i++)
    result.setSupports(
        Thyra::ModelEvaluatorBase::OUT_ARG_DfDp, i + num_param_vecs,
...
Albany::ModelEvaluatorT::evalModelImpl(
...
#ifdef WRITE_MASS_MATRIX_TO_MM_FILE
  // IK, 4/24/15: adding object to hold mass matrix to be written to matrix
  // market file
  ...
  if (Teuchos::nonnull(W_op_out_crsT)) {
    ...
    // reading in of Mass.
    app->computeGlobalJacobianT(
        1.0, 0.0, 0.0, curr_time, x_dotT.get(), x_dotdotT.get(), *xT,
  ...
  for (int l = 0; l < outArgsT.Np(); ++l) {
    ...
    const Teuchos::RCP<Tpetra_MultiVector> dfdp_outT =
        Teuchos::nonnull(dfdp_out) ? ConverterT::getTpetraMultiVector(dfdp_out)
                                   : Teuchos::null;
  ...
  if (app->is_adjoint) {
    ...
    app->evaluateResponseDerivativeT(
        ...
        sacado_param_vec, NULL, NULL, f_derivT, dummy_derivT, dummy_derivT,
        dummy_derivT);
  } else {
  ...
  for (int j = 0; j < outArgsT.Ng(); ++j) {
    ...
    if (supports_xdot) dgdxdotT_out = outArgsT.get_DgDx_dot(j);
    ...
    if (!dgdxT_out.isEmpty() || !dgdxdotT_out.isEmpty()) {
      const Thyra::ModelEvaluatorBase::Derivative<ST> dummy_derivT;
      app->evaluateResponseDerivativeT(
          j, curr_time, x_dotT.get(), x_dotdotT.get(), *xT, sacado_param_vec,
          NULL, gT_out.get(), dgdxT_out, dgdxdotT_out, dgdxdotdotT_out,
          dummy_derivT);
    ...
    for (int l = 0; l < num_param_vecs; ++l) {
      ...
      if (Teuchos::nonnull(dgdpT_out)) {
        ...
        app->evaluateResponseTangentT(
            j, alpha, beta, omega, curr_time, false, x_dotT.get(),
            x_dotdotT.get(), *xT, sacado_param_vec, p_vec.get(), NULL, NULL,
            NULL, NULL, gT_out.get(), NULL, dgdpT_out.get());
        gT_out = Teuchos::null;
      }
    }
    ...
    for (int l = 0; l < num_dist_param_vecs; l++) {
      ...
      if (Teuchos::nonnull(dgdpT_out)) {
        ...
        app->evaluateResponseDistParamDerivT(
            j, curr_time, x_dotT.get(), x_dotdotT.get(), *xT, sacado_param_vec,
            dist_param_names[l], dgdpT_out.get());
      }
    }
