...
#define ALBANY_STATEARRAY_BOUNDS_CHECKING

#include <Albany_StateArray.hpp>
...
std::string
ArrayDimTag::to_string(ArrayDimTag::size_type n, ArrayDimTag::size_type i) const
{
...
const char*
ArrayDimension::name() const
{
  static const char n[] = "anonymous";
  return n;
}
...
ArrayDimension::tag()
{
  static const ArrayDimension self;
  return self;
}
...
void
array_stride_from_fortran_dimensions(
...
void
array_stride_to_fortran_dimensions(
    const array_traits::int_t        rank,
...
void
array_stride_to_fortran_indices(
    const array_traits::int_t        rank,
    const array_traits::int_t* const stride,
    ...
    const array_traits::int_t        arg_i8)
{
  const array_traits::int_t indices[8] = {
      arg_i1, arg_i2, arg_i3, arg_i4, arg_i5, arg_i6, arg_i7, arg_i8};
  ...
  if (arg_natural) {
    ...
    for (array_traits::int_t ord = 0; ord < arg_rank; ++ord) {
      sizes[ord] =
          ord ? arg_stride[ord] / arg_stride[ord - 1] : arg_stride[ord];
    }
  }
