  ...
  numDim(numDim_)
{
  ...
  // Then, we have the Enthalpy and w_z equations, 4 equations in total
  neq += 2;
                                    ...
                                    Albany::StateManager& stateMgr)
{
  ...
  const int vecDim          = 2;	// which is also the number of components of the velocity field
        ...
        const Albany::MeshSpecsStruct& meshSpecs)
{
   ...
   std::vector<std::string> dirichletNames(neq - 1); // we don't construct any Dirichlet evaluator for w_z
   // Firstly, we set the Dirichlet evaluator for Stokes
   for (int i=0; i < 2; i++)
   {
     std::stringstream s;
     s << "U" << i;
...
void FELIX::StokesFOThermoCoupled::constructNeumannEvaluators (const Teuchos::RCP<Albany::MeshSpecsStruct>& meshSpecs)
{
   ...
   // CHECK THIS, NEUMANN IS ONLY USED BY STOKES
   /*
   if (neq>2){
     neumannNames[2] = "U2";
     offsets[2].resize(1);
   ...
   // Even though I don't need this, I definy it.
   if (neq>3){
     neumannNames[3] = "w_z";
     offsets[3].resize(1);
     offsets[3][0] = 3;
...
FELIX::StokesFOThermoCoupled::getValidProblemParameters () const
{
  Teuchos::RCP<Teuchos::ParameterList> validPL =
    this->getGenericProblemParams("ValidStokesFOThermoCoupledProblemParams");
