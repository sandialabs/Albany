...
//												   verticesOnEdge, indexToEdgeID, nGlobalEdges, 50);
    ...
    void velocity_solver_extrude_3d_grid(double const * levelsRatio_F, double const * lowerSurface_F, double const * thickness_F)
    {
		...
		//construct the local vector of coordinates
			...
			slvrfctryPtr = Teuchos::rcp(new Albany::SolverFactory("albany_input.xml", reducedComm));
			discParams = Teuchos::sublist(Teuchos::rcp(&slvrfctryPtr->getParameters(),false), "Discretization", true);
			sis=Teuchos::rcp(new Albany::StateInfoStruct);
