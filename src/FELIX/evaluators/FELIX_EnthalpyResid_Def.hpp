...
namespace FELIX
{
  ...
  homotopy		(p.get<std::string> ("Continuation Parameter Name"), dl->shared_param)
  {
    ...
    this->addDependentField(EnthalpyGrad.fieldTag());
    ...
    if (needsBasFric)
    {
      ...
      if(haveSUPG)
      {
        ...
        //this->addDependentField(basalFricHeatSUPG.fieldTag());
      }
    }
    ...
    if(haveSUPG)
    {
      ...
      //this->addDependentField(geoFluxHeatSUPG.fieldTag());
      this->addDependentField(basalFricHeatSUPG.fieldTag());
    }
    ...
    c_i = physics_list->get("Heat capacity of ice", 2000.0); //[J Kg^{-1} K^{-1}]
    ...
    for (std::size_t cell = 0; cell < d.numCells; ++cell)
    {
      ...
      for (std::size_t node = 0; node < numNodes; ++node)
      {
        for (std::size_t qp = 0; qp < numQPs; ++qp)
        {
          ...
          ScalarT wSUPG = delta*diam/vmax*(Velocity(cell,qp,0) * wGradBF(cell,node,qp,0) + Velocity(cell,qp,1) * wGradBF(cell,node,qp,1) + verticalVel(cell,qp) * wGradBF(cell,node,qp,2) +
