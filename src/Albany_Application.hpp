...
class Application
    : public Sacado::
          ParameterAccessor<PHAL::AlbanyTraits::Residual, SPL_Traits> {
 public:
  enum SolutionMethod
  {
    Steady,
...
#endif
  Teuchos::RCP<AAdapt::AdaptiveSolutionManagerT>
  getAdaptSolMgrT()
  {
    return solMgrT;
  }
 ...
 */
...
#if defined(ALBANY_LCM)
  void
  computeGlobalJacobianSDBCsImplT(
      const double                             alpha,
      const double                             beta,
      const double                             omega,
      const double                             current_time,
 ...
 public:
   ...
   */
  void
  computeGlobalPreconditionerT(
      const Teuchos::RCP<Tpetra_CrsMatrix>& jac,
      const Teuchos::RCP<Tpetra_Operator>&  prec);

#if defined(ALBANY_EPETRA)
  void
  computeGlobalPreconditioner(
 ...
 public:
   ...
   */
  void
  applyGlobalDistParamDerivImplT(
      const double                                  current_time,
      const Teuchos::RCP<const Tpetra_Vector>&      xdotT,
      const Teuchos::RCP<const Tpetra_Vector>&      xdotdotT,
      const Teuchos::RCP<const Tpetra_Vector>&      xT,
 ...
 */
 ...
 public:
  ...
  void
  loadBasicWorksetInfoT(PHAL::Workset& workset, double current_time);
  ...
  void
  setScale(Teuchos::RCP<Tpetra_CrsMatrix> jacT = Teuchos::null);
  void
  setScaleBCDofs(PHAL::Workset& workset);
...
template<typename EvalT>
void
Albany::Application::loadWorksetBucketInfo(
    PHAL::Workset& workset,
    const int&     ws)
{
