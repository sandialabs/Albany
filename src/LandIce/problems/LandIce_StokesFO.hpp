...
protected:
  ...
  // Layouts
  Teuchos::RCP<Albany::Layouts> dl, dl_scalar;
                                      ...
                                      const Teuchos::RCP<Teuchos::ParameterList>& responseList)
{
  ...
  // --- Volume evaluators --- //
  constructStokesEvaluators<EvalT> (fm0, meshSpecs, stateMgr, fieldManagerChoice, extruded_params_levels, is_dist_param);
  ...
  constructSMBEvaluators<EvalT> (fm0, is_dist_param);
...
template <typename EvalT>
void StokesFO::constructStatesEvaluators (PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
                                          const Albany::MeshSpecsStruct& meshSpecs,
                                          Albany::StateManager& stateMgr,
                                          Teuchos::RCP<std::map<std::string, int> > extruded_params_levels,
                                          std::map<std::string,bool>& is_dist_param)
...
template <typename EvalT>
void StokesFO::constructStokesEvaluators (PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
                                          const Albany::MeshSpecsStruct& meshSpecs,
                                          Albany::StateManager& stateMgr,
                                          Albany::FieldManagerChoice fieldManagerChoice,
                                          Teuchos::RCP<std::map<std::string, int> > extruded_params_levels,
                                          std::map<std::string,bool> is_dist_param)
{
  ...
  ev = evalUtils.getMSTUtils().constructQuadPointsToCellInterpolationEvaluator(Albany::coord_vec_name, dl->qp_gradient, dl->cell_gradient, enableMemoizer);
  fm0.template registerEvaluator<EvalT> (ev);

  // Interpolate surface height to a field defined on cells (needed by PressureCorrectedTemperature)
  ev = evalUtils.getPSTUtils().constructQuadPointsToCellInterpolationEvaluator("surface_height", Teuchos::null, Teuchos::null, enableMemoizer);
  fm0.template registerEvaluator<EvalT> (ev);
  ...
  p->set<bool>("Needs Basal Residual", landice_bcs.find(LandIceBC::BasalFriction)!=landice_bcs.end());
  p->set<bool>("Needs Lateral Residual", landice_bcs.find(LandIceBC::Lateral)!=landice_bcs.end());
...
template <typename EvalT>
void StokesFO::constructLateralBCEvaluators (PHX::FieldManager<PHAL::AlbanyTraits>& fm0)
{
  Albany::EvaluatorUtils<EvalT, PHAL::AlbanyTraits> evalUtils(dl);
  Teuchos::RCP<PHX::Evaluator<PHAL::AlbanyTraits> > ev;
  Teuchos::RCP<Teuchos::ParameterList> p;
...
template <typename EvalT>
void StokesFO::constructBasalBCEvaluators (PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
                                           std::map<std::string,bool>& is_dist_param)
{
  Albany::EvaluatorUtils<EvalT, PHAL::AlbanyTraits> evalUtils(dl);
  Teuchos::RCP<PHX::Evaluator<PHAL::AlbanyTraits> > ev;
  ...
  for (auto pl : landice_bcs[LandIceBC::BasalFriction]) {
    ...
    ev = evalUtils.getPSTUtils().constructDOFGradInterpolationSideEvaluator(effective_pressure_side, ssName, enableMemoizer);
    fm0.template registerEvaluator<EvalT>(ev);
...
template <typename EvalT>
void StokesFO::constructSurfaceVelocityEvaluators (PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
                                                   const Albany::MeshSpecsStruct& meshSpecs,
                                                   Albany::StateManager& stateMgr)
{
  Albany::EvaluatorUtils<EvalT, PHAL::AlbanyTraits> evalUtils(dl);
...
template <typename EvalT>
void StokesFO::constructSMBEvaluators (PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
                                       std::map<std::string,bool>& is_dist_param)
{
  Albany::EvaluatorUtils<EvalT, PHAL::AlbanyTraits> evalUtils(dl);
  Teuchos::RCP<PHX::Evaluator<PHAL::AlbanyTraits> > ev;
                                            ...
                                            Teuchos::RCP<std::map<std::string, int> > extruded_params_levels)
{
  ...
  if(neq > vecDimFO) {
    ...
    // Scatter residual
    ev = evalUtils_scalar.constructScatterResidualEvaluatorWithExtrudedParams(false, resid_name_auxiliary, extruded_params_levels, vecDimFO, aux_resid_scatter_tag);
    fm0.template registerEvaluator<EvalT> (ev);
    ...
    p->set<std::string>("Coordinate Vector Variable Name", Albany::coord_vec_name);
    p->set<std::string>("Field Name", "basal_friction_" + ssName);
    p->set<std::string>("Field Gradient Name", "beta_" + ssName + " Gradient");
    ...
    if (fieldManagerChoice == Albany::BUILD_RESID_FM) {
      PHX::Tag<typename EvalT::ScalarT> res_tag(aux_resid_scatter_tag, dl_scalar->dummy);
      fm0.requireField<EvalT>(res_tag);
    }
  }
}
...
template<typename EvalT>
Teuchos::RCP<const PHX::FieldTag>
StokesFO::constructResponsesEvaluators (PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
                                        const Albany::MeshSpecsStruct& meshSpecs,
                                        Albany::StateManager& stateMgr,
                                        Albany::FieldManagerChoice fieldManagerChoice,
                                        const Teuchos::RCP<Teuchos::ParameterList>& responseList)
