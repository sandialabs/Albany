...
LandIce::StokesFOThermoCoupled::constructEvaluators (PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
                                                   const Albany::MeshSpecsStruct& meshSpecs,
                                                   Albany::StateManager& stateMgr,
...
//    fm0.template registerEvaluator<EvalT> (evalUtils.constructDOFGradInterpolationEvaluator(dof_names[0], offset));
  ...
  // Interpolate temperature from nodes to cell
  fm0.template registerEvaluator<EvalT> (evalUtils.constructNodesToCellInterpolationEvaluator("Corrected Temperature",false));
...
/*
  // --- LandIce pressure-melting temperature
  {
    ...
    ev = Teuchos::rcp(new LandIce::PressureCorrectedTemperature<EvalT,PHAL::AlbanyTraits,typename EvalT::ScalarT>(*p,dl));
    fm0.template registerEvaluator<EvalT>(ev);
  }
*/
  //--- LandIce viscosity ---//
  ...
  ev = Teuchos::rcp(new LandIce::ViscosityFO<EvalT,PHAL::AlbanyTraits,typename EvalT::ScalarT,typename EvalT::ScalarT>(*p,dl));
  fm0.template registerEvaluator<EvalT>(ev);
  ...
  // --- Enthalpy Residual ---
  {
    ...
    ev = rcp(new LandIce::EnthalpyResid<EvalT,AlbanyTraits,typename EvalT::ScalarT>(*p,dl));
    fm0.template registerEvaluator<EvalT>(ev);
  }
  ...
  // --- Enthalpy Basal Residual ---
  {
    ...
    ev = rcp(new LandIce::EnthalpyBasalResid<EvalT,AlbanyTraits,typename EvalT::ScalarT>(*p,dl));
    fm0.template registerEvaluator<EvalT>(ev);
  }
  ...
  if(!compute_w)  // --- W_z Residual ---
  {
    ...
    ev = rcp(new LandIce::w_ZResid<EvalT,AlbanyTraits,typename EvalT::ScalarT>(*p,dl));
    fm0.template registerEvaluator<EvalT>(ev);
  }
  else  // --- W Residual ---
  {
    ...
    ev = rcp(new LandIce::w_Resid<EvalT,AlbanyTraits,typename EvalT::ScalarT>(*p,dl));
    fm0.template registerEvaluator<EvalT>(ev);
  }
  ...
  // --- LandIce Temperature: diff enthalpy is h - hs.
  {
    ...
    {
      std::string stateName = "Corrected Temperature";
      entity = Albany::StateStruct::NodalDataToElemNode;
      p = stateMgr.registerStateVariable(stateName, dl->node_scalar, elementBlockName, true, &entity, "");
      p->set<std::string>("Field Name", "Corrected Temperature");
      p->set("Field Layout", dl->node_scalar);
      p->set<bool>("Nodal State", true);
