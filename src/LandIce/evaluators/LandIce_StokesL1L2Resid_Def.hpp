...
StokesL1L2Resid(const Teuchos::ParameterList& p,
  ...
  epsilonXX  (p.get<std::string> ("LandIce EpsilonXX QP Variable Name"), dl->qp_scalar),
  epsilonYY  (p.get<std::string> ("LandIce EpsilonYY QP Variable Name"), dl->qp_scalar),
  epsilonXY  (p.get<std::string> ("LandIce EpsilonXY QP Variable Name"), dl->qp_scalar),
  Residual   (p.get<std::string> ("Residual Name"), dl->node_vector)
{
  ...
  this->setName("StokesL1L2Resid"+PHX::print<EvalT>());
...
//*out << " in LandIce Stokes L1L2 residual! " << endl;
...
if (vecDim != 2)  {TEUCHOS_TEST_FOR_EXCEPTION(true, Teuchos::Exceptions::InvalidParameter,
          std::endl << "Error in LandIce::StokesL1L2Resid constructor:  " <<
