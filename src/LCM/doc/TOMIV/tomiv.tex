...
This section showcases four numerical examples concerning polynomial interpolations and transformation of internal variables from a source mesh to a target mesh. In the first example,  we compare the recovered internal variable fields obtained from global and element-wise projections. We then use a kinematics problem to determine the error caused by improper interpolation of internal variables belonging to Lie group and the remedy procedures formulated via Lie algebra. A solution of a large deformation J2 elasto-plasticity problem is presented to demonstrate the error associated with transformation of internal variables from one mesh to a new mesh using Lie algebra and global variational projection operators. We conclude this section with a benchmark example from literature to illustrate the remarkable accuracy of the proposed internal variable recovery procedure. 

\subsection{Projection of Fields with Jump Condition}
