...
\rhead
[\fancyplain{} {\emph{Projection Operator for Internal Variables}}]
{\fancyplain{} {\emph{Projection Operator for Internal Variables}}}
...
\begin{abstract}
  ...
  A procedure for the mapping of internal variables from one finite element mesh
  to another is proposed. The objective is to perform the mapping with minimum
  error and at the same time guarantee that the mapped variables remain in their
  admissible spaces.
  ...
  \emph{target} or mapped internal variables and a Lagrange multiplier that
  enforces the equality between the \emph{source} and target internal variables.
  This formulation leads to an $L_2$ projection that minimizes the distance
  between the source and target internal variables as measured in the $L_2$ norm
  of the internal variable space.
