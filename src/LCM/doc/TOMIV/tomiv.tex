...
algebra interpolation techniques in a framework where projection is
used in conjugation with logarithmic mapping.  The calculation we
conducted is proposed as a severe test problem by
...
The simulation is performed with two meshes, consisting of 350 and
1680 hexahedral trilinear elements respectively. Notice that the 3D
hexahedral mesh is biased in the cylindrical domain and thus the
projected $F^{p}$ field may not be rotational symmetric with respect
to the vertical axis.  During the simulation, $\bF^{p} =
(\bF^{e})^{-1} \cdot \bF$ is stored. Figure \ref{fig:BilletNOLie}
illustrates two projections of $\bF^{p}$ calculated from the Gauss
points of the two meshes via standard Galerkin projection, without the
application of Lie algebra. Ideally, the plastic deformation of the
material obeying J2 plasticity must be isochoric, as the plastic flow
direction is isochoric. However, this is not the case when projection
...
depicted in Figures \ref{fig:BilletNOLie}(a) and (b) where determinant
of $\bF^{p}$ is plotted as color contour on the deformed meshes
corresponding to $57\% $ upsetting.

The determinant of the plastic split of the projected deformation
gradient ranges from 0.2 to 2.1 in the coarse mesh, and from -2.9 and
5.8 in the fine mesh at $57\%$ upsetting. This severe error occurs
when significant amount of distortion of mesh accumulated at the edge
of the bottom face, as shown in Figure
\ref{fig:BilletNOLie}(a). Interestingly, this error cannot be
eliminated by refinement, as the distortion of the finite element is
inevitable regardless of the mesh size.

\begin{figure}[htbp]
  \begin{center}
    \unitlength=1.0mm
    \subfigure[]{
      \includegraphics[width=80mm]{medium_wo_lie_step31.eps}}
    \subfigure[]{
      \includegraphics[width=80mm]{fine_wo_lie_step31.eps}}
    \caption{The plastic split of the deformation gradient projected
      on deformed meshes with (a) 350 and (b) 1680 hexahedral finite
...
\begin{figure}[htbp]
  \begin{center}
    \unitlength=1.0mm
    \subfigure[]{
      \includegraphics[width=80mm]{medium_w_lie_step31.eps}}
    \subfigure[]{
      \includegraphics[width=80mm]{fine_w_lie_step31.eps}}
    \caption{The plastic split of the deformation gradient projected
      on deformed meshes with (a) 350 and (b) 1680 hexahedral finite
