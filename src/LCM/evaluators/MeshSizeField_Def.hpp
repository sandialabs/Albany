  ...
  evaluateFields(typename Traits::EvalData workset)
  {
    ...
    for (int cell=0; cell < workset.numCells; ++cell) {
      for (int qp=0; qp < numQPs; ++qp) {
        ...
        for (int dim = 0; dim  < numDims; ++dim ) {
          for (int ref_dim = 0; ref_dim  < numDims; ++ref_dim ) { // xi, eta, zeta
            for (int node = 0; node < numNodes; ++node) {
               ...
               // Calculate the element width, in each of the x, y, and z dimensions
               // grad_at_cub_points(node, qp, dim) - node corresponds to the element node (local)
               //                                   - qp is the integration point
               //                                   - ref_dim is xi, etc, zeta dimension
        ...
        // Calculate Euclidean distance of the element in each of the master directions
        for (int ref_dim = 0; ref_dim  < numDims; ++ref_dim ) { // xi, eta, zeta
          for (int dim = 0; dim  < numDims; ++dim ) {
