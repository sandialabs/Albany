                 ...
                 p.get<Teuchos::RCP<PHX::DataLayout> >("QP Vector Data Layout") )
{
  ...
  //**********************************************************************
  template<typename EvalT, typename Traits>
  void SurfaceVectorJump<EvalT, Traits>::
  ...
  {
    LCM::Vector<ScalarT,3> vecA(0.0), vecB(0.0), vecJump(0.0);
    ...
    for (std::size_t cell=0; cell < workset.numCells; ++cell) {
      for (std::size_t pt=0; pt < numQPs; ++pt) {
        ...
        for (std::size_t node=0; node < numPlaneNodes; ++node) {
          ...
          vecA += LCM::Vector<ScalarT,3>(refValues(node, pt) * vector(cell, node, 0),
                                         refValues(node, pt) * vector(cell, node, 1),
                                         refValues(node, pt) * vector(cell, node, 2));
          vecB += LCM::Vector<ScalarT,3>(refValues(node, pt) * vector(cell, topNode, 0),
