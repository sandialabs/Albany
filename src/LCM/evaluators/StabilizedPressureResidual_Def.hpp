...
StabilizedPressureResidual(Teuchos::ParameterList& p,
  ...
  pressure_(p.get<std::string>("Pressure Name"), dl->qp_vector),
  ...
  h_(p.get<std::string>("Element Characteristic Length"), dl->cell_scalar2),
  residual_(p.get<std::string>("Residual Name"), dl->node_scalar),
  small_strain_(p.get<bool>("Small Strain", false)),
  alpha_(p.get<RealType>("Stabilization Parameter", 0.0))
{
...
evaluateFields(typename Traits::EvalData workset)
{
  ...
  //- alpha_ * h_ * h_ / (2.0 * shear_modulus_(cell,pt));
}
//------------------------------------------------------------------------------
}
