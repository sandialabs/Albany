...
namespace LCM {
			...
			PHX::FieldManager<Traits>& fm)
  {
    ...
    // Allocate workspace
    flux = Kokkos::createDynRankView(TGrad.get_view(), "XXX", numCells, numQPs, numDims);
    fluxdt = Kokkos::createDynRankView(TGrad.get_view(), "XXX", numCells, numQPs, numDims);
    pterm = Kokkos::createDynRankView(TGrad.get_view(), "XXX", numCells, numQPs);
    tpterm = Kokkos::createDynRankView(TGrad.get_view(), "XXX", numCells, numNodes, numQPs);
    if (haveAbsorption)  aterm = Kokkos::createDynRankView(TGrad.get_view(), "XXX", numCells, numQPs);
...
evaluateFields(typename Traits::EvalData workset)
{
  ...
  // Pore-Fluid Diffusion Term
   ...
   ScalarT dt = deltaTime(0);
    ...
    FST::scalarMultiplyDataData (flux, kcPermeability.get_view(), TGrad.get_view()); // flux_i = k I_ij p_j
