...
ThermoMechanicalCoefficients<EvalT, Traits>::ThermoMechanicalCoefficients(
  ...
  : thermal_cond_(p.get<std::string>("Thermal Conductivity Name"), dl->qp_scalar),
    thermal_transient_coeff_(
        p.get<std::string>("Thermal Transient Coefficient Name"), dl->qp_scalar),
    thermal_diffusivity_(
        p.get<std::string>("Thermal Diffusivity Name"), dl->qp_tensor),
...
{  
  if (SolutionType_ == "Continuation")
  {
    Albany::MDArray const
    temperature_old = (*workset.stateArrayPtr)[temperature_name_];
  ...
  if (have_mech_) {
    ...
    for (int cell = 0; cell < workset.numCells; ++cell) {
      for (int pt = 0; pt < num_pts_; ++pt) {
        ...
        I(minitensor::eye<RealType>(num_dims_));
