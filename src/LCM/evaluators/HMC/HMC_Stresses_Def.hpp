  ...
  C66                (p.get<RealType>("C66"))
{
  ...
  for(int i=0; i<numMicroScales; i++){
    ...
    strainDifference[i] = Teuchos::rcp(new cHMC2Tensor(p.get<std::string>          (sdname.str()),
                                             p.get<Teuchos::RCP<PHX::DataLayout>>("QP 2Tensor Data Layout") ) );
    std::stringstream sdgradname;
    sdgradname << "Micro Strain Gradient " << i << " Name";
    microStrainGradient[i] = Teuchos::rcp(new cHMC3Tensor(p.get<std::string>          (sdgradname.str()),
...
/*\begin{text}
   ...
   \begin{equation}
     ...
     \end{array} \right\}
   \end{equation}
   where $C_{ijkl}$ are the linear elastic constants at the macroscale, $B_{ijkl} = \beta C_{ijkl}$,
   and $A_{ijklmn} = l^2 \beta C_{ijlm}\delta_{kn}$.
\end{text}*/
template<typename EvalT, typename Traits>
void Stresses<EvalT, Traits>::
