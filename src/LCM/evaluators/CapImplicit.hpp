...
private:
  ...
  ScalarT
  compute_f(LCM::Tensor<ScalarT> & sigma, LCM::Tensor<ScalarT> & alpha, ScalarT & kappa);

  std::vector<ScalarT>
  initialize(LCM::Tensor<ScalarT> & sigmaVal, LCM::Tensor<ScalarT> & alphaVal, ScalarT & kappaVal, ScalarT & dgammaVal);

  void
  compute_ResidJacobian(std::vector<ScalarT> const & XXVal, std::vector<ScalarT> & R,
  ...
  DFadType
  compute_f(LCM::Tensor<DFadType> & sigma, LCM::Tensor<DFadType> & alpha, DFadType & kappa);

  D2FadType
  compute_g(LCM::Tensor<D2FadType> & sigma, LCM::Tensor<D2FadType> & alpha, D2FadType & kappa);
  ...
  LCM::Tensor<DFadType>
  compute_halpha(LCM::Tensor<DFadType> const & dgdsigma, DFadType const J2_alpha);
