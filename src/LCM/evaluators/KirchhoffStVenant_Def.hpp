...
namespace LCM {
  ...
  KirchhoffStVenant<EvalT, Traits>::
  ...
  //**********************************************************************
  template<typename EvalT, typename Traits>
  void KirchhoffStVenant<EvalT, Traits>::
  ...
  evaluateFields(typename Traits::EvalData workset)
  {
    ...
    for (std::size_t cell=0; cell < workset.numCells; ++cell) {
      ...
      for (std::size_t qp=0; qp < numQPs; ++qp) {
        ...
        LCM::Tensor<ScalarT> C(LCM::transpose(F)*F);
        LCM::Tensor<ScalarT> I(LCM::eye<ScalarT>(numDims));
        LCM::Tensor<ScalarT> E(0.5 * (C - I));
        ScalarT J = LCM::det(F);
        
        LCM::Tensor<ScalarT> S = lambda*LCM::trace(E)*I + 2*mu*E;
        LCM::Tensor<ScalarT> sigma = (1/J) * F * S * LCM::transpose(F);
