...
namespace LCM {
  ...
  ThermoPoroPlasticityResidEnergy(const Teuchos::ParameterList& p) :
	...
	gammaMixture (p.get<std::string>           ("Mixture Specific Heat Name"),
	      p.get<Teuchos::RCP<PHX::DataLayout> >("QP Scalar Data Layout") ),
	gammaFluid (p.get<std::string>           ("Pore-Fluid Specific Heat Name"),
	       p.get<Teuchos::RCP<PHX::DataLayout> >("QP Scalar Data Layout") ),
	...
	PGrad       (p.get<std::string>                   ("Pore Pressure Gradient Name"),
		 p.get<Teuchos::RCP<PHX::DataLayout> >("QP Vector Data Layout") ),
...
//Irina TOFIX intrepid
   ...
   //Irina TOFIX FST::integrateTemp<ScalarT>(TResidual, fluxdt, wGradBF, Intrepid::COMP_CPP, false); // "true" sums into
		...
		//  TResidual(cell,node)=0.0;
		  	  for (int qp=0; qp < numQPs; ++qp) {
			      ...
			      dporePressure = porePressure(cell,qp)-porePressureold(cell, qp);
 				  ...
 				  TResidual(cell,node) -=  3.0*dporePressure
	  ...
	  for (int node=0; node < numNodes; ++node) {
		  for (int qp=0; qp < numQPs; ++qp) {
				...
				bulkModulus =  young_modulus_(cell,qp)/3.0/(1.0- 2.0*poissons_ratio_(cell,qp));
 				 ...
 				 TResidual(cell,node) += (dTemperature)
 				  				         *stabParameter(cell, qp)
 				  				         *(gammaMixture(cell, qp) + Temp(cell,qp)
 				  				         *9.0*alphaSkeleton(cell,qp)*bulkModulus
