    ...
    Thyra::ModelEvaluatorBase::OutArgs<ST> const & out_args) const
{
  ...
  for (auto m = 0; m < num_models_; ++m) {
    if (apps_[m]->is_adjoint) {
      const Thyra_Derivative f_deriv(solver_outargs_[m].get_f(),
      ...
      for (auto m = 0; m < num_models_; ++m) {
        ...
        if (mf_prec_type_ == JACOBI) {
          ...
          apps_[m]->computeGlobalJacobian(alpha, beta, omega, curr_time,
              ...
              sacado_param_vecs_[m], fs_out[m], Albany::createThyraLinearOp(jacs_[m]));
      ...
      for (auto m = 0; m < num_models_; ++m) {
        for (auto l = 0; l < out_args.Np(); ++l) {
          ...
          apps_[m]->evaluateResponse(
              ...
              sacado_param_vecs_[m], g_out->getNonconstVectorBlock(m));
        }
      }
    }
  }
