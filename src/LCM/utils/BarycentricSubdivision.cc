...
unsigned int topology::get_number_lower_rank_entities(Entity & entity) {
	...
	for (int ii = 0; ii < entity_relations.size(); ++ii) {
		if (entity.entity_rank()
				> entity_relations[ii].entity()->entity_rank()) {
			count++;
		}
	}
	return count;
 ...
 */
std::vector<Entity*> topology::get_directly_connected_entities(Entity & entity,
...
std::vector<Entity*> topology::get_boundary_entities(Entity & entity,
	...
	for (int ii = given_entity_rank - 1; ii > entity_rank; ii--) {
		for (iterator_entities1 = boundary_entities[ii].begin();
				...
				++iterator_entities1) {
			temp_vector1 = get_directly_connected_entities(
					*(*iterator_entities1), ii - 1);
...
std::vector<Entity*> topology::find_segments_from_element(Entity & element) {
	...
	for (iterator_element_faces = element_faces.begin();
		...
		element_node = get_directly_connected_entities(
				*(*iterator_element_faces), 0);
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	for (int ii = 0; ii < initial_entities_1D.size(); ++ii) {
		...
		vector_nodes = get_directly_connected_entities(
				*(initial_entities_1D[ii]), 0);
	...
	for (int ii = 0; ii < initial_entities_1D.size(); ++ii) {
		...
		for (int i = 0; i < _relations.size(); ++i) {
			if (_relations[i].entity()->entity_rank() == 0
				...
				add_relation(*(modified1_entities_1D[ii]),
						*(_relations[i].entity()), 0);
	...
	for (int ii = 0; ii < initial_entities_1D.size(); ++ii) {
		...
		for (int i = 0; i < _relations.size(); ++i) {
			if (_relations[i].entity()->entity_rank() == 2) {
				add_relation(*(_relations[i].entity()),
						*(modified1_entities_1D[ii]),
						get_number_lower_rank_entities(
								*(_relations[i].entity())));
			}
		}
	}
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	for (iterator_faces = initial_entities_2D.begin();
		...
		vector_segments = get_directly_connected_entities(*(*iterator_faces),
				1);
	...
	for (int ii = 0; ii < initial_entities_2D.size(); ++ii) {
		...
		for (iterator_entities_3D = elements.begin();
			...
			remove_relation(*(*iterator_entities_3D), *(former_face[0]),
					get_local_relation_id(*(*iterator_entities_3D),
							*(former_face[0])));
		}
	}
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	for (int ii = 0; ii < initial_entities_3D.size(); ++ii) {
		element_boundary_nodes = get_boundary_entities(
				*(initial_entities_3D[ii]), 0);
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	for (int ii = 0; ii < All_boundary_segments.size(); ++ii) {
		...
		add_relation(*(modified2_entities_2D[ii]), *(All_boundary_segments[ii]),
				0);
		add_relation(*(modified2_entities_2D[ii]),
				*(adjacent_segments_inside[0]), 1);
		add_relation(*(modified2_entities_2D[ii]),
				*(adjacent_segments_inside[1]), 2);
	}
...
//-----------------------------------------------------------------------------------------------------------------------------------
	...
	for (int ii = 0; ii < All_boundary_faces.size(); ++ii) {
		...
		add_relation(*(modified1_entities_3D[ii]), *(All_boundary_faces[ii]),
				0);
		add_relation(*(modified1_entities_3D[ii]), *adjacent_faces_inside[0],
				1);
		add_relation(*(modified1_entities_3D[ii]), *adjacent_faces_inside[1],
				2);
		add_relation(*(modified1_entities_3D[ii]), *adjacent_faces_inside[2],
				3);
	}
