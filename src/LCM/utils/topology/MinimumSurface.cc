    ...
    std::vector<std::vector<double> > points)
{
  ...
  for (I_BoundaryFaces = BoundaryFaces.begin();
      I_BoundaryFaces != BoundaryFaces.end(); I_BoundaryFaces++) {
...
//
double Topology::getDistanceNodeAndPoint(
...
//
double Topology::getDistanceBetweenNodes(
    ...
    stk::mesh::Entity node2)
{
  ...
  double distance = sqrt(
      pow((x1 - x2), 2.0) + pow((y1 - y2), 2.0) + pow((z1 - z2), 2.0));
  return distance;
}
...
std::vector<std::vector<int> > Topology::facesDirections()
{
  ...
  for (mapIter = face_map.begin(); mapIter != face_map.end(); ++mapIter) {
    int index = mapIter->second;
    std::vector<stk::mesh::Entity> edgeBoundaryNodes = getBoundaryEntities(
        mapIter->first,
        stk::topology::NODE_RANK);
    std::vector<int> temp;
...
std::vector<double> Topology::facesAreas()
{
  ...
  for (mapIter = face_map.begin(); mapIter != face_map.end(); ++mapIter) {
    ...
    //Compute the area
    std::vector<stk::mesh::Entity> Nodes = getBoundaryEntities(
        mapIter->first,
        stk::topology::NODE_RANK);
    ...
    std::vector<std::vector<int> > & shortPath)
{
  ...
  for (unsigned int i = 0; i < edgesDirec.size(); i++) {
    ...
    for (unsigned int j = 0; j < shortPath.size(); j++) {
      ...
      else if (edgesDirec[i][1] == shortPath[j][0]
