...
class PeridigmManager
{
 public:
  struct OutputField
  {
    ...
    int         length;
    bool
    operator==(const OutputField& rhs)
    {
      if (rhs.albanyName != albanyName || rhs.peridigmName != peridigmName ||
          rhs.initType != initType || rhs.relation != relation)
        return false;
      return true;
    }
  };
  ...
  struct PartialStressElement
  {
    ...
    bool
    operator==(const PartialStressElement& rhs)
    {
  ...
  //! Singleton.
  static const Teuchos::RCP<PeridigmManager>&
  self();
  //! Instantiate the singleton
  static void
  initializeSingleton(const Teuchos::RCP<Teuchos::ParameterList>& params);
  ...
  //! Identify the overlapping solid element for each peridynamic sphere element
  //! (applies only to overlapping discretizations).
  void
  obcOverlappingElementSearch();

  //! Evaluate the functional for optimization-based coupling
  double
  obcEvaluateFunctional(
      Epetra_Vector* obcFunctionalDerivWrtDisplacement = NULL);

  //! Load the current time and displacement from Albany into the Peridigm
  //! manager.
  void
  setCurrentTimeAndDisplacement(
      double                                   time,
  ...
  //! Modify Albany graphs for tangent stiffness matrix to include Peridigm
  //! nonzeros.
  void
  insertPeridigmNonzerosIntoAlbanyGraph()
  {
    stkDisc->insertPeridigmNonzerosIntoGraph();
  }

  //! Copy values from the Peridigm tangent stiffness matrix into the Albany
  //! jacobian.
  bool
  copyPeridigmTangentStiffnessMatrixIntoAlbanyJacobian(
  ...
  void
  writePeridigmSubModel(RealType currentTime);

  //! Retrieve the force for the given global degree of freedom
  //! (evaluateInternalForce() must be called prior to getForce()).
  double
  getForce(int globalAlbanyNodeId, int dof);

  //! Computes a least squares fit of the displacement field for all particles
  //! within the horizon of the given node
  double
  getDisplacementNeighborhoodFit(
      int     globalAlbanyNodeId,
      double* coord,
      int     dof);

  //! Retrieve the partial stress tensors for the quadrature points in the given
  //! element (evaluateInternalForce() must be called prior to
  //! getPartialStress()).
  void
  getPartialStress(
  ...
  //! Accessor for the list of solid elements in the overlap region for
  //! optimization-based coupling.
  Teuchos::RCP<std::vector<OBCDataPoint>>
  getOBCDataPoints()
  {
  ...
  getBlockData(std::string blockName, std::string fieldName);

  //! Sets the output variable list from the user-provided ParameterList.
  void
  setOutputFields(const Teuchos::ParameterList& params);

  //! Get the list of peridynamics variables that will be written to Exodus.
  std::vector<OutputField>
  getOutputFields();

  //! Set Dirichlet Fields;
  void
  setDirichletFields(Teuchos::RCP<Albany::AbstractDiscretization> disc);

  //! Get the STK discretization
  Teuchos::RCP<Albany::STKDiscretization>
  getSTKDisc()
  {
    return stkDisc;
  }
 ...
 private:
  ...
  // Private to prohibit use.
  PeridigmManager&
  operator=(const PeridigmManager&);
};
