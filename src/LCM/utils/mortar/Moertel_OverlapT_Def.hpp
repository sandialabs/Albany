...
MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::OverlapT(
 ...
 *----------------------------------------------------------------------*/
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::~Overlap()
{
...
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::build_lines_s()
{
  if (!havemxi_) {
    std::cout << "***ERR*** Moertel::Overlap::build_lines:\n"
              << "***ERR*** projection of master element nodes has to be done "
                 "before\n"
              << "***ERR*** file/line: " << __FILE__ << "/" << __LINE__ << "\n";
    return false;
  }
  ...
  /*
     mxi_[i][j] - the i index is the ith corner node of the master segment. The
     j index selects either the \xi (j = 0) or the \eta (j = 1) coordinate of
     the projection of the node into sseg.

     mline_[i][j] - the i index selects the outside edge of the master segment.
     There are 3 edges for triangular segments, and 4 for quads. j = 0 is the
     \xi coord in sseg of the beginning of the edge, j = 1 is the \eta coord, j
     = 2 is the \xi coord in sseg of the end point of the edge, and j = 3 is the
     \eta coord of the end point of the edge.
     */
...
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::build_mxi()
{
  ...
  for (int i = 0; i < mseg_.Nnode(); ++i) {
    ...
    /*
       ...
       sseg_, and mxi_[i][1] is the \eta coordinate of the projection in sseg_.
       */
...
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::ComputeOverlap()
{
  ...
  // If we pass the quick overlap test, that means that the master and slave
  // segments are close enough to each other that they could interact.
  ...
  // The master nodes that define the master segment are projected into the
  // slave segment using:
  //  ProjectNodetoSegment_SegmentNormal(*mnode[i],sseg_,mxi_[i],gap);

  if (!havemxi_) havemxi_ = build_mxi();

  // build array of local sxi coords of nodes
  // These do not need to be projected as they are the coords of the slave nodes
  // in the slave segment

  havesxi_ = build_sxi();

  // project slave nodes onto master segment
  // These nodal coordinates are projected from the slave segment into the
  // master, using
  ...
  if (!havelines_) havelines_ = build_lines_s();

  // compute information about the edges of the slave/master polygons
  // in master polygon coord system
  if (!havelinem_) havelinem_ = build_lines_m();

  // perform clipping algorithm
  // If this returns false, it means that there is no effective overlap between
...
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::Clipelements()
{
  ...
  // slave segment's coordinate system

  for (int i = 0; i < nmnode; ++i) AddPointtoPolygon(100 + i, &mline_[i][0]);
  ...
  for (int clipedge = 0; clipedge < nsnode; ++clipedge) {
    // point on that clip edge (dim 2). These are the edges of sseg in its \xi
    // \eta space. Note that if sseg is a quad, we are clipping against its
    // parametric space which is a square -1 <= \xi <= 1 and -1 <= \eta <= 1
    ...
    for (int medge = 0; medge < nmnode; ++medge) {
      ...
      if (ok) {
        // std::cout << "OVERLAP Clipelements: adding intersection point " <<
        // 10*clipedge+medge << endl;
  ...
  // that if sseg is a quad, we are clipping against its parametric space which
  // is a square -1 <= \xi <= 1 and -1 <= \eta <= 1

  {
    int np = SizePointPolygon();

    std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)>> point;
    PointView(point);
    int p;
    ...
    for (p = 0; p < np; ++p) {
      ...
      for (int clipedge = 0; clipedge < nsnode; ++clipedge) {
        ...
        // P must be Nodes_Identical_Epsilon (in a cos(theta) sense) in front of
        // the clip edge (on the normal side)
        ...
        if (ok) {
          ...
          // std::cout << "OVERLAP Clipelements: removing point " <<
          // point[p]->Id() << endl;
  ...
  if (np) {
    for (int i = 0; i < nsnode; ++i) {
      ...
      if (!ok)
        ...
        // std::cout << "OVERLAP Clipelements: inserting slave point " << 1000+i
        // << " xi="
  ...
  {
    int np    = SizePointPolygon();
    std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > > point; PointView(point);
    for (int p=0; p<np; ++p)
    {
  ...
  if (!np) {
    ...
    np = SizePointPolygon();
    if (np > 2)
      ;  // all slave points are in master segment, just continue
    else if (np && np <= 2) {
  ...
  if (p_.size() > 3) CollapsePoints(p_, 1.0e-5);
...
/*---------------------------------------------------------------------------------*
  |  Clipping elements using Sutherland-Hodgman algorithm (1974) (private)  gah
 10/10|
 *---------------------------------------------------------------------------------*/
    ...
    double*              N)
{
  ...
  for (unsigned int i = 0; i < source_xi.size(); i++) {
    // clip point s against this edge (PE = point on edge, N is outward normal
    // of edge)
    ...
    if (s_in && p_in) {  // both s and p are in polygon
      ...
      if (!ok) {  // Cannot find the intersection between edge and clip edge
        ...
        oss << "***ERR*** Moertel::Overlap::buildPoly:\n"
            << "***ERR*** Cannot find the intersection between edge and clip "
               "edge\n"
            << "***ERR*** file/line: " << __FILE__ << "/" << __LINE__ << "\n";
        throw ReportError(oss);
      }
  ...
  // Note that here s_in, and point_s are set to the 0th point in the source
  // data We now need to march back up to index
  ...
  return true;  // If we have not thrown an error or returned false, return the
                // polygon
}
...
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::ClipelementsSH()
{
  ...
  // and http://en.wikipedia.org/wiki/Sutherland–Hodgman_algorithm as I write
  // this code.

  // I assume that the master segment is convex. It should be if this is a mesh.
  // Secondly, the slave segment is a square in its parametric space -1 <= \xi
  // <= 1 and -1 <= \eta <= 1. It need only be convex.

  // For the Sutherland-Hodgman algorithm, the slave segment is used for the
  // clip polygon.

  // Start with an input list of polygon vertices, in the slave coordinate
  // system. Note that these points are ordered around the polygon, as i is the
  // line number of the boundaries of the master seg.
  ...
  for (int clipedge = 0; clipedge < nsnode; ++clipedge) {
    ...
    if (!ok) {
      // there is no intersection between polys. However, it is possible that
      // the slave poly is completely contained within the master
  ...
  if (ok) {  // We have a target polygon
    ...
    for (unsigned int p = t_poly_xi.size() - 1; p >= 1; p--) {
      ...
      double dist = MoertelT::length(xi, 2);
  ...
  // We come down here if there is no polygon from the above. This could mean
  // that the slave segment is completely inside the master segment.
  ...
  for (int i = 0; i < nsnode; ++i) {
    ...
    // We will be here, with ok == true only if the point is inside ALL clip
    // edges
  ...
  if (s_node_id.size() <
     ...
     * slave (no nodes). If the slave is completely within the master, then all
     * 4 nodes need to cleanly show up inside the master. If they do not,
     * chances are only a corner or edge of the master is touched.
     */
...
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::Triangulation()
{
  ...
  if (np < 3) {
    std::stringstream oss;
    oss << "***ERR*** MoertelT::Overlap::Triangulization:\n"
  ...
  for (int i = 0; i < np; ++i) {
    ...
    MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)** snodes = sseg_.Nodes();
  ...
  // find projection values for all points in polygon on mseg
  {
    ...
    for (int i = 0; i < np; ++i) {
      ...
      // normal of both segments and do not do the projection if its absolute
      // value
  ...
  // if we plan to interpolate function values at the gaussian points we need
  ...
  if (np > 3) {
    ...
    int                            nodeid[3];
    MoertelT::Segment_BiLinearTri* tmp;
    MoertelT::Function_LinearTri*  func = new Function_LinearTri(OutLevel());
    for (int i = 2; i < np; ++i) {
  ...
  } else if (np == 3)  // single triangle without centerpoint
  {
    ...
    // *this class takes ownership of triangle
    tmp = new MoertelT::Segment_BiLinearTri(0, 3, nodeid, OutLevel());
    // set a linear shape function to this triangle
    tmp->SetFunction(0, func);
  ...
  std::vector<MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)*> nodes(np);
