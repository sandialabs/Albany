...
public:
  ...
  */
  inline const double* XCoords() { if (node_ != Teuchos::null) return node_->XCoords(); else return NULL; }
  ...
  */
  inline Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT) > Node() { return node_; }
  ...
  */
  bool SetNode(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)* node) { node_ = Teuchos::rcp(node); return true; }
...
private:
  ...
  // don't want = operator
  PointT operator = (const PointT& old);
...
// << operator
MOERTEL_TEMPLATE_STATEMENT
std::ostream& operator << (std::ostream& os, const MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)& point);
