...
public:
  ...
  */
  explicit ProjectedNodeT(const MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& basenode, 
          const double* xi, MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)* pseg);
  ...
  */
  explicit ProjectedNodeT(const MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& basenode, const double* xi, 
				  MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)* pseg, int orthseg);
...
protected:  
  // don't want = operator
  ProjectedNodeT operator = (const MoertelT::MOERTEL_TEMPLATE_CLASS(ProjectedNodeT)& old);
...
// << operator
MOERTEL_TEMPLATE_STATEMENT
std::ostream& operator << (std::ostream& os, const Moertel::MOERTEL_TEMPLATE_CLASS(ProjectedNodeT)& pnode);
