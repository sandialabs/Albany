...
public:
  ...
  */
  explicit NodeT(GO Id, std::array<ST, DIM>& x, const std::vector<LO>& dof, bool isonboundary, int out);
  ...
  */
  inline const std::array<ST, DIM> XCoords() const { return x_; }
  ...
  */
  inline bool SetX(std::array<ST, DIM>& x) { for (int i=0; i<DIM; ++i) x_[i] = x[i]; return true; }

  /*!
  \brief Return pointer to vector of length 3 of normal of this node
  
  */
  inline const std::array<ST, DIM> Normal() const { return n_; }
  ...
  */
  inline bool SetN(std::array<ST, DIM>& n) { for (int i=0; i<DIM; ++i) n_[i] = n[i]; return true; }
  ...
  */
  Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(ProjectedNodeT) >* GetProjectedNode(int& length);
  ...
  */
  Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(ProjectedNodeT) >  GetProjectedNode();
  ...
  */
  ...
  \params col : nodal column id of the value added
  */
  void AddMmodValue(int row, ST val, int col);

  /*!
  \brief Get view of the 'D' map of this node
  
  */
  inline Teuchos::RCP<std::map<int,ST> > GetD() { return Drow_; }

  /*!
  \brief Get view of the 'M' map of this node
  
  */
  inline Teuchos::RCP<std::map<int,ST> > GetM() { return Mrow_; }

  /*!
  \brief Get view of the 'Mmod' map of this node
  
  */
  inline Teuchos::RCP<std::vector<std::map<int,ST> > > GetMmod() { return Mmodrow_; }
  /*!
  ...
  */
  std::map<int,MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)*>& GetSupportedByNode() { return supportedby_; }
...
protected:  
  // don't want = operator
  NodeT operator = (const NodeT& old);
...
protected:
  ...
  std::vector<MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)*>    segptr_;   // ptrs to segments length nseg_ (not in charge of destorying the ptrs))
...
// << operator
MOERTEL_TEMPLATE_STATEMENT
std::ostream& operator << (std::ostream& os, const MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node);
