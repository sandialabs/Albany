...
#ifndef MOERTEL_MANAGERT_HPP
#define MOERTEL_MANAGERT_HPP
...
- This class will then choose appropriate mortar and slave sides on all
interfaces (if not already done by the user) such that conflicts are avoided on
...
- It will then perform the construction of a C0-continuous field of normals on
 ...
 Mortar projection operator that projects nodal values from the mortar to the
slave side
- It will perform the integration of the Mortar integrals and assemble the
 ...
 the slave side to the Lagrange multipliers (LMs) and the coupling matrix \b M
that couples the degrees of freedom of the master side to the LMs.
...
- Some limited support in solving the coupled system of equations using direct
 and iterative, multigrid preconditioned solvers is supplied. (not yet
implemented)
- The user has access to coupling matrices \b D and \b M to use in contact
formulations (not yet impl.).
- It supplies a projection operator that projects arbitrary nodal field values
  from the slave side to the master side and vice versa to use in domain
decomposition methods. (not yet impl.)
...
For more detailed information on the usage, see "The Moertel User's Guide",
Sandia report XXXX

For background information on Mortar methods, we refer to <br>
Wohlmuth, B.:"Discretization Methods and Iterative Solvers Based on Domain
Decomposition", Springer, New York, 2001, ISBN 3-540-41083-X
...
class ManagerT
{
  ...
  */
  Teuchos::RCP<const Teuchos::Comm<LO>>
  Comm() const
  {
    return comm_;
  }
  ...
  /*!
  \brief Returns the Level of output (0 - 10) the user specified in the
  constructor
  */
  int
  OutLevel()
  ...
  Returns the number of non-conforming interfaces passed to this class via
  AddInterface
  ...
  */
  ...
  Any number of interfaces can be added. This class does not take ownership over
  the added class. The interface added can be destroyed immediately after the
  call to this method as this class stores a deep copy of it.
  ...
  */
  ...
  - Makes special boundary modifications to the Lagrange multiplier shape
  functions
  ...
  */
  ...
  bool
  Integrate_Interfaces();

  /*!
  \brief Assembles contributions from the D and M matrices into the JFNK
  residual vector.
  ...
  */
  ...
  bool
  AssembleInterfacesIntoResidual();
  ...
  A row map has to be passed to this class before a call to \ref
  Mortar_Integrate()
  ...
  */
  bool
  SetProblemMap(const Teuchos::RCP<const Tpetra::Map<LO, GO, N>>& map)
  {
    problemmap_ = map;
    return true;
  }
  ...
  */
  ...
  Currently Moertel has interfaces to the Amesos, the AztecOO and the ML
  package.<br> All packages are controlled by this parameter list. The parameter
  list Contains a general part where the package and the type of linear system
  ...
  params.set("System","SaddleSystem"); // use a saddle point system of equations
  in solve
  // or
  params.set("System","SPDSystem");    // use a spd system with condensed
  Lagrange multipliers in solve
  ...
  // Moertel sets the Aztec default parameters first which are then overridden
  by
  // this list
  // This is an example configuration:
  ...
  \warning When using ML and/or Aztec, one has to use "SPDSystem" as system
  matrix as the amg-preconditioned iterative method will currently fail on the
           saddle point system.
  */
  void
  SetSolverParameters(Teuchos::ParameterList& params);
  ...
  */
  const Tpetra::CrsMatrix<ST, LO, GO, N>*
  D() const
  {
    return D_.get();
  }
  ...
  */
  Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(InterfaceT)>
  GetInterface(int idx)
  {
    return interface_[idx];
  }
 ...
 private:
  ...
  ManagerT operator = (const MOERTEL_TEMPLATE)CLASS(ManagerT)& old);
  ManagerT(MoertelT::MOERTEL_TEMPLATE_CLASS(ManagerT) & old);
  ...
  Teuchos::RCP<Tpetra::Map<LO, GO, N>>
  LagrangeMultiplierDofs();

  // automatically choose mortar side (called when mortarside==-2 on any
  // interface)
  bool
  ChooseMortarSide();
...
outputs all information stored in the \ref MoertelT::Manager class to
std::ostream
