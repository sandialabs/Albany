...
public:
  ...
  */
  bool AddInterface(const Teuchos::RCP<const MoertelT::MOERTEL_TEMPLATE_CLASS(InterfaceT) >& interface);
  ...
  */
  ...
  bool AssembleInterfacesIntoResidual();
  ...
  */
  Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(InterfaceT) > GetInterface(int idx) { return interface_[idx]; }
...
protected:
  ...
  bool ChooseMortarSideInterface(std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(InterfaceT) > >& inter);
...
// Now, the explicit template function declarations (templated on dimension)
  ...
  template<class ST, class LO, class GO, class N > bool MoertelT::ManagerT<2, ST, LO, GO, N>::Integrate_Interfaces();
  // just do integration of all interfaces 3D case
  template<class ST, class LO, class GO, class N > bool MoertelT::ManagerT<3, ST, LO, GO, N>::Integrate_Interfaces();

  // Build the M and D matrices 2D case
  template<class ST, class LO, class GO, class N > bool MoertelT::ManagerT<2, ST, LO, GO, N>::Build_MD();
  
  // Build the M and D matrices 3D case
  template<class ST, class LO, class GO, class N > bool MoertelT::ManagerT<3, ST, LO, GO, N>::Build_MD();

  template<class ST, class LO, class GO, class N > bool MoertelT::ManagerT<2, ST, LO, GO, N>::
    ChooseMortarSideInterface(std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(InterfaceT) > >& inter);

  template<class ST, class LO, class GO, class N > bool MoertelT::ManagerT<3, ST, LO, GO, N>::
    ChooseMortarSideInterface(std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(InterfaceT) > >& inter);
...
*/
MOERTEL_TEMPLATE_STATEMENT
std::ostream& operator << (std::ostream& os, const MoertelT::MOERTEL_TEMPLATE_CLASS(ManagerT)& node);
