...
public:
  ...
  */
  explicit OverlapT(MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& sseg, MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& mseg, 
...
private:  
  // don't want = operator
  OverlapT operator = (const MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)& old);
...
private:
  ...
  bool ConvexHull(std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& p);
  bool MakeRightTurnUpper(int i,std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& hull);
  bool MakeRightTurnLower(int i,std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& hull);
  void RemovePointBefore(int i,std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& hull);
  void RemovePointAfter(int i,std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& hull);

  //-------------------------------------------make triangulization of polygon
  bool Triangulation();
  
  //-----------------------------------------------------------collapse points
  bool CollapsePoints(std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& p, const double eps);
  
protected:
  // @{ \name Methods to construct triangulation of overlap region

  // add a segment to the triangulization
  bool AddSegment(int id, MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)* seg);
  // return # segments the overlap was discretized with
  int Nseg() { return s_.size(); }
  // get view of segments in triangulation map
  void SegmentView(std::vector<Teuchos::RCP<MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT) > >& segs);

  // @{ \name Methods to construct an overlap polygon

  // add a point to the polygon
  bool AddPointtoPolygon(const int id, const double* P);
  bool AddPointtoPolygon(std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& p, const int id, 
			const double* P);
  ...
  void PointView(std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& points);
  void PointView(std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)l> >& p, 
       std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)> >& points);
  void PointView(std::vector<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)*>& p,const int* nodeids,const int np);
  // copy a point polygon to another polygon
  bool CopyPointPolygon(std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& from, 
  ...
  bool Centroid(double xi[], const std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& points, const int np);
