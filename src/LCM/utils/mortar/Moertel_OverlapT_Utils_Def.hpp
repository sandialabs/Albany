      ...
      std::map<int, Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& to)
{
  ...
  for (pcurr=from.begin(); pcurr != from.end(); ++pcurr)
    ...
    {
	  Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > tmp = Teuchos::rcp(new MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)(pcurr->second->Id(),pcurr->second->Xi(),pcurr->second->OutLevel()));
      to.insert(std::pair<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >(tmp->Id(),tmp));
    }
  return true;
}
...
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::Clip_Intersect(const double* N, const double* PE,
...
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::Guarded_Clip_Intersect(const double* N, const double* PE,
	const double* P0, const double* P1, double* xi){
 ...
 *----------------------------------------------------------------------*/
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::Clip_TestPoint(const double* N, const double* PE, 
 ...
 *----------------------------------------------------------------------*/
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
double MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::Clip_ParameterPointOnLine(const double* P0,const double* P1,const double* P)
{
 ...
 *----------------------------------------------------------------------*/
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::AddSegment(int id, MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)* seg)
{
  Teuchos::RCP<MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT) > tmp = Teuchos::rcp(seg);
  s_.insert(std::pair<int,Teuchos::RCP<MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT) > >(id,tmp));
  return true;
}
 ...
 *----------------------------------------------------------------------*/
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::AddPointtoPolygon(const int id,const double* P)
{
...
//         << " xi=" << P[0] << "/" << P[1] << std::endl;
	...
	Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > p = 
           Teuchos::rcp(new MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)(id,P,OutLevel()));
    p_.insert(std::pair<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >(id,p));
...
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::AddPointtoPolygon(
  ...
  Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > point = 
          Teuchos::rcp(new MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)(id,P,OutLevel()));
  p.insert(std::pair<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >(id,point));
 ...
 *----------------------------------------------------------------------*/
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::RemovePointfromPolygon(const int id,const double* P)
{
...
MOERTEL_TEMPLATE_STATEMENT_1A(class IFace)
void MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::PointView(
       std::vector<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT)*>& p,const int* nodeids,const int np)
{
  p.resize(np);
  
  for (int i=0; i<np; ++i)
  {
	std::map<int,Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >::iterator pcurr = p_.find(nodeids[i]);
    if (pcurr==p_.end())
    {
	  std::stringstream oss;
      oss << "***ERR*** MOERTEL::Overlap::PointView:\n"
...
bool MoertelT::MOERTEL_TEMPLATE_CLASS_1A(OverlapT, IFace)::Centroid(
                      double xi[], 
                      const std::vector<Teuchos::RCP<MoertelT::MOERTEL_TEMPLATE_CLASS(PointT) > >& points, 
                      const int np)
{
  xi[0] = xi[1] = 0.0;
  double A = 0.0;
