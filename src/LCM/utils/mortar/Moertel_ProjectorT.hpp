...
public:
  ...
  */
  bool ProjectNodetoSegment_NodalNormal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
                  MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double xi[],
				  double &gap);
  ...
  \param gap (out) : Gap between node and projection along projection vector.
  */
  bool ProjectNodetoSegment_SegmentNormal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
                  MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double xi[],
				  double &gap);
  ...
  \param gap (out) : Gap between node and projection along projection vector.
  */
  bool ProjectNodetoSegment_SegmentOrthogonal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
                  MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double xi[],
				  double &gap);
  ...
  */
  bool ProjectNodetoSegment_Orthogonal_to_Slave(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& snode, 
                  MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double xi[], 
				  double &gap, MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& sseg);

  //@}

private:  
  // don't want = operator
  ProjectorT operator = (const MoertelT::MOERTEL_TEMPLATE_CLASS(ProjectorT)& old);
  ...
  double evaluate_F_2D_NodalNormal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
         MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double eta, double &gap);

  double evaluate_gradF_2D_NodalNormal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
         MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double eta);

  // evaluate F and gradF functions for ProjectNodetoSegment_SegmentNormal in 2D
  double evaluate_F_2D_SegmentNormal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
         MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double eta, double &gap);

  double evaluate_gradF_2D_SegmentNormal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
         MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double eta);

  // evaluate F and gradF functions for ProjectNodetoSegment_SegmentOrthogonal in 2D
  double evaluate_F_2D_SegmentOrthogonal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
         MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double eta, double &gap);

  double evaluate_gradF_2D_SegmentOrthogonal(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
         MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double eta);
  
  // evalauate F and gradF functions for ProjectNodetoSegment_Orthogonal_to_Slave in 2D
  double evaluate_F_2D_SegmentOrthogonal_to_g(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
         MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double eta, double &gap,
				  double* g);

  double evaluate_gradF_2D_SegmentOrthogonal_to_g(MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
         MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg, double eta, double* g);
  
  //====3D projection methods

  // evaluate F and gradF functions for ProjectNodetoSegment_NodalNormal in 3D
  bool evaluate_FgradF_3D_NodalNormal(double* F,double dF[][3],const MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
                                      MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg,double* eta,double alpha,double &gap);
  // evaluate F and gradF functions for ProjectNodetoSegment_SegmentNormal in 3D
  bool evaluate_FgradF_3D_SegmentNormal(double* F,double dF[][3],const MoertelT::MOERTEL_TEMPLATE_CLASS(NodeT)& node, 
                                      MoertelT::SEGMENT_TEMPLATE_CLASS(SegmentT)& seg,double* eta,double alpha,double &gap);
