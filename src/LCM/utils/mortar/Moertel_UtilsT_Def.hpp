...
#include "Moertel_Segment_bilinearquad.H"
...
MoertelT::solve33T(const double A[][3], double* x, const double* b)
{
  ...
  for (int i = 0; i < 3; ++i) {
    BB(i, 0) = b[i];
    for (int j = 0; j < 3; ++j) AA(i, j) = A[i][j];
  }
    ...
    Teuchos::RCP<Tpetra::CrsMatrix<ST, LO, GO, N>>& A22)
{
  ...
  std::map<int, int> a22gmap;
  {
    ...
    // create the map
    for (int i = 0; i < count; ++i) a22gmap[a22global[i]] = 1;
    a22global.clear();
  }
