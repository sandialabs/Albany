...
void calc_error(stk::io::StkMeshIoBroker& src_broker, std::string field_name, 
              stk::io::StkMeshIoBroker& tgt_broker, std::string tgt_interp_field_name, 
              std::string abs_err_field_name, std::string rel_err_field_name, 
              int tgt_snap_no, std::string source_mesh_input_file, int interpolation_intervals,
              Teuchos::RCP<Teuchos::ParameterList> plist, std::string target_mesh_output_file)
{
  ...
  *out << "  Source field has " << neq << " dofs/node." << std::endl; 
   ...
   FieldType& target_abs_error_field = tgt_broker.meta_data().declare_field<FieldType>( 
   ...
   FieldType& target_rel_error_field = tgt_broker.meta_data().declare_field<FieldType>( 
   ...
   Teuchos::RCP<Ioss::Region> io_region = tgt_broker.get_input_io_region();
    ...
    switch(field_type_num) { 
      ...
      case 0: //VectorFieldType 
      {
        *out << "Interpolating and calculating error in fields of type Node Vector..." << std::endl; 
        calc_error<stk::mesh::Field<double, stk::mesh::Cartesian>>(src_broker, field_name, 
                      tgt_broker, tgt_interp_field_name, abs_err_field_name, rel_err_field_name, 
                      tgt_snap_no, source_mesh_input_file, interpolation_intervals, plist, target_mesh_output_file); 
        break; 
      }
      case 1: //ScalarFieldType 
      { 
        *out << "Interpolating and calculating error in fields of type Node Scalar..." << std::endl; 
        calc_error<stk::mesh::Field<double>>(src_broker, field_name, 
                      tgt_broker, tgt_interp_field_name, abs_err_field_name, rel_err_field_name, 
                      tgt_snap_no, source_mesh_input_file, interpolation_intervals, plist, target_mesh_output_file); 
        break; 
      }
      ...
      { 
        *out << "Interpolating and calculating error in fields of type Node Scalar..." << std::endl; 
        calc_error<stk::mesh::Field<double, shards::ArrayDimension>>(src_broker, field_name, 
                      tgt_broker, tgt_interp_field_name, abs_err_field_name, rel_err_field_name, 
                      tgt_snap_no, source_mesh_input_file, interpolation_intervals, plist, target_mesh_output_file); 
        break;  
      }
