...
CP::hardeningParameterFactory(CP::HardeningLawType type_hardening_law)
{
  ...
  switch (type_hardening_law) {
    ...
    case HardeningLawType::LINEAR_MINUS_RECOVERY:
      return HPUP(
          new CP::LinearMinusRecoveryHardeningParameters<NumDimT, NumSlipT>());
      break;
    ...
    case HardeningLawType::DISLOCATION_DENSITY:
      return HPUP(
          new CP::DislocationDensityHardeningParameters<NumDimT, NumSlipT>());
      break;
    ...
    HardeningLawType type_hardening_law) const
{
  switch (type_hardening_law) {
    ...
    case HardeningLawType::LINEAR_MINUS_RECOVERY:
      return allocator_
          .create<LinearMinusRecoveryHardeningLaw<NumDimT, NumSlipT, ArgT>>();
      break;

    case HardeningLawType::SATURATION:
      return allocator_
          .create<SaturationHardeningLaw<NumDimT, NumSlipT, ArgT>>();
      break;

    case HardeningLawType::DISLOCATION_DENSITY:
      return allocator_
          .create<DislocationDensityHardeningLaw<NumDimT, NumSlipT, ArgT>>();
      break;
    ...
    bool&                                         failed)
{
  ...
  minitensor::Vector<ArgT, NumSlipT> const driver_hardening =
      slip_family.latent_matrix_ * rate_slip_abs;

  auto const& phardening_params = slip_family.phardening_parameters_;

  auto const modulus_recovery =
      phardening_params->getParameter(Params::MODULUS_RECOVERY);

  auto const modulus_hardening =
      phardening_params->getParameter(Params::MODULUS_HARDENING);

  auto const hardness_initial =
  ...
  if (modulus_recovery > 0.0) {
    for (minitensor::Index ss_index(0); ss_index < num_slip_sys; ++ss_index) {
      ...
      RealType const effective_slip_n =
          ...
          std::log(
              1.0 -
              modulus_recovery / modulus_hardening *
                  (state_hardening_n[ss_index_global] - hardness_initial));
      ...
      state_hardening_np1[ss_index_global] =
          modulus_hardening / modulus_recovery *
              (1.0 -
               std::exp(
                   -modulus_recovery *
                   (effective_slip_n + dt * driver_hardening[ss_index]))) +
          hardness_initial;
    ...
    std::vector<CP::SlipSystem<NumDimT>> const& slip_systems)
{
  ...
  for (minitensor::Index ss_index_i(0); ss_index_i < slip_family.num_slip_sys_;
       ++ss_index_i) {
    auto const slip_system_i =
        slip_systems[slip_family.slip_system_indices_[ss_index_i]];

    for (minitensor::Index ss_index_j(0);
         ss_index_j < slip_family.num_slip_sys_;
         ++ss_index_j) {
      auto const slip_system_j =
          slip_systems[slip_family.slip_system_indices_[ss_index_j]];
    ...
    bool&                                         failed)
{
  ...
  minitensor::Vector<ArgT, NumSlipT> const driver_hardening =
      2.0 * slip_family.latent_matrix_ * rate_slip_abs;
  ...
  if (effective_slip_rate < CP::TINY) {
    for (minitensor::Index ss_index(0); ss_index < num_slip_sys; ++ss_index) {
      ...
      state_hardening_np1[ss_index_global] =
          state_hardening_n[ss_index_global] + dt * driver_hardening[ss_index];
  ...
  auto const stress_saturation_initial =
      phardening_params->getParameter(Params::STRESS_SATURATION_INITIAL);

  auto const rate_slip_reference =
      phardening_params->getParameter(Params::RATE_SLIP_REFERENCE);

  auto const exponent_saturation =
      phardening_params->getParameter(Params::EXPONENT_SATURATION);

  auto const rate_hardening =
      phardening_params->getParameter(Params::RATE_HARDENING);

  auto const resistance_slip_initial =
      phardening_params->getParameter(Params::STATE_HARDENING_INITIAL);
  ...
  for (minitensor::Index ss_index(0); ss_index < num_slip_sys; ++ss_index) {
    ...
    if (exponent_saturation > 0.0) {
      ...
      stress_saturation =
          stress_saturation_initial * std::pow(ratio_rate, exponent_saturation);
    }
        ...
        std::vector<CP::SlipSystem<NumDimT>> const& slip_systems)
{
  ...
  for (minitensor::Index ss_index_i(0); ss_index_i < slip_family.num_slip_sys_;
    ...
    for (minitensor::Index ss_index_j(0);
         ss_index_j < slip_family.num_slip_sys_;
         ++ss_index_j) {
      slip_family.aux_matrix_(ss_index_i, ss_index_j) = std::sqrt(
          1.0 -
          std::pow(slip_family.latent_matrix_(ss_index_i, ss_index_j), 2));
    }
  }
    ...
    bool&                                         failed)
{
  ...
  RealType const factor_generation =
      phardening_params->getParameter(Params::FACTOR_GENERATION);

  RealType const factor_annihilation =
      phardening_params->getParameter(Params::FACTOR_ANNIHILATION);

  RealType const factor_geometry_dislocation =
      phardening_params->getParameter(Params::FACTOR_GEOMETRY_DISLOCATION);

  RealType const modulus_shear =
      phardening_params->getParameter(Params::MODULUS_SHEAR);

  RealType const magnitude_burgers =
      phardening_params->getParameter(Params::MAGNITUDE_BURGERS);
  ...
  for (minitensor::Index ss_index(0); ss_index < num_slip_sys; ++ss_index) {
    ...
    ArgT const generation =
        factor_generation * std::sqrt(densities_forest[ss_index]);

    ArgT const annihilation =
        factor_annihilation * state_hardening_np1[ss_index_global];
  ...
  minitensor::Vector<ArgT, NumSlipT> const densities_parallel =
      slip_family.aux_matrix_ * state_hardening_np1;
  ...
  for (minitensor::Index ss_index(0); ss_index < num_slip_sys; ++ss_index) {
    ...
    slip_resistance[ss_index_global] = factor_geometry_dislocation *
                                       modulus_shear * magnitude_burgers *
