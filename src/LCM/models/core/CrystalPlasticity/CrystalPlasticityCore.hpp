...
struct SlipSystem
{
  ...
  minitensor::Index slip_family_index_;

  //! Slip system vectors.
  minitensor::Vector<RealType, MAX_DIM> s_;

  minitensor::Vector<RealType, MAX_DIM> n_;
...
class SlipFamily
{
 public:
  ...
  void
  setHardeningLawType(HardeningLawType law);
  HardeningLawType
  getHardeningLawType() const
  {
    return type_hardening_law_;
  }
  void
  setFlowRuleType(FlowRuleType rule);
  FlowRuleType
  getFlowRuleType() const
  {
    return type_flow_rule_;
  }

  minitensor::Index num_slip_sys_{0};
  ...
  minitensor::Tensor<RealType, NumSlipT> latent_matrix_;

  minitensor::Tensor<RealType, NumSlipT> aux_matrix_;

 private:
  HardeningLawType type_hardening_law_{HardeningLawType::UNDEFINED};

  FlowRuleType type_flow_rule_{FlowRuleType::UNDEFINED};
};
...
struct StateMechanical
{
  ...
  InputTensorType const F_n_;

  InputTensorType const Fp_n_;

  TensorType const F_np1_;
