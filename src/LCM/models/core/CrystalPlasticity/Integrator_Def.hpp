...
utility::StaticPointer<
    typename CP::IntegratorFactory<EvalT, NumDimT, NumSlipT>::IntegratorBase>
    ...
    case CP::IntegrationScheme::IMPLICIT: {
      switch (residual_type) {
        ...
        case CP::ResidualType::CONSTRAINED_SLIP_HARDNESS: {
          using IntegratorType = CP::ImplicitConstrainedSlipHardnessIntegrator<
              EvalT,
              NumDimT,
              NumSlipT>;
          return allocator_.create<IntegratorType>(
...
CP::ExplicitIntegrator<EvalT, NumDimT, NumSlipT>::update() const
{
  ...
  minitensor::Tensor<ScalarT, NumDimT> Fp_n_FAD(
      state_mechanical_.Fp_n_.get_dimension());

  for (minitensor::Index i = 0;
       i < state_mechanical_.Fp_n_.get_number_components();
       ++i) {
  ...
  minitensor::Vector<ScalarT, NumSlipT> slip_computed(
      state_internal_.slip_np1_.get_dimension());
...
CP::ImplicitIntegrator<EvalT, NumDimT, NumSlipT>::reevaluateState() const
{
  ...
  std::stringstream ss;
  ss << "slips_" << state_internal_.cell_ << "_" << state_internal_.pt_
     << ".out";
...
CP::ImplicitSlipIntegrator<EvalT, NumDimT, NumSlipT>::update() const
{
  ...
  using StepType = minitensor::
      StepBase<NonlinearSolver, ValueT, CP::NlsDim<NumSlipT>::value>;

  std::unique_ptr<StepType> pstep = minitensor::
      stepFactory<NonlinearSolver, ValueT, CP::NlsDim<NumSlipT>::value>(
          step_type_);
  ...
  minitensor::Vector<ScalarT, NumSlipT> slip_rate(
      state_internal_.rates_slip_.get_dimension());
...
{
  // Unknown for solver
  minitensor::Vector<ScalarT, CP::NlsDim<NumSlipT>::value> x(
      this->num_slip_ * 2);
  ...
  using StepType = minitensor::
      StepBase<NonlinearSystem, ValueT, CP::NlsDim<NumSlipT>::value>;

  std::unique_ptr<StepType> pstep = minitensor::
      stepFactory<NonlinearSystem, ValueT, CP::NlsDim<NumSlipT>::value>(
          step_type_);
