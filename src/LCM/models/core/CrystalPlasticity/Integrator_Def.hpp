...
CP::ExplicitIntegrator<EvalT, NumDimT, NumSlipT>::ExplicitIntegrator(
      ...
      RealType dt)
  : Base(nox_status_test, slip_systems, slip_families, plasticity_state, slip_state, C, F_np1, dt)
{
...
CP::ImplicitSlipIntegrator<EvalT, NumDimT, NumSlipT>::ImplicitSlipIntegrator(
  ...
  : Base(minimizer, step_type, nox_status_test, slip_systems,
         slip_families, plasticity_state, slip_state, C, F_np1, dt)
{
}
    ...
    RealType & residual_norm) const
{
  ...
  Intrepid2::Vector<ScalarT, NumSlipT>
  slip_rate(slip_state_.rate_.get_dimension());

  slip_rate.fill(Intrepid2::ZEROS);
  if (dt_ > 0.0) {
    slip_rate = (slip_state_.slip_np1_ - slip_state_.slip_n_) / dt_;
  }
    ...
    RealType & residual_norm) const
{
  ...
  for (int i = 0; i < this->num_slip_; ++i) {
    ...
    x(i + this->num_slip_) = 
    Sacado::ScalarValue<ScalarT>::eval(slip_state_.hardening_n_(i));
  }
  ...
  for(int i=0; i<this->num_slip_; ++i) {
    slip_state_.slip_np1_[i] = x[i];
    slip_state_.hardening_np1_[i] = x[i + this->num_slip_];
  }
