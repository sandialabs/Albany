  ...
  {    
    ...
    for (int cell = 0; cell < workset.numCells; ++cell) {
      for (int qp = 0; qp < num_pts_; ++qp) {
        ...
        // elastic matrix
        minitensor::Tensor4<ScalarT> Celastic = lame * minitensor::identity_3<ScalarT>(3)
          + mu * (minitensor::identity_1<ScalarT>(3) + minitensor::identity_2<ScalarT>(3));
              
        // elastic compliance tangent matrix
        minitensor::Tensor4<ScalarT> compliance = (1. / bulkModulus / 9.)
          * minitensor::identity_3<ScalarT>(3) + (1. / mu / 2.)
          * (0.5 * (minitensor::identity_1<ScalarT>(3) 
        ...
        minitensor::Tensor<ScalarT> dev_plastic = deps_plastic 
        ...
        if(compute_tangent_) {
          minitensor::Tensor4<ScalarT> Cep = compute_Cep(
...
//------------------------------ yield function ------------------------------// 
  ...
  CapImplicitModel<EvalT, Traits>::compute_f(minitensor::Tensor<T> & sigma, 
    minitensor::Tensor<T> & alpha, T & kappa)
  {
...
//------------------------ unknow variable value list ------------------------//    
  ...
  CapImplicitModel<EvalT,Traits>::initialize(minitensor::Tensor<ScalarT> & sigmaVal, 
    minitensor::Tensor<ScalarT> & alphaVal, ScalarT & kappaVal, ScalarT & dgammaVal)
  {
...
//---------------------------- plastic potential -----------------------------//            
  ...
  CapImplicitModel<EvalT, Traits>::compute_g(
    minitensor::Tensor<T> & sigma, minitensor::Tensor<T> & alpha, T & kappa)
  {
    minitensor::Tensor<T> xi = sigma - alpha;
...
//--------------------------- hardening functions ----------------------------//    
  ...
  CapImplicitModel<EvalT, Traits>::compute_halpha(
    minitensor::Tensor<T> const & dgdsigma, T const J2_alpha)
  {     
    ...
    //minitensor::Tensor<T, 3> halpha = calpha * Galpha * s; // * operator not defined;
...
//------------------------ elasto-plastic tangent modulus --------------------//    
  ...
  CapImplicitModel<EvalT, Traits>::compute_Cep(
    minitensor::Tensor4<ScalarT> & Celastic, minitensor::Tensor<ScalarT> & sigma,
    minitensor::Tensor<ScalarT> & alpha, ScalarT & kappa, ScalarT & dgamma)
  {
...
//-------------------- elasto-plastic perfect tangent modulus ----------------//    
    ...
    minitensor::Tensor<ScalarT> & alpha, ScalarT & kappa, ScalarT & dgamma)
  {
    ...
    // compute tangent   
    Cepp = Celastic - 1.0 / chi * minitensor::dotdot(Celastic, 
      minitensor::tensor(dgdsigma, minitensor::dotdot(dfdsigma, Celastic)) );
