...
FerroicDriver<EvalT, Traits>::
FerroicDriver(Teuchos::ParameterList* p,
...
/******************************************************************************/
{
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(num_dims_ != FM::THREE_D, std::invalid_argument,
...
//        newfractions[vnt] = newBinFractions[vnt](cell,qp);
      ...
      ferroicModel->computeState(/* In:  strain, Efield, ...  */ x, E, oldfractions,
                                 /* Out: stress, Edisp,  ...  */ X, D, newfractions);
