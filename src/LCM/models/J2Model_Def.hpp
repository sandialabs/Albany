...
//#if !defined(ALBANY_KOKKOS_UNDER_DEVELOPMENT) || defined(PHX_KOKKOS_DEVICE_TYPE_CUDA)
  ...
  minitensor::Tensor<ScalarT> F(num_dims_), be(num_dims_), s(num_dims_), sigma(
      num_dims_);
  minitensor::Tensor<ScalarT> N(num_dims_), A(num_dims_), expA(num_dims_), Fpnew(
      num_dims_);
...
compute_common(const int cell) const{
  ...
  minitensor::Tensor<ScalarT> F(dims_), be(dims_), s(dims_), sigma(dims_);
  minitensor::Tensor<ScalarT> N(dims_), A(dims_), expA(dims_), Fpnew(dims_);
  minitensor::Tensor<ScalarT> I(minitensor::eye<ScalarT>(dims_));
  minitensor::Tensor<ScalarT> Fpn(dims_), Fpinv(dims_), Cpinv(dims_);
...
/*     for (int i(0); i < dims_; ++i) {
     ...
     for (int i(0); i < dims_; ++i) {
        for (int j(0); j < dims_; ++j) {
          ScalarT theta=(1.0/dims_) * minitensor::trace(be);
          s(i,j) = mu * (be(i,j)-theta*I(i,j));
      }
    }*/
     mubar = minitensor::trace(be) * mu / (dims_);
     smag = minitensor::norm(s);
     f = smag - sq23 * (Y + K * eqpsold(cell, pt)
         + sat_mod_ * (1. - std::exp(-sat_exp_ * eqpsold(cell, pt))));
...
compute_with_temperature(const int cell) const{
  ...
  minitensor::Tensor<ScalarT> sigma(dims_), F(dims_);
  minitensor::Tensor<ScalarT> I(minitensor::eye<ScalarT>(dims_));
