...
//------------------------------------------------------------------------------
template<typename EvalT, typename Traits>
void CapExplicitModel<EvalT, Traits>::
    ...
    std::map<std::string, Teuchos::RCP<PHX::MDField<ScalarT>>> eval_fields)
{
  ...
  Albany::MDArray backStressold = (*workset.stateArrayPtr)[backStress_string
      + "_old"];
  Albany::MDArray capParameterold = (*workset.stateArrayPtr)[capParameter_string
      + "_old"];
  ...
  for (int cell = 0; cell < workset.numCells; ++cell) {
    for (int qp = 0; qp < num_pts_; ++qp) {
      ...
      mu = elastic_modulus(cell, qp) / 2.0
          / (1.0 + poissons_ratio(cell, qp));
      ...
      if (f > 0.0) {
        ...
        while (condition == false) {
          ...
          kai = minitensor::dotdot(dfdsigma,
              minitensor::dotdot(Celastic, dgdsigma));
          ...
          // update
          sigmaK = sigmaVal
              - delta_gamma * minitensor::dotdot(Celastic, dgdsigma);
...
CapExplicitModel<EvalT, Traits>::compute_f(
    minitensor::Tensor<ScalarT> & sigma, minitensor::Tensor<ScalarT> & alpha,
    ScalarT & kappa)
{
...
//------------------------------------------------------------------------------
template<typename EvalT, typename Traits>
typename CapExplicitModel<EvalT, Traits>::ScalarT CapExplicitModel<EvalT, Traits>::compute_Galpha(
    ScalarT & J2_alpha)
{
  if (N != 0)
...
template<typename EvalT, typename Traits>
typename CapExplicitModel<EvalT, Traits>::ScalarT
CapExplicitModel<EvalT, Traits>::compute_dedkappa(
    ScalarT & kappa)
{
