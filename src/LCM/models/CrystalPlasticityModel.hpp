...
class CrystalPlasticityKernel : public ParallelKernel<EvalT, Traits>
{
 public:
  ...
  /// Pointer to NOX status test, allows the material model to force a global
  /// load step reduction
  ...
  CrystalPlasticityKernel&
  operator=(CrystalPlasticityKernel const&) = delete;
  ...
  ///  global load step, assuming the load-step-reduction feature is active.
 ...
 private:
  ...
  RealType c11_temperature_coeff_{0.0};

  RealType c12_temperature_coeff_{0.0};

  RealType c13_temperature_coeff_{0.0};

  RealType c33_temperature_coeff_{0.0};

  RealType c44_temperature_coeff_{0.0};

  RealType c66_temperature_coeff_{0.0};

  RealType reference_temperature_{0.0};

  RealType norm_slip_residual_{0.0};

  int num_iter_residual_{0};

  minitensor::Tensor<RealType, CP::MAX_DIM> element_block_orientation_;
  ...
  int index_element_{0};

  /// Unrotated elasticity tensor
  minitensor::Tensor4<ScalarT, CP::MAX_DIM> C_unrotated_;

  /// Vector of structs holding slip system family data
  std::vector<CP::SlipFamily<CP::MAX_DIM, CP::MAX_SLIP>> slip_families_;

  /// Vector of structs holding slip system data
  std::vector<CP::SlipSystem<CP::MAX_DIM>> slip_systems_;

  /// Flags for reading lattice orientations from file
  bool read_orientations_from_mesh_{false};

  ///
  /// Solution options
  ///
  CP::IntegrationScheme integration_scheme_{CP::IntegrationScheme::UNDEFINED};

  CP::ResidualType residual_type_{CP::ResidualType::UNDEFINED};

  CP::PredictorSlip predictor_slip_{CP::PredictorSlip::UNDEFINED};

  minitensor::StepType step_type_{minitensor::StepType::UNDEFINED};

  /// Minisolver Minimizer
  minitensor::Minimizer<ValueT, CP::NLS_DIM> minimizer_;

  /// ROL Minimizer
  ROL::MiniTensor_Minimizer<ValueT, CP::NLS_DIM> rol_minimizer_;

  ///
  /// Output options
  ///
  CP::Verbosity verbosity_{CP::Verbosity::UNDEFINED};

  bool write_data_file_{false};
  ...
  ScalarField velocity_gradient_plastic_;
  ...
  std::string const dt_string_ = "Delta Time";

  std::string const temperature_string_ = "Temperature";
  ...
  Albany::MDArray previous_defgrad_;
