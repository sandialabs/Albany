                  ...
                  const Teuchos::RCP<const Tpetra_Vector>& initial_guessT)
{
  ...
  Teuchos::Array< Teuchos::RCP<Tpetra_Map> > disc_maps(n_models_); 
  Teuchos::Array< Teuchos::RCP<Tpetra_Map> > disc_overlap_map(n_models_); 
  ...
  */
  ...
  for (int m=0; m<n_models_; m++) {
   ...
   for (int m=0; m<n_models_; m++) {
    //FIXME: set disc_map = apps[m]->getMapT(); 
    //FIXME: set disc_overlap_map = apps[m]->getStateMgr().getDiscretization()->getOverlapMapT(); 
    //FIXME: set solver_inargs_[m] = models_[m]->createInArgs(); 
    //FIXME: set solver_outargs_[m] = models_[m]->createOutArgs(); 
    //FIXME: set num_params[m] = solver_inargs_[m].Np(); 
    //FIXME: set num_responses[m] = solver_outargs_[m].Ng(); 
    //Does it make sense for num_params_total and num_responses_total to be the sum 
    //of these values for each model?  I guess so. 
    //FIXME: set num_params_total_ += num_params[m]; 
    //FIXME: set num_responses_total_ += num_responses[m]; 
   }
   ...
   //FIXME: Do we need MaterialDB Filename?  If so, need to add. 
   //
