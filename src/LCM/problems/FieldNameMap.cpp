...
FieldNameMap::FieldNameMap(bool surface_flag)
{
  ...
  for (int sys{0}; sys < max_slip_systems; ++sys) {
    std::string const g = Albany::strint("gamma", sys + 1, '_');
  ...
  for (int sys{0}; sys < max_slip_systems; ++sys) {
    std::string const g_dot = Albany::strint("gamma_dot", sys + 1, '_');
  ...
  for (int sys{0}; sys < max_slip_systems; ++sys) {
    std::string const t_h = Albany::strint("tau_hard", sys + 1, '_');
  ...
  for (int sys{0}; sys < max_slip_systems; ++sys) {
    std::string const tau = Albany::strint("tau", sys + 1, '_');
