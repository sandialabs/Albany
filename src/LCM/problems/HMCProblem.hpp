...
/*\begin{text}
This source has been annotated with latex comments.  Use the eqcc script to
compile into a summary pdf.  The source is best viewed using folding in vim
(i.e., \begin{verbatim} :g/\\begin{text}/foldc \end{verbatim}) \end{text}*/
    ...
    const Teuchos::RCP<Teuchos::ParameterList>& responseList)
{
  ...
  *out << "Field Dimensions: Workset=" << worksetSize
  ...
  // Define Field Names:
  /*\begin{text}
  The 'Displacement' and 'Microstrain_n' variables defined below are increments,
  ...
  //   micro
  Teuchos::ArrayRCP<Teuchos::ArrayRCP<std::string>> micro_dof_names(
      numMicroScales);
  Teuchos::ArrayRCP<Teuchos::ArrayRCP<std::string>> micro_resid_names(
      numMicroScales);
  Teuchos::ArrayRCP<Teuchos::ArrayRCP<std::string>> micro_scatter_names(
      numMicroScales);
  for (int i = 0; i < numMicroScales; i++) {
    ...
    micro_dof_names[i][0] =
        std::string("Delta") + Albany::strint(strMicrostrain, i);
  ...
  Teuchos::ArrayRCP<Teuchos::ArrayRCP<std::string>> micro_dof_names_dotdot(
      numMicroScales);
  Teuchos::ArrayRCP<Teuchos::ArrayRCP<std::string>> micro_resid_names_dotdot(
      numMicroScales);
  Teuchos::ArrayRCP<Teuchos::ArrayRCP<std::string>> micro_scatter_names_dotdot(
      numMicroScales);
  ...
  for (int i = 0; i < numMicroScales; i++) {
    ...
    micro_resid_names_dotdot[i][0] =
        micro_resid_names_dotdot[i][0] + " Residual";
    micro_scatter_names_dotdot[i][0] =
        micro_scatter_names_dotdot[i][0] + " Scatter";
  }
  ...
  /*\begin{text}
     The total displacement variable is registered as two state (i.e.,
  registerOldState=true),
    ...
    \begin{tabular}{l l l l}
       $u_{Ii}^N$     & Displacement at state N   & "Displacement_current"  &
  dims(cell,I=nNodes,i=vecDim) \\ $u_{Ii}^{N+1}$ & Displacement at state N+1 &
  "Displacement_updated"  & dims(cell,I=nNodes,i=vecDim) \end{tabular}
  \end{text}*/
  std::string strDisplacement_Updated =
      strDisplacement;  // updated state, i.e., state at N+1
  std::string strDisplacement_Current =
      strDisplacement + "_old";  // current state, i.e., state at N
  {
    RCP<ParameterList> p = rcp(new ParameterList);
    p                    = stateMgr.registerStateVariable(
        strDisplacement_Updated,
        dl->node_vector,
        dl->dummy,
        eb_name,
        /* init type = */ "scalar",
  ...
  /*\begin{text}
     ...
     \textbf{DEPENDENT FIELDS:} \\
       $u_{Ii}^N$      & Displacement at state N   & "Displacement_old"   &
  dims(cell,I=nNodes,i=vecDim) \\
       $\Delta u_{Ii}$ & Displacement Increment    & "DeltaDisplacement"  &
  dims(cell,I=nNodes,i=vecDim) \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $u_{Ii}^{N+1}$  & Displacement at state N+1 & "Displacement"       &
  dims(cell,I=nNodes,i=vecDim) \\ \end{tabular} \end{text}*/
  {
  ...
  /*\begin{text}
     The total microstrain variables are registered as two state (i.e.,
  registerOldState=true),
  ...
  \\
    ...
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\epsilon_{Iij}^{n\ N}$   & Total Microstrain n at state N   &
  "totalMicrostrain_n"     & dims(cell,I=nNodes,i=vecDim) \\
       $\epsilon_{Iij}^{n\ N+1}$ & Total Microstrain n at state N+1 &
  "totalMicrostrain_n_old" & dims(cell,I=nNodes,i=vecDim) \end{tabular}
  \end{text}*/
  ...
  for (int i = 0; i < numMicroScales; i++) {
    ...
    strMicrostrains_Updated[i] =
        strMicrostrains[i];  // updated state, i.e., state at N+1
    strMicrostrains_Current[i] =
        strMicrostrains[i] + "_old";  // current state, i.e., state at N
    p = stateMgr.registerStateVariable(
        strMicrostrains[i],
        dl->qp_tensor,
        dl->dummy,
  ...
  /*\begin{text}
     ...
     \textbf{DEPENDENT FIELDS:} \\
       $\epsilon_{Iij}^N$      & Microstrain n at state N   &
  "Microstrain_n_old"   & dims(cell,I=nNodes,i=vecDim,j=vecDim) \\
       $\Delta \epsilon_{Iij}$ & Microstrain n Increment    &
  "DeltaMicrostrain_n"  & dims(cell,I=nNodes,i=vecDim,j=vecDim) \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\epsilon_{Iij}^{N+1}$  & Microstrain n at state N+1 & "Microstrain_n" &
  dims(cell,I=nNodes,i=vecDim,j=vecDim) \\ \end{tabular} \end{text}*/
  for (int i = 0; i < numMicroScales; i++) {
  ...
  \\
    ...
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $u_{Ii}$ & Nodal displacements & "Displacement"         &
  dims(cell,I=nNodes,i=vecDim) \\ $a_{Ii}$ & Nodal accelerations &
  "Displacement\_dotdot" & dims(cell,I=nNodes,i=vecDim) \end{tabular}
  \end{text}*/
  ...
  \\
    ...
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\epsilon^n_{Iij}$        & Nodal microstrains at scale 'n'        &
  "Microstrain\_n"         & dims(cell,I=nNodes,i=vecDim,j=vecDim) \\
       $\ddot{\epsilon}^n_{Iij}$ & Nodal micro accelerations at scale 'n' &
  "Microstrain\_n\_dotdot" & dims(cell,I=nNodes,i=vecDim,j=vecDim)
    \end{tabular} \\
  \end{text}*/
  ...
  The single argument to constructGatherCoordinateVectorEvaluator causes the
  created evaluator to add
  ...
  fm0.template registerEvaluator<EvalT>(
      evalUtils.constructGatherCoordinateVectorEvaluator(
          strDisplacement_Current));

  // Compute gradient matrix and weighted basis function values in current
  // coordinates
  /*\begin{text}
   ...
   \\
      ...
      %
      $N_I(\mathbf{x}_p)\ det\left(\frac{\partial x_{ip}}{\partial \xi_j}\right)
   \omega_p$ &
      Weighted basis function values & "wBF" & dims(cell,I=nNode,p=nQPs)\\
      %
      $\frac{\partial N_I (\mathbf{x}_p)}{\partial \xi_k} J^{-1}_{kj}$ &
      Gradient matrix wrt physical frame & "Grad BF" &
   dims(cell,I=nNodes,p=nQPs,j=spcDim)\\
      %
      $\frac{\partial N_I (\mathbf{x}_p)}{\partial \xi_k} J^{-1}_{kj}
   det\left(\frac{\partial x_{ip}}{\partial \xi_j}\right) \omega_p$ &
      Weighted gradient matrix wrt current config & "wGrad BF" &
   dims(cell,I=nNodes,p=nQPs,j=spcDim)\\
      \end{tabular} \\
   \end{text}*/
  fm0.template registerEvaluator<EvalT>(
  ...
  /*\begin{text}
    ...
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $u_{Ii}$      & Nodal Displacements  & "Displacements"  &
  dims(cell,I=nNodes,i=vecDim) \\
       $N_I(\xi_p)$  & Basis Functions      & "BF"             &
  dims(cell,I=nNodes,p=nQPs)   \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $u_i(\xi_p)$ & Displacements at quadrature points & "Displacements" &
  dims(cell,p=nQPs,i=vecDim)
    \end{tabular} \\
  \end{text}*/
  fm0.template registerEvaluator<EvalT>(
  ...
  /*\begin{text}
    ...
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $\epsilon^n_{Iij}$  & Nodal microstrains at scale 'n'  & "Microstrain\_n"
  & dims(cell,I=nNodes,i=vecDim,j=vecDim) \\
       $N_I(\xi_p)$        & Basis Functions                  & "BF" &
  dims(cell,I=nNodes,p=nQPs)            \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\epsilon^n_{ij}(\xi_p)$ & Microstrains at scale 'n' at quadrature points
  & "Microstrain\_n"  & dims(cell,p=nQPs,i=vecDim,j=spcDim)
    \end{tabular} \\
  \end{text}*/
  for (int i = 0; i < numMicroScales; i++)
    fm0.template registerEvaluator<EvalT>(
        evalUtils.constructDOFTensorInterpolationEvaluator(
            micro_dof_names[i][0], dof_offset + i * dof_stride));
  ...
  /*\begin{text}
    ...
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $a_{Ii}$      & Nodal Acceleration  & "Displacement\_dotdot"  &
  dims(cell,I=nNodes,i=vecDim) \\
       $N_I(\xi_p)$  & Basis Functions     & "BF"                    &
  dims(cell,I=nNodes,p=nQPs)   \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $a_i(\xi_p)$ & Acceleration at quadrature points  &
  "Displacement\_dotdot"  & dims(cell,p=nQPs,i=vecDim)
    \end{tabular} \\
  \end{text}*/
  fm0.template registerEvaluator<EvalT>(
      evalUtils.constructDOFVecInterpolationEvaluator(
          macro_dof_names_dotdot[0]));
  ...
  /*\begin{text}
    ...
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $\ddot{\epsilon}^n_{Iij}$  & Nodal micro acceleration at scale 'n'  &
  "Microstrain\_n\_dotdot"  & dims(cell,I=nNodes,i=vecDim,j=vecDim)\\
       $N_I(\xi_p)$               & Basis Functions                        &
  "BF"                      & dims(cell,I=nNodes,p=nQPs) \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\ddot{\epsilon}^n_{ij}(\xi_p)$  & Micro acceleration at scale 'n' at
  quadrature points &  "Microstrain\_n\_dotdot"  &
  dims(cell,p=nQPs,i=vecDim,j=vecDim)
    \end{tabular} \\
  \end{text}*/
  for (int i = 0; i < numMicroScales; i++)
    fm0.template registerEvaluator<EvalT>(
        evalUtils.constructDOFTensorInterpolationEvaluator(
            micro_dof_names_dotdot[i][0], dof_offset + i * dof_stride));
  ...
  /*\begin{text}
    ...
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $x_{Ii}$  & Nodal coordinates        & "Coord Vec"  &
  dims(cell,I=nNodes,i=vecDim) \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
     \begin{tabular}{l l l l}
       $x_{pi}$  & Gauss point coordinates  & "Coord Vec"  &
  dims(cell,p=nQPs,i=vecDim) \\
     \end{tabular} \\
  \end{text}*/
  fm0.template registerEvaluator<EvalT>(
  ...
  /*\begin{text}
     New evaluator:
    \begin{align*}
       \left.\frac{\partial u_i}{\partial x_j}\right|_{\xi_p} &= \partial_j
  N_{I}(\xi_p) u_{iI}\\ (c,p,i,j) &= (c,I,p,j)*(c,I,i) \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $u_{Ii}$      & Nodal Displacement           & "Displacement"  &
  dims(cell,I=nNodes,i=vecDim)        \\
       $B_I(\xi_p)$  & Gradient of Basis Functions  & "Grad BF"       &
  dims(cell,I=nNodes,p=nQPs,i=vecDim) \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\left.\frac{\partial u_i}{\partial x_j}\right|_{\xi_p}$  \\
       & Gradient of node vector  & "Displacement Gradient"  &
  dims(cell,p=nQPs,i=vecDim,j=spcDim)
    \end{tabular} \\
  \end{text}*/
  fm0.template registerEvaluator<EvalT>(
  ...
  \partial_k N_{I}(\xi_p) \epsilon^n_{Iij}\\ (c,p,i,j,k) &= (c,I,p,k)*(c,I,i,j)
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $\epsilon^n_{Iij}$  & Nodal microstrain at scale 'n'  & "Microstrain\_n"
  & dims(cell,I=nNodes,i=vecDim,j=vecDim) \\
       $B_I(\xi_p)$        & Gradient of Basis Functions     & "Grad BF" &
  dims(cell,I=nNodes,p=nQPs,i=vecDim)   \\
     \end{tabular} \\
    ...
    \begin{tabular}{l l l l}
       $\left.\frac{\partial \epsilon^n_{ij}}{\partial x_k}\right|_{\xi_p}$
       & Microstrain gradient at scale 'n'  & "Microstrain\_n Gradient"  &
  dims(cell,p=nQPs,i=vecDim,j=vecDim,k=spcDim)
    \end{tabular} \\
  \end{text}*/
  for (int i = 0; i < numMicroScales; i++) {
    fm0.template registerEvaluator<EvalT>(
        evalUtils.constructDOFTensorGradInterpolationEvaluator(
            micro_dof_names[i][0], dof_offset + i * dof_stride));
    ...
    std::string strMSGrad_Updated =
        msGrad;  // updated state, i.e., state at N+1
    std::string strMSGrad_Current =
        msGrad + "_old";  // current state, i.e., state at N
    {
  ...
  /*\begin{text}
    ...
    \begin{align*}
       \epsilon^p_{ij} &=
                 \frac{1}{2}\left(\left.\frac{\partial u_i}{\partial
  x_j}\right|_{\xi_p}
                                 +\left.\frac{\partial u_j}{\partial
  x_i}\right|_{\xi_p} \right)\\ (c,p,i,j) &= ((c,p,i,j)+(c,p,j,i)/2.0)
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $\left.\frac{\partial u_i}{\partial x_j}\right|_{\xi_p}$
       & Gradient of displacement  & "Displacement Gradient"  & dims(cell,
  p=nQPs, i=vecDim, j=spcDim)
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\epsilon^p_{ij}$ & Infinitesimal strain  & "Strain" & dims(cell, p=nQPs,
  i=vecDim, j=spcDim) \\
    \end{tabular} \\
  \end{text}*/
  {
    ...
    // Input
    p->set<std::string>(
        "Gradient QP Variable Name", macro_dof_names[0] + " Gradient");
  ...
  std::string strStrain_Updated =
      strStrain;  // updated state, i.e., state at N+1
  std::string strStrain_Current =
      strStrain + "_old";  // current state, i.e., state at N
  {
  ...
  /*\begin{text}
    ...
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $\epsilon^p_{ij}$   & Macro strain                     & "Strain" &
  dims(cell, p=nQPs, i=vecDim, j=spcDim) \\
       $\epsilon^n_{ijI}$  & Nodal microstrains at scale 'n'  & "Microstrain\_1"
  & dims(cell, I=nNodes, i=vecDim, j=vecDim) \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\Delta\epsilon^{np}_{ij}$ & Strain Difference at scale 'n'  & "Strain
  Difference n"  & dims(cell, p=nQPs, i=vecDim, j=spcDim) \\
    \end{tabular} \\
  \end{text}*/
  for (int i = 0; i < numMicroScales; i++) {
    ...
    // Output
    p->set<std::string>(
        "Strain Difference Name", Albany::strint("Strain Difference", i));
  ...
  for (int i = 0; i < numMicroScales; i++) {
    RCP<ParameterList> p =
        rcp(new ParameterList("Increment of Strain Difference"));
  ...
  /*\begin{text}
    ...
    \end{align*}
     ...
     \begin{tabular}{l l l l}
       $\epsilon^p_{ij}$          & Macro strain             & "Strain" &
  dims(cell, p=nQPs, i=vecDim, j=spcDim) \\
       $\Delta\epsilon^{np}_{ij}$ & Strain Difference 'n'    & "Strain
  Difference 1"      & dims(cell, p=nQPs, i=vecDim, j=spcDim) \\
       $\epsilon^{np}_{ij,k}$     & Microstrain gradient 'n' & "Microstrain\_n
  Gradient"  & dims(cell, p=nQPs, i=vecDim, j=vecDim, k=spcDim)
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $\sigma^p_{ij}$               & Macro Stress       & "Stress"           &
  dims(cell, p=nQPs, i=vecDim, j=spcDim) \\
       $\bar{beta}^{np}_{ij}$        & Micro stress 'n'   & "Micro Stress n"   &
  dims(cell, p=nQPs, i=vecDim, j=spcDim) \\
       $\bar{\bar{beta}}^{np}_{ijk}$ & Double stress 'n'  & "Double Stress n"  &
  dims(cell, p=nQPs, i=vecDim, j=spcDim, k=spcDim) \\
    \end{tabular} \\
  \end{text}*/
  {
    RCP<ParameterList> p =
  ...
  /*\begin{text}
    ...
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $\sigma^{tp}_{ij}$            & Total stress       & "Total Stress"     &
  dims(cell, p=nQPs, i=vecDim, j=spcDim) \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
  ...
  if (params->isType<Teuchos::RCP<ATO::Topology>>("Topology")) {
    Teuchos::RCP<ATO::Topology> topology =
        params->get<Teuchos::RCP<ATO::Topology>>("Topology");
    ...
    if (topology->getEntityType() == "Distributed Parameter") {
      ...
      ev = rcp(
          new PHAL::GatherScalarNodalParameter<EvalT, AlbanyTraits>(*p, dl));
      fm0.template registerEvaluator<EvalT>(ev);
    }
    ...
    Teuchos::ParameterList& wfParams =
        params->sublist("Apply Topology Weight Functions");

    p->set<std::string>("BF Name", "BF");
    p->set<std::string>("Unweighted Variable Name", macroKinematicFieldName);
    p->set<std::string>(
        "Weighted Variable Name", macroKinematicFieldName + "_Weighted");
    ...
    // if(some input stuff)
    atoUtils.SaveCellStateField(
        fm0,
        stateMgr,
        macroKinematicFieldName + "_Weighted",
        eb_name,
        dl->qp_tensor);
    ...
    for (int i = 0; i < numMicroScales; i++) {
      ...
      // if(some input stuff)
      atoUtils.SaveCellStateField(
          fm0, stateMgr, ms + "_Weighted", eb_name, dl->qp_tensor);
    }
    for (int i = 0; i < numMicroScales; i++) {
      RCP<ParameterList> p = rcp(new ParameterList("TopologyWeighting"));
      ...
      // if(some input stuff)
      atoUtils.SaveCellStateField(
          fm0, stateMgr, ds + "_Weighted", eb_name, dl->qp_tensor3);
    }
  }
  /******************************************************************************/
  ...
  det\left(\frac{\partial x_{ip}}{\partial \xi_j}\right) \omega_p
  \sigma^{tp}_{ij}
     + \sum_p N_I(\mathbf{x}_p)\ det\left(\frac{\partial x_{ip}}{\partial
  \xi_j}\right) \omega_p a^p_i \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $\sigma^{tp}_{ij}$ & Total stress &  "Total Stress" & dims(cell, p=nQPs,
  i=vecDim, j=spcDim) \\
       $\frac{\partial N_I (\mathbf{x}_p)}{\partial \xi_k} J^{-1}_{kj}
  det\left(\frac{\partial x_{ip}}{\partial \xi_j}\right) \omega_p$ &
       Weighted Gradient matrix wrt current config & "wGrad BF" & dims(cell,
  I=nNodes, p=nQPs, i=spcDim)\\
       $a^p_i$ & Acceleration at quadrature points & "Displacement\_dotdot" &
  dims(cell, p=nQPs, i=vecDim)\\ $N_I(\mathbf{x}_p)\ det\left(\frac{\partial
  x_{ip}}{\partial \xi_j}\right) \omega_p$ &
       Weighted BF &  "wBF"  & dims(cell, I=nNodes, p=nQPs)\\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $f_{Ii}$ & Macroscale Residual  & "Displacement Residual"  & dims(cell,
  I=nNodes, i=spcDim)\\
    \end{tabular} \\
  \end{text}*/
  {
    ...
    p->set<std::string>(
        "Time Dependent Variable Name", macro_dof_names_dotdot[0]);
  ...
  /*\begin{text}
    ...
    \begin{align*}
      f^n_{Iij} = \sum_p
      \frac{\partial N_I (\mathbf{x}_p)}{\partial \xi_l} J^{-1}_{lk}
  \bar{\bar{\beta}}^{np}_{ijk}\} det\left(\frac{\partial x_{ip}}{\partial
  \xi_j}\right) \omega_p
    ...
    \end{align*}
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $\bar{beta}^{np}_{ij}$         & Micro stress 'n'         & "Micro Stress
  n"         & dims(cell, p=nQPs, i=vecDim, j=spcDim) \\
       $\bar{\bar{beta}}^{np}_{ijk}$  & Double stress 'n'        & "Double
  Stress n"        & dims(cell, p=nQPs, i=vecDim, j=spcDim, k=spcDim) \\
       $\ddot{\epsilon}^n_{Iij}$      & micro accel at scale 'n' &
  "Microstrain\_n\_dotdot" & dims(cell,I=nNodes,i=vecDim,j=vecDim) \\
       $\frac{\partial N_I (\mathbf{x}_p)}{\partial \xi_l} J^{-1}_{lk}
  det\left(\frac{\partial x_{ip}}{\partial \xi_j}\right) \omega_p$ &
       Weighted Gradient matrix wrt current config & "wGrad BF" & dims(cell,
  I=nNodes, p=nQPs, i=spcDim)\\ $N_I(\mathbf{x}_p)\ det\left(\frac{\partial
  x_{ip}}{\partial \xi_j}\right) \omega_p$ &
       Weighted BF &  "wBF"  & dims(cell, I=nNodes, p=nQPs)\\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
    \begin{tabular}{l l l l}
       $f^n_{Iij}$  & Residual at scale 'n'  & "Microstrain\_n Residual"  &
  dims(cell, I=nNodes, i=vecDim, j=vecDim)\\
    \end{tabular} \\
  \end{text}*/
  for (int i = 0; i < numMicroScales; i++) {
    ...
    // extra input for time dependent term
    p->set<std::string>(
        "Time Dependent Variable Name", micro_dof_names[i][0] + "_dotdot");
    p->set<RCP<DataLayout>>("QP Vector Data Layout", dl->qp_vector);
  ...
  /*\begin{text}
     Register new evaluator: Scatter the nodal forces from grid based structures
  to solver data structures.\\
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $f_{Ii}$ & Macroscale Residual  & "Displacement Residual"  & dims(cell,
  I=nNodes, i=vecDim) \\
  ...
  to solver data structures.\\
     \textbf{DEPENDENT FIELDS:} \\
     \begin{tabular}{l l l l}
       $f^n_{Iij}$ & Microscale Residual  & "Microstrain\_n Residual"  &
  dims(cell, I=nNodes, i=vecDim, j=vecDim) \\
     \end{tabular} \\
    \textbf{EVALUATED FIELDS:} \\
       None. \\
  ...
  {  // Time
    ...
    fm0.template registerEvaluator<EvalT>(ev);
    p = stateMgr.registerStateVariable(
        "Time", dl->workset_scalar, dl->dummy, eb_name, "scalar", 0.0, true);
    ev = rcp(new PHAL::SaveStateField<EvalT, AlbanyTraits>(*p));
    fm0.template registerEvaluator<EvalT>(ev);
  }
  ...
  if (fieldManagerChoice == Albany::BUILD_RESID_FM) {
    ...
    for (int i = 0; i < numMicroScales; i++) {  // Micro forces
      PHX::Tag<typename EvalT::ScalarT> res_tag(
          micro_scatter_names[i][0], dl->dummy);
      fm0.requireField<EvalT>(res_tag);
    }
...
template <typename EvalT>
void
Albany::HMCProblem::registerStateVariables(
    Teuchos::RCP<LCM::ConstitutiveModelInterface<EvalT, PHAL::AlbanyTraits>>
                                           cmiEv,
    PHX::FieldManager<PHAL::AlbanyTraits>& fm0,
    Albany::StateManager&                  stateMgr,
