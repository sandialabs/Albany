...
TEUCHOS_UNIT_TEST( Projection, Instantiation )
{
  ...
  // LameStress evaluator
  Teuchos::RCP<Teuchos::ParameterList> MRStressParameterList = Teuchos::rcp(new Teuchos::ParameterList("Stress"));
  MRStressParameterList->set<string>("DefGrad Name", "Deformation Gradient");
  MRStressParameterList->set<string>("Stress Name", "Stress");
  MRStressParameterList->set< Teuchos::RCP<PHX::DataLayout> >("QP Scalar Data Layout", qp_scalar);
  MRStressParameterList->set< Teuchos::RCP<PHX::DataLayout> >("QP Tensor Data Layout", qp_tensor);
  MRStressParameterList->set<string>("Lame Material Model", "Elastic_New");
  Teuchos::ParameterList& materialModelParametersList = MRStressParameterList->sublist("Lame Material Parameters");
  ...
  Teuchos::RCP<LCM::MooneyRivlin<PHAL::AlbanyTraits::Residual, PHAL::AlbanyTraits> > MRStress =
    Teuchos::rcp(new LCM::MooneyRivlin<PHAL::AlbanyTraits::Residual, PHAL::AlbanyTraits>(*MRStressParameterList));
  ...
  for(std::vector<Teuchos::RCP<PHX::FieldTag> >::const_iterator it = MRStress->evaluatedFields().begin() ; it != MRStress->evaluatedFields().end() ; it++)
