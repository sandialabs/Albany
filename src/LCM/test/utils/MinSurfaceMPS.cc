...
int main (int ac, char* av[]){
	...
	//--------------------------------------------------------------------------------------------------------
	//NOTE: When analyzing a single mesh, the coefficientsObjFunction, and the BoundaryOperator
	//remain the same. This is set in the first part of the MPS file.The only one parameter that varies is the
	//BoundaryVector, which is defined in the second part of the MPS file
	//--------------------------------------------------------------------------------------------------------
	...
	std::string file_name2 = file_name_.substr(0, Position); //name of the problem (important for the LP solver that reads the mps file)
	...
	if (ac == 2)
	{
		...
		if (mpsFile.is_open())
		{
			...
			mpsFile << "NAME" <<" "<< "MPS_"<<file_name2<<"_" <<endl; //The linear problem name comes from the second argument in the command line
			mpsFile << "ROWS" <<endl;
			...
			for (unsigned int i = 0; i < outputForMpsFile.size(); i+=4)
			{
				...
				mpsFile <<" "<<"X"<<outputForMpsFile[i+1][0]<<" "<<"Z"<<" "<<setprecision(numeric_limits<long double>::digits10)<<outputForMpsFile[i][0]<<endl;
				mpsFile <<" "<<"X"<<outputForMpsFile[i+1][0]<<" "<<"R"<<outputForMpsFile[i+1][1]<<" "<<outputForMpsFile[i+1][2]<<endl;
				mpsFile <<" "<<"X"<<outputForMpsFile[i+2][0]<<" "<<"R"<<outputForMpsFile[i+2][1]<<" "<<outputForMpsFile[i+2][2]<<endl;
				mpsFile <<" "<<"X"<<outputForMpsFile[i+3][0]<<" "<<"R"<<outputForMpsFile[i+3][1]<<" "<<outputForMpsFile[i+3][2]<<endl;
				mpsFile <<" "<<"X"<<outputForMpsFile[i+1][3]<<" "<<"Z"<<" "<<setprecision(numeric_limits<long double>::digits10)<<outputForMpsFile[i][0]<<endl;
				mpsFile <<" "<<"X"<<outputForMpsFile[i+1][3]<<" "<<"R"<<outputForMpsFile[i+1][4]<<" "<<outputForMpsFile[i+1][5]<<endl;
				mpsFile <<" "<<"X"<<outputForMpsFile[i+2][3]<<" "<<"R"<<outputForMpsFile[i+2][4]<<" "<<outputForMpsFile[i+2][5]<<endl;
				mpsFile <<" "<<"X"<<outputForMpsFile[i+3][3]<<" "<<"R"<<outputForMpsFile[i+3][4]<<" "<<outputForMpsFile[i+3][5]<<endl;
			}
		}
	}
	...
	else
	{
		...
		int numberNormals_; //Variable to define the number of rows of the matrix mNormals
		if (normals_.is_open())
		{
		...
		for(int i = 0; i < numberNormals_; i++)
		{
			...
			if (mpsFile.is_open())
			{
				...
				{
					mpsFile <<" "<<"RHS1"<<" "<<"R"<<BoundaryVector[i][0]<<" "<<BoundaryVector[i][1]<<endl;
				}
