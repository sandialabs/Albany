...
int main(int ac, char* av[]){
	...
	const int TotalNumberNodes = MeshNodes.size();//Total number of nodes of the input the mesh
	...
	dijkstra_shortest_paths(Graph, source, predecessor_map(&predecessor[0]).distance_map(&dis[0])); //Compute the shortest path
	...
	//When changing the sourceVertex, MAKE SURE TO CHANGE "source" above. Otherwise, an run time error will be displayed
	vertex_descriptor sourceVertex = vertex(3,Graph);//Source from where the distance and path are calculated (Same as above!)
	vertex_descriptor goalVertex = vertex(4, Graph);


	graph_traits< graph_t >::vertex_descriptor currentVertex=goalVertex;
	while(currentVertex!=sourceVertex) { ShortestPath.push_back(currentVertex);
