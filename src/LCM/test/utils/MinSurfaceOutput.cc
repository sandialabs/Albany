...
int main(int ac, char* av[]){
	...
	if (ac == 3){
		std::cout <<  "Generating the msh file with mesh " << av[1] << " and from text file: " << av[2] << std::endl;
		stringstream ss1;
		stringstream ss2;
	...
	//BASED ON THE OUTPUT USING CLP LINEAR SOLVER (./clp mps_file -dualsimplex -solu outputFileName)

	using namespace std;

	//Obtain the number of columns of the boundary operator
	std::vector<std::vector<int> > facesDirec =  topology.facesDirections();
	cout << "The number of columns of the boundary operator is: " <<facesDirec.size()*2<<endl;

    //Read input from text file
	std::ifstream solSolver(results_file.c_str()); //Read the file that comes from the solver
	...
	cout << "The number of faces that belong to the minimum surface are: " <<EntitiesMinSurface.size() << endl;
	...
	std::set<int> boundaryNodes; std::vector<std::vector<int> >::const_iterator I_rows;
	std::vector<int>::const_iterator I_cols;
	for (I_rows = boundaryNodes_.begin();I_rows != boundaryNodes_.end();I_rows++)
	{
		for (I_cols = I_rows->begin();I_cols != I_rows->end();I_cols++)
		{
			boundaryNodes.insert(*I_cols);//All nodes organized in ascending order. Starting from 1
		}
	}
	...
	if (outputToVtk.is_open())
	{
		...
		outputToVtk << dimension << " " << NumberNodesPerElement <<endl; //Dimension  NodesPerElement
		outputToVtk << boundaryNodes.size() << " " << boundaryNodes_.size()<<endl;//Nnodes  Nelements
		...
		for (unsigned int i = 0; i < boundaryNodes_.size(); ++i){
			for (unsigned int j = 0; j < boundaryNodes_[i].size(); ++j){
				outputToVtk << (node_map.find(boundaryNodes_[i][j])->second)+1 << " "; //List of edges     OUTPUT
			}
			outputToVtk << endl;
		}
	}//end if
