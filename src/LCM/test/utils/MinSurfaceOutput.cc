...
int main(int ac, char* av[])
{
  ...
  std::vector<stk::mesh::Entity> EntitiesMinSurface = topology
      .minimumSurfaceFaces(resultsVector);
  ...
  if (outputToVtk.is_open()) {
    ...
    for (I_setOfNodes = boundaryNodes.begin();
        I_setOfNodes != boundaryNodes.end(); ++I_setOfNodes) {
      std::vector<double> nodeCoordinates = topology.findCoordinates(
          *I_setOfNodes);
