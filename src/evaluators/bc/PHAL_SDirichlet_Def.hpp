...
// TODO: remove this include when you manage to abstract away from Tpetra the
// Jacobian impl.
    ...
    typename Traits::EvalData dirichlet_workset)
{
  ...
  //       actual Tpetra_Vector is stored as a
  //       Teuchos::ConstNonconstObjectContainer, which (most likely) happens to
  //       be created from a const RCP, and therefore when calling
  //       getTpetraVector (from Thyra::TpetraVector), the container will throw.
    ...
    typename Traits::EvalData dirichlet_workset)
{
  ...
  Teuchos::RCP<Tpetra_CrsMatrix> J =
      Albany::getTpetraMatrix(dirichlet_workset.Jac);
  ...
  {
    auto row_is_dbc_data = row_is_dbc_->getLocalViewHost();
    ...
    typename Traits::EvalData dirichlet_workset)
{
  ...
  for (auto local_row = min_local_row; local_row <= max_local_row;
    ...
    for (size_t row_entry = 0; row_entry < num_row_entries; ++row_entry) {
      ...
      if (is_diagonal_entry && row_is_dbc) { entries[row_entry] *= scale; }
    ...
    typename Traits::EvalData dirichlet_workset)
{
  ...
  /* Draft of implementation
    ...
    if (JV != Teuchos::null) {
      ...
      Teuchos::RCP<Tpetra_CrsMatrix> J =
    Albany::getTpetraMatrix(dirichlet_workset.Jac); jac_diag = Teuchos::rcp(new
    Tpetra_Vector(J->getRowMap())); J->getLocalDiagCopy(*jac_diag);
    }
    if (fp != Teuchos::null) {
      // TODO: abstract away the tpetra interface
    ...
    for (unsigned int inode = 0; inode < nsNodes.size(); inode++) {
      ...
      if (JV != Teuchos::null) {
        for (int i=0; i<dirichlet_workset.num_cols_x; i++) {
          //TODO make sure that jac has not been already updated, otherwise we
    must not multiply by scale. JV_nonconst2dView[i][lunk] =
    scale*jac_diag->getData()[lunk]*Vx_const2dView[i][lunk];
        }
      }
