    ...
    if (numDims == 2) { //2D case
      ...
      for (std::size_t cell=0; cell < workset.numCells; ++cell) {
        for (std::size_t node=0; node < numNodes; ++node) {
          ...
          for (std::size_t qp=0; qp < numQPs; ++qp) {
             for (std::size_t i=0; i < vecDim; i++) {
                Residual(cell,node,i) = rhobar*CDot(cell,qp,i)*wBF(cell,node,qp); 
                if (i == vecDim-1)
                  Residual(cell,node,i) = CDot(cell,qp,i)*wBF(cell,node,qp); 
             }
          }
          for (std::size_t qp=0; qp < numQPs; ++qp) {
             Residual(cell, node, 0) += rhobar*ubar*Cgrad(cell,qp,0,0)*wBF(cell,node,qp) + rhobar*vbar*Cgrad(cell,qp,0,1)*wBF(cell,node,qp) 
                                     + rhobar*Rgas*Cgrad(cell,qp,2,0)*wBF(cell,node,qp) + Rgas*Tbar*Cgrad(cell,qp,3,0)*wBF(cell,node,qp) 
                                     + 1.0/Re*((2.0*mu + lambda)*Cgrad(cell,qp,0,0)*wGradBF(cell,node,qp,0) + lambda*Cgrad(cell,qp,1,1)*wGradBF(cell,node,qp,0) 
                                               + mu*Cgrad(cell,qp,1,0)*wGradBF(cell,node,qp,1) + mu*Cgrad(cell,qp,0,1)*wGradBF(cell,node,qp,1))
                                     + force(cell,qp,0)*wBF(cell,node,qp); //rhobar*ubar*du'/dx + rhobar*vbar*du'/dy + rhobar*R*dT'/dx + R*Tbar*drho'/dx + 
                                                                           ...
                                                                           //f0
             Residual(cell, node, 1) += rhobar*ubar*Cgrad(cell,qp,1,0)*wBF(cell,node,qp) + rhobar*vbar*Cgrad(cell,qp,1,1)*wBF(cell,node,qp) 
                                     + rhobar*Rgas*Cgrad(cell,qp,2,1)*wBF(cell,node,qp) + Rgas*Tbar*Cgrad(cell,qp,3,1)*wBF(cell,node,qp) 
                                     + 1.0/Re*(mu*Cgrad(cell,qp,1,0)*wGradBF(cell,node,qp,0) + mu*Cgrad(cell,qp,0,1)*wGradBF(cell,node,qp,0)
                                               + lambda*Cgrad(cell,qp,0,0)*wGradBF(cell,node,qp,1) + (2.0*mu + lambda)*Cgrad(cell,qp,1,1)*wGradBF(cell,node,qp,1))
                                     + force(cell,qp,1)*wBF(cell,node,qp); //rhobar*ubar*dv'/dx + rhobar*vbar*dv'/dy + rhobar*R*dT'/dy + R*Tbar*drho'/dy + 
                                                                           ...
                                                                           //f1
             Residual(cell, node, 2) += rhobar*Tbar*(gamma_gas - 1.0)*(Cgrad(cell,qp,0,0) + Cgrad(cell,qp,1,1))*wBF(cell,node,qp) 
                                     + rhobar*ubar*Cgrad(cell,qp,2,0)*wBF(cell,node,qp) + rhobar*vbar*Cgrad(cell,qp,2,1)*wBF(cell,node,qp) 
                                     + 1.0/Re*(gamma_gas*kappa/Pr*(Cgrad(cell,qp,2,0)*wGradBF(cell,node,qp,0) + Cgrad(cell,qp,2,1)*wGradBF(cell,node,qp,1)))
                                     + force(cell,qp,2)*wBF(cell,node,qp); //rhobar*Tbar*(gamma-1)*div(u') + rhobar*ubar*dT'/dx + rhobar*vbar*dT'/dy +
                                                                           ...
                                                                           //f2 
             Residual(cell, node, 3) += rhobar*(Cgrad(cell,qp,0,0) + Cgrad(cell,qp,1,1))*wBF(cell,node,qp) 
                                     + ubar*Cgrad(cell,qp,3,0)*wBF(cell,node,qp) + vbar*Cgrad(cell,qp,3,1)*wBF(cell,node,qp) 
      ...
      for (std::size_t cell=0; cell < workset.numCells; ++cell) {
        for (std::size_t node=0; node < numNodes; ++node) {
          ...
          for (std::size_t qp=0; qp < numQPs; ++qp) {
             Residual(cell, node, 0) += rhobar*ubar*Cgrad(cell,qp,0,0)*wBF(cell,node,qp) + rhobar*vbar*Cgrad(cell,qp,0,1)*wBF(cell,node,qp)
                                     + rhobar*wbar*Cgrad(cell,qp,0,2)*wBF(cell,node,qp) 
                                     + rhobar*Rgas*Cgrad(cell,qp,3,0)*wBF(cell,node,qp) + Rgas*Tbar*Cgrad(cell,qp,4,0)*wBF(cell,node,qp) 
                                     + 1.0/Re*((2.0*mu + lambda)*Cgrad(cell,qp,0,0)*wGradBF(cell,node,qp,0) + lambda*Cgrad(cell,qp,1,1)*wGradBF(cell,node,qp,0)
                                               + lambda*Cgrad(cell,qp,2,2)*wGradBF(cell,node,qp,0) 
                                               + mu*Cgrad(cell,qp,1,0)*wGradBF(cell,node,qp,1) + mu*Cgrad(cell,qp,0,1)*wGradBF(cell,node,qp,1)
                                               + mu*Cgrad(cell,qp,2,0)*wGradBF(cell,node,qp,2) + mu*Cgrad(cell,qp,0,2)*wGradBF(cell,node,qp,2))
             ...
             Residual(cell, node, 1) += rhobar*ubar*Cgrad(cell,qp,1,0)*wBF(cell,node,qp) + rhobar*vbar*Cgrad(cell,qp,1,1)*wBF(cell,node,qp)
                                     + rhobar*wbar*Cgrad(cell,qp,1,2)*wBF(cell,node,qp)  
                                     + rhobar*Rgas*Cgrad(cell,qp,3,1)*wBF(cell,node,qp) + Rgas*Tbar*Cgrad(cell,qp,4,1)*wBF(cell,node,qp) 
                                     + 1.0/Re*(mu*Cgrad(cell,qp,1,0)*wGradBF(cell,node,qp,0) + mu*Cgrad(cell,qp,0,1)*wGradBF(cell,node,qp,0)
                                               + lambda*Cgrad(cell,qp,0,0)*wGradBF(cell,node,qp,1) + (2.0*mu + lambda)*Cgrad(cell,qp,1,1)*wGradBF(cell,node,qp,1)
                                               + lambda*Cgrad(cell,qp,2,2)*wGradBF(cell,node,qp,1) 
                                               + mu*Cgrad(cell,qp,2,1)*wGradBF(cell,node,qp,2) + mu*Cgrad(cell,qp,1,2)*wGradBF(cell,node,qp,2))
             ...
             Residual(cell, node, 2) += rhobar*ubar*Cgrad(cell,qp,2,0)*wBF(cell,node,qp) + rhobar*vbar*Cgrad(cell,qp,2,1)*wBF(cell,node,qp) 
                                     + rhobar*wbar*Cgrad(cell,qp,2,2)*wBF(cell,node,qp) 
                                     + rhobar*Rgas*Cgrad(cell,qp,3,2)*wBF(cell,node,qp) + Rgas*Tbar*Cgrad(cell,qp,4,2)*wBF(cell,node,qp)
                                     + 1.0/Re*(mu*Cgrad(cell,qp,2,0)*wGradBF(cell,node,qp,0) + mu*Cgrad(cell,qp,0,2)*wGradBF(cell,node,qp,0) 
                                               + mu*Cgrad(cell,qp,2,1)*wGradBF(cell,node,qp,1) + mu*Cgrad(cell,qp,1,2)*wGradBF(cell,node,qp,1) 
                                               + lambda*Cgrad(cell,qp,0,0)*wGradBF(cell,node,qp,2) + lambda*Cgrad(cell,qp,1,1)*wGradBF(cell,node,qp,2) 
                                               + (2.0*mu + lambda)*Cgrad(cell,qp,2,2)*wGradBF(cell,node,qp,2)) 
             ...
             Residual(cell, node, 3) += rhobar*Tbar*(gamma_gas - 1.0)*(Cgrad(cell,qp,0,0) + Cgrad(cell,qp,1,1) + Cgrad(cell,qp,2,2))*wBF(cell,node,qp) 
                                     + rhobar*ubar*Cgrad(cell,qp,3,0)*wBF(cell,node,qp) + rhobar*vbar*Cgrad(cell,qp,3,1)*wBF(cell,node,qp) 
                                     + 1.0/Re*(gamma_gas*kappa/Pr*(Cgrad(cell,qp,3,0)*wGradBF(cell,node,qp,0) + Cgrad(cell,qp,3,1)*wGradBF(cell,node,qp,1) + Cgrad(cell,qp,3,2)*wGradBF(cell,node,qp,2)))
                                     + force(cell,qp,3)*wBF(cell,node,qp); //rhobar*Tbar*(gamma-1)*div(u') + rhobar*ubar*dT'/dx + rhobar*vbar*dT'/dy + rhobar*wbar*dT'/dz +
                                                                           ...
                                                                           //f3
             Residual(cell, node, 4) += rhobar*(Cgrad(cell,qp,0,0) + Cgrad(cell,qp,1,1) + Cgrad(cell,qp,2,2))*wBF(cell,node,qp) 
                                     + ubar*Cgrad(cell,qp,4,0)*wBF(cell,node,qp) + vbar*Cgrad(cell,qp,4,1)*wBF(cell,node,qp) + wbar*Cgrad(cell,qp,4,2)*wBF(cell,node,qp)
