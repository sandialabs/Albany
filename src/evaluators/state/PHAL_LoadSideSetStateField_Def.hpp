...
{
  printf("loading node state %s on ss %s\n",stateName.c_str(),sideSetName.c_str());
  ...
  if (stateName=="observed_surface_velocity") {
    std::cout << stateName << ":\n";
    std::cout << " bstate:\n";
    for (int i=0; i<state_h.extent_int(0); ++i) {
      for (int j=0; j<state_h.extent_int(1); ++j) {
        std::cout <<   "(cell,node), vals: (" << i << "," << j << "),";
        for (int k=0; k<state_h.extent_int(2); ++k) {
          std::cout << " " << state_h(i,j,k);
        }
        std::cout << "\n";
      }
    }
  ...
  for (int sideSet_idx = 0; sideSet_idx < sideSet.size; ++sideSet_idx) {
    ...
    for (int side_node=0; side_node<state_h.extent_int(1); ++side_node) {
      ...
      switch (state_h.rank()) {
        ...
        case 3:
          // if (stateName=="observed_surface_velocity") {
          //   std::cout << "  side node: " << side_node << "\n";
          //   std::cout << "   vals:";
          // }
          for (int idim=0; idim<state_h.extent_int(2); ++idim) {
            field_h_mirror(sideSet_idx,side_node,idim) = state_h(icell,ss_cell_node,idim);
            // std::cout << " " << field_h_mirror(sideSet_idx,side_node,idim);
            // if (stateName=="observed_surface_velocity")
            //   std::cout << " " << state_h(icell,ss_cell_node,idim);
          }
          // if (stateName=="observed_surface_velocity")
          //   std::cout << "\n";
          break;
...
{
  printf("loading elem state %s on ss %s\n",stateName.c_str(),sideSetName.c_str());
