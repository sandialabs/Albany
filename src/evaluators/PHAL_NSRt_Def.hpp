...
//  this->addEvaluatedField(TResidual);
 ...
 /* if (p.isType<std::string>("Convection Velocity")) {
  ...
  this->setName("NSRt"+PHX::TypeString<EvalT>::value);
}

//**********************************************************************
template<typename EvalT, typename Traits>
void NSRt<EvalT, Traits>::
                      ...
                      PHX::FieldManager<Traits>& fm)
{
  ...
  if (haverhoCp) this->utils.setFieldData(rhoCp,fm);
  //this->utils.setFieldData(TResidual,fm);
  this->utils.setFieldData(Rt,fm);
}
    ...
    for (std::size_t cell=0; cell < workset.numCells; ++cell) {
      for (std::size_t qp=0; qp < numQPs; ++qp) {
        ...
        for (std::size_t i=0; i < numDims; ++i) {
          if (haverhoCp)  
            convection(cell,qp) += rhoCp(cell,qp) * V(cell,qp,i) * TGrad(cell,qp,i);
          else
            convection(cell,qp) += V(cell,qp,i) * TGrad(cell,qp,i);
        }
      }
    }
      ...
      for (std::size_t node=0; node < numNodes; ++node) {          
         for (std::size_t qp=0; qp < numQPs; ++qp) {               
            for (std::size_t j=0; j < numDims; ++j) { 
              TResidual(cell,node) += TauT(cell,qp)*acceleration(cell,qp,j)*V(cell,qp,j)*wGradBF(cell,node,qp,j);
            }  
         }
       }
    }*/  
